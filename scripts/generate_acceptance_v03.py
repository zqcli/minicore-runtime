...
def render_acceptance(mapping: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "| GitHub Actions CI | [Run 32705101762](https://github.com/zqcli/minicore-runtime/actions/runs/32705101762) passed (Rust stable/Clippy 1.98, Rust 1.85.0 MSRV, `macos-latest`, `windows-latest` MSVC) |",
        "",
        "## OS Matrix",
        "",
        ...
        "| macOS | Passed | Native `macos-latest` job passed in GitHub Actions [run 32705101762](https://github.com/zqcli/minicore-runtime/actions/runs/32705101762). |",
        "| Windows | Passed | Native MSVC `windows-latest` job passed in GitHub Actions [run 32705101762](https://github.com/zqcli/minicore-runtime/actions/runs/32705101762). |",
        "",
        "All functional acceptance criteria passed on Linux, and native macOS and Windows CI jobs passed. Release validation is complete and ready for publication.",
        "",
    ]
    current_group = None
    ...
    lines.extend(
        [
            ...
            "",
            "All functional rows above are Passed on Linux. Cross-platform release validation is complete across Linux, macOS, and Windows. The repository is ready for publication, though crate/tag release publication has not yet been executed. See the [v0.3 release note](release-v0.3.md) and [migration guide](migrations/v0.2-to-v0.3.md).",
            "",
        ]
    )
    return "\n".join(lines)
