...
fn p6_observation_surface_is_precise_and_keeps_owners_private() {
    ...
    assert!(session.contains("pub(crate) mod legacy_event_stream;"));
    assert!(session.contains("pub use event_stream::SessionEventStream;"));
    assert!(!session.contains("pub use legacy_event_stream"));
    ...
    let stream = include_str!("../src/session/legacy_event_stream.rs");
    for required in [
        "now_utc",
        "pub(crate) async fn usage",
        "MAX_EVENT_CAPACITY",
        "LegacySessionObservation",
        "LegacySessionEventStream",
        "pending_resync_snapshot",
        "watch::Sender<LegacySessionSnapshot>",
        "broadcast::Sender<LegacySessionEvent>",
        "publish_snapshot",
        "publish(",
        "subscribe",
        "ResyncRequired",
