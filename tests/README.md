...
`tests/m14_live_provider_smoke.rs`包含两个explicit opt-in、real-network smoke tests：通过完整public host/runtime path（config → `MiniCoreRuntime::open` → catalog resolution → Agent/Session Create/Load → Submit → env-backed dynamic `CredentialSource` → direct provider adapter → real protocol terminal）分别驱动真实OpenAI Responses与Anthropic Messages API。两个tests都是`#[ignore]`d，**从不**在`./scripts/check.sh`、`./scripts/check-msrv.sh`或默认`cargo test`（包括`cargo test --all-targets`）中执行——这些gate只编译它们。必须显式运行`--ignored`才会执行，且每个test仍然要求其provider的opt-in env var恰好为`1`并具备完整的documented env set，否则test以仅点名缺失变量、绝不打印变量值/credential/endpoint/model值的消息panic。
