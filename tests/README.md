...
- `tests/tool_driver_contract.rs` source-checks the private canonical P5-B ToolDriver and suspension protocol, legacy agent quarantine, forbidden owner dependencies, narrow ToolSet/progress/input capabilities, file bounds, and unchanged public surfaces; behavioral coverage lives under `src/agent/tool_driver/tests/`.
- `tests/context_prompt_driver_contract.rs` source-checks the private canonical P5-C ContextDriver and final PromptBuilder, conversation-owned validated prompt projection, exact final-request serialization estimates, forbidden owner/adaptor dependencies, legacy prompt quarantine, deterministic roles, file bounds, and unchanged public surfaces; behavioral coverage lives under `src/context/driver/tests/`, `src/conversation/view/tests.rs`, and `src/prompt/builder/tests/`.
- `tests/compaction_driver_contract.rs` source-checks the private canonical P5-D CompactionDriver, conversation-owned validated candidate proof, stale-head result without commit authority, forbidden owner/adaptor dependencies, public Compaction Port stability, and file bounds; behavioral coverage lives under `src/conversation/view/tests/compaction.rs` and `src/compaction/driver/tests/`.
...
- Final SessionHandle commands and turn/actor execution acceptance remain deferred to P4-C/P5-E; P4-B does not claim submit support.
