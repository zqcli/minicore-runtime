...
- `model_driver_contract.rs`, `tool_driver_contract.rs`, `context_prompt_driver_contract.rs`, and `compaction_driver_contract.rs` source-check the private drivers, authority boundaries, deadline provenance, panic/cancellation behavior, focused file limits, the checked ContextBundle-to-PromptBuilder seam, and the PromptPlan seam. ContextDriver tests cover provider validation/canonical ordering and child-before-Drop cancellation; PromptBuilder tests consume only driver-checked context and prove one projection per plan plus independent fixed/final serialization oracles, while runner compaction tests prove that two valid completed boundaries still yield one proactive strategy/commit and exactly two projections, forced recovery does not retry proactive after the head changes, and the ToolDriver moves an in-bound completed ToolOutput without reconstructing it while retaining the exact over-bound failure. `turn_runner_contract.rs` source-locks the private `port_call` module, its four allowed callers, the distinct `InvalidDeadline` outcome, and the exclusion of ModelDriver, ConversationLog/SessionLog, Interaction waits, and critical channels. Compaction source/behavior tests lock candidate validation and the test pause outside the helper, direct helper entry, closure-first `Ok(None)` availability handling, and cancellation/deadline classification before strategy invocation; forced races remain Cancelled/BudgetExceeded rather than CompactionFailure. PromptBuilder does not repeat context validation. Behavioral tests live beside those private modules.
- `turn_runner_contract.rs` protects ordinary execution, exact delta/Summary acknowledgements, commit/suspend-only critical traffic, join-owned completion, durable rounds, compaction recovery, critical taxonomy, cancellation-first control, usage retention, and owner neutrality.
