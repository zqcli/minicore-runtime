...
- Public `tools::Tool`, immutable `tools::ToolSet`, checked invocations, content-only outputs, input requests, cancellation, deadlines, and best-effort progress.
...
| `tools` | `Tool`, immutable `ToolSet`, invocation/context/progress/output DTOs |

The old `runtime` and `workspace` modules, `RuntimeConfig`, `SessionConfig`, `SessionSummary`, `ToolRegistry`, and legacy policy are not public compatibility surfaces. `legacy_context` and `legacy_types` are private migration scaffolding scheduled for P3-C/P6 deletion or replacement.

## Transitional Scope

The removed v0.2 Runtime, concrete builtin/process, and public Tool integration tests are baseline evidence for the reset, not current extension contracts. `tests/tool_set_contract.rs` is the focused P3-B replacement. SessionRuntime acceptance coverage is intentionally deferred to P4/P5; this revision does not claim that the full replacement is complete.
...
The deterministic offline checks are Python/docs/diff/architecture checks. Rust validation for this phase is intentionally run remotely by the project workflow; no local Rust build or test command is part of the P3-B review.
