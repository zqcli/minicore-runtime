...
本仓库定义并逐步实现可复用的Agent运行时核心能力：Runtime协议、Session control actor、async Turn loop、资源加载、技能、工具、`CommandSurface`、live observer事件、best-effort Session recording、上下文压缩、`ModelGateway`和usage stats。M4已实现Prompt-owned opaque `ModelMessage`与纯内存LiveConversation reducer；M5.0 recovery/root lease/owner-tracked actor、private permanent reservation foundation与crate-private Agent Create exact G1 publication已实现，仍无standalone reservation API/token/receipt；public Runtime Agent command接入、Session Create/Fork/update/CAS、完整cross-platform native matrix、SessionRecorder/replay与后续behavioral slices仍待交付。CLI、TUI和GUI产品在独立仓库中开发。
