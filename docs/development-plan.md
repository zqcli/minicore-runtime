...
退出条件：每个adapter独立通过contract suite，未配置credential时整个默认test suite仍可运行。当前仅OpenAI direct adapter/local contract slice完成，M14尚未关闭。
