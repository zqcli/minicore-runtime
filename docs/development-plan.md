...
production direct ProviderAdapter和OS/network/process Tool不属于上述internal milestone。V4-P1-3已由M12关闭，V4-C0-1已由M13/ADR 0140关闭；production Provider与Tool/Sandbox实现仍按M14交付，并必须消费对应contract suite。
...
`M2`按行为slice增量扩展，不再先横向实现全部DTO；`M3`只关闭line codec与physical scanner，semantic corruption sidecar由`M5`关闭。`M7`必须同时等待minimal public DTO、reducer、record/replay和Turn resources。M13已关闭；M14 production Tool/Sandbox adapters必须遵守ADR 0140，首个file-mutation adapter还必须先满足ADR 0116。
...
- 第四轮review在全部finding关闭后继续作为current closed review保留；research继续保留但明确标记non-authoritative；
...
当前进度：Completed（scripted foundation）。`PromptSet::assemble`把固定System sections、ordered User static context和sanitized `LiveConversationView`组装为唯一`AssembledModelContext`，使用pinned `TurnModelSnapshot` estimator/context limit执行final input preflight，proof绑定AgentRun、exact process-local `TurnModelRef`和ConversationRevision；Model catalog resolution、retained `TurnModelSnapshot`、唯一`ModelCallRequest` constructor、single `ProviderAdapter::execute` attempt、provider-neutral progress、typed delivery-aware errors、minimal text terminal validation及cancel/terminal first-wins均已实现。M8.1增加最小ToolCall，M10增加CompactionSummary purpose、budget proof、explicit max-output request validation与ActiveTurnTask orchestration。unsafe `AcceptedNoOutput | Unknown | OutputStarted`不会保留包括`RateLimited`在内的retryable reason。Runtime在未配置provider时拥有empty `ModelGateway`与initial immutable empty `ModelCatalogView`；trusted host现在可通过validated `ModelProviderConfig`安装OpenAI Responses/Anthropic Messages static model sources，reload只影响future snapshot，旧snapshot继续调用旧adapter。credential由public host-only `CredentialSource`在每次attempt前动态解析，缺失为`AuthMissing/NotSent`且不调用adapter；stable `ModelId`与private API model name分离，provider terminal回报的model必须exact匹配。crate-private Structured output foundation已实现（`OutputContract::Structured` exact-model contract、schema v1 subset、terminal本地schema validation与ScriptedProviderAdapter conformance），provider-native schema mapping及两个explicit ignored live smoke harness亦已在M14实现；public structured activation与实际real-credential smoke run仍Pending；其中public Structured若进入SessionDefinition/Wire必须使用新protocol minor与Durable Store migration，因为V1 public/storage schemas是closed exact contracts，不能通过optional字段漂移；显式cache/continuation不是pending实现——M14 stateless full-request wire policy已由[ADR 0141](adr/0141-provider-calls-are-stateless-full-request.md)冻结（每次`generate_model_turn`至多调用一次`ProviderAdapter::execute`、独立地发送零或一个HTTP POST：owner validation/pre-send cancellation/`AuthMissing`在调用adapter前以typed error terminal（零execute/零POST），adapter编码/build失败或adapter级pre-send cancellation为一次execute/零POST，若发送POST则携带完整full request，OpenAI store=false且无previous_response_id/prompt cache字段，Anthropic递归无cache_control/无anthropic-beta header，Anthropic `provider_total_tokens`恒为`None`）。
...
1. 首个production `ProviderProtocol` baseline只保留OpenAI Responses与Anthropic Messages；默认Runtime不内置或自动发布model，只有trusted host显式安装的validated definition可解析。protocol adapter与dynamic credential/catalog installation已完成，explicit live smoke harness已实现但实际real-credential release run仍需记录；OpenAI Chat Completions/Gemini等未验证protocol不接受；
...
状态：Completed。ADR 0140冻结closed `FilesystemRead | FilesystemWrite | Network | Process` class set、final `ToolPermissionSet` narrowing、adapter `Available(enforceable) | Unavailable` contract、exact capability gap与fixed `PreExecution + Denied` mapping。Direct Execute在任何ToolStartGate reservation/start factory poll前admit；Execute/Approval各自携带唯一permissions，host AllowOnce/Restricted AllowWith后复验ceiling与captured Sandbox，失败不进入start gate。非空、已admit permission plan的adapter-independent Session round conformance覆盖SecurityRevoked-before-start、Sandbox unavailable与Running cooperative truthful settlement。
...
6. fake backend完成SecurityRevoked、Sandbox unavailable与Running Tool truthful settlement contract tests；
7. Session-local file mutation queue不属于V4-C0-1：ADR 0116继续有效，首个production file-mutation adapter必须用真实authorized canonical target实现same-Session alias FIFO与cross-Session no coordination；没有consumer前不创建测试专用queue。

退出条件已满足：O1/R7/V4-C0-1由ADR 0140和adapter-independent conformance正式关闭；closure SHA `0951b12`的GitHub Actions run `31518732896`四平台全部success。production permission producer、resource-level grants与adapter仍属于M14。

## M14 · Production Adapters

状态：In Progress。M12 provider gate与M13 Tool/Sandbox gate均已关闭，按独立adapter交付：

- [x] OpenAI Responses direct private `ProviderAdapter`：exact `reqwest = 0.13.4`最小features、single POST、no retry/redirect/proxy、explicit endpoint、ordered request/terminal mapping、provider-native Structured strict schema、bounded fragmented SSE、metadata redaction、delivery/error和cancellation；success额外要求terminal `response.model` exact匹配pinned private API model name。35个默认离线测试通过真实`ModelGateway.generate_model_turn → ProviderAdapter`与`127.0.0.1:0/responses`路径，stable及真实Rust 1.85 all-target/focused门禁通过；
- [x] Anthropic Messages direct private `ProviderAdapter`：复用已证明的private reqwest client/bounded drain/SSE framing，但独立拥有`/v1/messages` request、non-empty `message_delta.stop_reason` terminal、thinking/signature/redacted replay、Structured+adaptive effort、service tier、usage/cache与typed envelope/delivery mapping；`message_start.message.model`必须exact匹配pinned private API model name，`x-api-key` HeaderValue显式标记sensitive。38个默认离线测试通过真实`ModelGateway.generate_model_turn → ProviderAdapter`与`127.0.0.1:0/v1/messages`路径，stable及真实Rust 1.85 all-target/focused门禁通过；
- [x] shared host-only dynamic credential/catalog installation：public redacted `ProviderCredential`/`CredentialSource`、provider-neutral explicit model descriptor、HTTPS-only或显式numeric-loopback HTTP endpoint policy、OpenAI/Anthropic route constructors及`MiniCoreRuntimeConfig::with_model_provider`；credential每attempt解析，missing/cancel在adapter前保持`AuthMissing|Cancelled/NotSent`，catalog availability不读取credential，stable `ModelId`与private API model name分离；11个installation、3个credential与Runtime installation tests通过stable/真实Rust 1.85；默认无config仍为空catalog；
- [x] explicit opt-in live smoke harness：`tests/m14_live_provider_smoke.rs`使用public Runtime path，两个tests默认ignored，stable/真实Rust 1.85完成`--no-run`，默认运行2 ignored且无env/network；
- [x] stateless full-request wire policy（[ADR 0141](adr/0141-provider-calls-are-stateless-full-request.md)）：每次`generate_model_turn`至多调用一次`ProviderAdapter::execute`，独立地发送零或一个HTTP POST（owner validation/pre-send cancellation/`AuthMissing`→零execute/零POST；adapter编码/build失败或adapter级pre-send cancellation→一次execute/零POST；若发送POST则携带完整full request），无optimization-specific fallback POST/重试/continuation state；OpenAI保留`store=false`且任何protocol/schema节点无provider-owned `previous_response_id`/`prompt_cache_key`/`prompt_cache_retention`/`cache_control`/`conversation`成员（user-authored schema property names保持为data）；Anthropic递归无provider-owned `cache_control` annotation、无`anthropic-beta` header、若发送则始终完整system/messages/tools；credential逐attempt动态解析（不pin到request）；Anthropic `provider_total_tokens`恒为`None`（provider-reported only，无derived sum/overflow路径）；recursive request denylist与same-Arc两次invocation（恰好各一次POST、body逐字节相同、auth随两次credential变化）loopback tests已补齐；
- [ ] 使用显式真实credential/network执行OpenAI与Anthropic live smoke并记录release evidence；credential不得进入命令行、日志或CI fixture；
- [x] production `ask_user` builtin（[ADR 0142](adr/0142-production-ask-user-is-a-closed-opt-in-builtin.md)）：closed、default-off、Runtime-owned；`MiniCoreRuntimeConfig::with_ask_user_tool()` idempotent opt-in与`ToolSet::ask_user_builtin()`生产构造（唯一实现位置`src/tools/ask_user.rs`，既有residency capture路径安装）；exact ToolName/description/closed schema披露；零`ToolCapabilityClass` permission + available empty sandbox contract；arguments经private strict serde mirrors解析（每层deny_unknown_fields，omitted/null title→None，indices严格递增但不要求连续）；仅返回UserQuestion或frozen `PreExecution + Failed`（恰一个Text part、`tool arguments are invalid`、无Interaction）plans，绝不创建ToolExecutionStart/executor future/cancellation pair/start-gate reservation/approval/OS资源；answer binding先经exact `validate_answer`验证再产生`PreExecution + Succeeded`恰一个deterministic compact JSON Text part（`{"answers":[{"questionIndex":3,"value":{"type":"text","data":"hello"}}]}`或choice `{"answers":[{"questionIndex":7,"value":{"type":"choice","data":{"optionIndex":11}}}]}`，ascending order、optional未答`{"answers":[]}`；escaping canonical/deterministic、输出受既有`ToolResultContent`约束，render/invariant失败fail closed为identity-bound Abandoned RuntimeFailure）；默认Runtime ToolSet保持空、无新Runtime capability/Wire/Store变化；
- [x] production `read_file` builtin（[ADR 0143](adr/0143-production-read-file-uses-workspace-capabilities.md)）：closed、default-off、Workspace-bound首个narrow OS-backed本地read slice；`MiniCoreRuntimeConfig::with_read_file_tool()` idempotent opt-in且与`with_ask_user_tool()`/`with_list_directory_tool()`相互独立（当前三个bool的frozen顺序`ask_user`→`read_file`→`list_directory`），`open`冻结单一production Tool config并选择共享read-only resolver；cwd-relative-only单一required `path`（strict `WorkspaceRelativePath` grammar：无absolute/dot segment、≤4,096 bytes、≤256 segments；无offset/range/encoding/options、无write/network/process）；ToolSet per-admission对exact captured Workspace materialize（`ProductionToolConfig`/`TurnToolResources::Production`，非single static ToolSet），默认Runtime ToolSet保持空；ReadOnly authority ceiling（`ReadOnlyWorkspaceAuthority`：每个declared root恰为`ReadOnly`、永不`ReadWrite`，Prompt/Skill source ceiling false，trust Restricted）；Execute plan恰带`ToolCapabilityClass::FilesystemRead` + sandbox contract available exactly for `FilesystemRead`（按ADR 0140对该single class复验admission）；capability-relative cap-std open（`AuthorizedWorkspaceReadPath::open_nonblocking`经captured root `Dir`解析，symlink escape在该open失败；无ambient path/`std::fs::read`/`canonicalize`）；至多65,537字节bounded读取→恰一个≤65,536字节UTF-8 Text part，frozen `PreExecution + Failed/Denied`与`Completed + Failed/Cancelled` texts；blocking job owner-tracked（`spawn_blocking_tracked`，一旦scheduled绝不drop/detach，仅`RuntimeTaskError`→`Abandoned{RuntimeFailure}`）；host `invalidate_session_workspace_authority`先经`WorkspaceReadAccessControl::revoke`永久撤销该Session read grant（idempotent、本Runtime lifetime内无unrevoke）再采样timestamp signal/re-resolve，recovery re-resolve只授予filesystem `None`、绝不恢复`ReadOnly`，restriction在residency返回SessionNotLoaded/Closing/internal时仍保持current；cleanup边界诚实：bytes/application work有界（固定65,537-byte预算）、special files以nonblocking capability open拒绝且不挂起、ordinary regular file的wall-clock完成依赖OS/filesystem且不宣称timeout；focused tests覆盖schema/parse/authorization/denial/symlink-escape/boundary/UTF-8/cancellation/FIFO/runtime-failure及Runtime opt-in端到端与authority revocation；
- [x] production `list_directory` builtin（[ADR 0144](adr/0144-production-list-directory-uses-bounded-capability-enumeration.md)）：closed/default-off、`with_list_directory_tool()` idempotent且与ask/read独立；与read_file共享ReadOnly authority、per-Session permanent revocation与per-admission exact WorkspaceSnapshot capability basis；empty path表示cwd，其他path仅cwd-relative `WorkspaceRelativePath`；opaque `AuthorizedWorkspaceReadDirectory`经captured root `Dir` capability-relative open，目标symlink escape fail closed；只枚举direct entries，不递归/不读content，entry symlink不跟随，type恰为`file|directory|symlink|other`；最多256 entries、retained UTF-8 name bytes≤8,192、compact sorted JSON≤65,536 bytes，fixed failure/cancel texts；一个owner-tracked blocking job，scheduled后no detach/truthful settlement；`ProductionToolConfig`扩为三个bool/八种closed selection，fixed order `ask_user → read_file → list_directory`；Runtime disclosure/e2e/invalidation后future Denied focused tests通过；不改Wire V1/Store V1/public DTO；
- [ ] production Tool/Sandbox adapters，每个声明并证明effective enforcement capability（read_file/list_directory两个narrow OS-backed read slices已完成；write/network/process及其他未实现adapter仍待交付）；
...
退出条件：每个adapter独立通过contract suite，未配置credential时整个默认test suite仍可运行。当前两个provider direct adapter/local contract slices、stateless full-request wire policy（ADR 0141）、dynamic credential/catalog installation与explicit ignored live smoke harness、production `ask_user` builtin（ADR 0142）及narrow OS-backed production `read_file`/`list_directory` builtins（ADR 0143/0144：closed/default-off、共享ReadOnly authority/revocation、cwd-relative capability enforcement、bounded deterministic outputs、owner-tracked no-detach jobs、per-admission Workspace-bound materialization）均已完成；实际real-credential smoke run与其余production OS-backed Tool/Sandbox adapters（write/network/process及其他未实现adapter）仍未完成，M14尚未关闭。
...
M2–M13 prerequisites与production gates均已关闭；当前进入M14。production Provider与Tool/Sandbox adapters分别遵守ADR 0138/0139与ADR 0140，首个file-mutation adapter还必须满足ADR 0116。
