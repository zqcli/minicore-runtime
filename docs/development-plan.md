...
production Rig ProviderAdapter和OS/network/process Tool不属于上述internal milestone。V4-P1-3已由M12关闭，production Provider实现仍按M14交付；OS/network/process Tool继续受M13/V4-C0-1门禁约束。
...
状态：Completed。V4-P1-3由[ADR 0138](adr/0138-production-provider-baseline-uses-verified-rig-contracts.md)、[M12 provider fixture](fixtures/provider-gate-m12/README.md)与真实Rig loopback contract tests关闭；本里程碑没有实现production adapter。
...
4. exact dev-only `rig-core = 0.40.0`通过两协议真实`127.0.0.1:0` unary/stream tests，覆盖system/instructions、ordered messages、Tool schema与identity/order、OpenAI reasoning/structured request、Anthropic thinking/signature/cache-control、usage、finish/terminal、body/header IDs、base URL、cancel、fragmented SSE、drop与early EOF；
...
8. `rig`只在dev-dependencies和`tests/m12_*`中出现；`src/`、Prompt、Conversation、Runtime public DTO与Wire V1均未引入Rig类型。

退出条件已满足：V4-P1-3为Closed并有独立ADR、fixture、real mock-server与MSRV evidence。production adapters继续属于M14。
