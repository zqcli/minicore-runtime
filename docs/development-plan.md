...
退出条件：每个adapter独立通过contract suite，未配置credential时整个默认test suite仍可运行。当前两个provider direct adapter/local contract slices、dynamic credential/catalog installation与explicit ignored live smoke harness均已完成；实际real-credential smoke run及production Tool/Sandbox adapters仍未完成，M14尚未关闭。
