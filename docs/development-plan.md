...
当前进度：M9.1 已完成最小 crate-private FollowUp FIFO seam：bounded admission、duplicate command rejection、按 CommandId remove 基础与 terminal handoff；SessionExecutor 仅在 active Turn 期间接受 FollowUp，当前 Turn terminal 后启动最多一条 queued FollowUp。完整 Steer lane、FollowUp public projection、queued-message cancellation、fair admission、logical retry与Skill composition仍后置。
