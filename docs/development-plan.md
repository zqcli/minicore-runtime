...
当前实现状态：M8.3的round-local gate foundation已完成——assistant ToolCall apply后、record await前构建per-round `RoundToolStartGate`，每个exact `ToolExecutionRequest`（ItemId + same `Arc<ToolCall>`）绑定一个`ToolStartGate` lock-free atomic slot（Prepared → Reserved → Started | Cancelled），reservation在EmergencyControl owner mutex内对exact unsignaled target/epoch执行lock-free CAS（与`signal`在同一mutex线性化，first-wins），move-only `ToolStartPermit`经Reserved→Started产生typed `ToolStartedExecution` proof，executor future只在proof后poll；signal/stale先赢→matching PreExecution Cancelled ToolResult apply+inline record后Turn Interrupted且不发起下一次Model；reservation/start先赢→exact Executed/Abandoned truthful settle；pre-start plan分Execute/PreExecution/Interaction三路，Interaction在gate前publish、allow后same gate revalidate、deny/cancel不reserve；serial/parallel保持call_index order；parent-owned join_all+catch_unwind使panic恰好映射Abandoned。Running Tool best-effort cancellation handle/teardown与完整ToolOperationSlot Running/Settling/Terminal owner state仍pending。
