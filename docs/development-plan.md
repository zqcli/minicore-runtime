...
2. **fixture是历史规范资产**：历史Rust runner直接消费归档的`docs/archive/v2/fixtures/wire-v1/`，不得复制一份测试schema或在测试中重新发明expected结果。
...
- consume archived `docs/archive/v2/fixtures/durable-store-v1/`, including native Linux/macOS/Windows process tests for lock contention/reacquire, create_new, aliases, links/reparse, holder death, cleanup/open-handle and deterministic crash matrix points.
...
状态：Completed。V4-P1-3由[ADR 0138](adr/0138-production-provider-baseline-uses-verified-rig-contracts.md)、[ADR 0139](adr/0139-rig-is-evidence-only-under-rust-1-85.md)、[Archived M12 provider fixture](archive/v2/fixtures/provider-gate-m12/README.md)、真实Rig standalone loopback evidence与真实Rust 1.85冷编译关闭；本里程碑没有实现production adapter。
...
7. Session-local file mutation queue不属于V4-C0-1：ADR 0116继续有效；后续ADR 0146已用真实`write_file` consumer交付capability-opened physical target、same-Session alias FIFO、cross-Session no coordination与permit-through-Settling。
...
- [x] production `read_file` builtin（[ADR 0143](adr/0143-production-read-file-uses-workspace-capabilities.md)）：closed/default-off、cwd-relative bounded UTF-8 regular-file read、ReadOnly authority ceiling、opaque capability open、`FilesystemRead` exact sandbox与owner-tracked truthful settlement；
