...
退出条件：每个adapter独立通过contract suite，未配置credential时整个默认test suite仍可运行。当前两个provider direct adapter/local contract slices、stateless full-request wire policy（ADR 0141）、dynamic credential/catalog installation与explicit ignored live smoke harness、production `ask_user` builtin（ADR 0142，closed/default-off/opt-in）与首个narrow OS-backed production `read_file` builtin（ADR 0143，closed/default-off/ReadOnly ceiling/cwd-relative/capability-relative/bounded UTF-8/owner-tracked job/per-admission Workspace-bound materialization，host invalidation先永久revoke read grant再signal/re-resolve）均已完成；实际real-credential smoke run与其余production OS-backed Tool/Sandbox adapters（write/network/process、其它file adapter）仍未完成，M14尚未关闭。
