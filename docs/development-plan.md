...
退出条件：每个adapter独立通过contract suite，未配置credential时整个默认test suite仍可运行。当前两个provider direct adapters、stateless full-request policy、dynamic credential/catalog、ignored live smoke、`ask_user`与filesystem `read_file`/`list_directory`/`write_file`均已完成；provider live evidence由ADR 0145关闭，首个mutation adapter由ADR 0146关闭。network/process及其他adapters、generic ToolService/schema/hooks/full policy/approval、public Structured activation、public Tool DTO与concrete Prompt/Skill sources仍未完成，M14尚未关闭。
