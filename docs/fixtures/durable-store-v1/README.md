...
The durable M5.0 exit consumes every `slice = m5_0 | platform_m5_0` case. M5.1 consumes all `slice = m5_1` Recorder cases. The first behavioral Runtime slice M7 consumes `slice = m7` for owner-settled loaded Workspace publication; M5.0 must not prematurely implement SessionExecutor command behavior merely to satisfy that coordinate. Native macOS/Windows process CI remains an implementation exit condition; this design gate does not change CI.
