...
状态：当前权威 physical format specification；M5.0 design gate 已冻结，implementation pending
日期：2026-08-03
...
`MINICORE_STORE_V1`, every reservation, `PUBLISHED`, and `COMMITTED` are regular files of exactly zero bytes. They are existence-only; a directory, symlink, reparse point, nonzero file or any alternative byte representation is invalid. `.minicore.lock` is a permanent regular lock file and is not an existence-only marker. `definition.json` is present only in a generation introducing a new domain definition. Every recognized descendant remains on the root's filesystem/volume identity; a recognized mount, reparse or cross-volume point is invalid.
...
An AgentId directory is exactly canonical `agt_<32 lowercase hex>`; a SessionId directory is exactly canonical `ses_<32 lowercase hex>`. Names with different case, lossy/non-UTF-8 spelling, a recognized link/reparse point, or any unexpected recognized namespace entry fail strict recovery. Enumerators parse canonical names, sort them bytewise, count all entries toward their relevant cap, then process; filesystem enumeration order is not meaningful.
...
3. sort and validate the exact root namespace; reject aliases, links/reparse points, cross-filesystem/volume descendants, non-UTF-8 recognized names and unexpected entries;
4. scan capped reservations (`agents` and `sessions`) and each capped entity namespace; each PUBLISHED ID must have its matching permanent reservation;
5. for each PUBLISHED entity, scan capped generation names in numerical order, require a contiguous committed chain, validate every retained referenced definition and every adjacent semantic transition; never fall back from corruption;
6. after handles close, remove only exact markerless/uncommitted operation staging with its matching permanent reservation and exact operation-produced shape; missing reservation or malformed staging is corruption, never recursively removable. Cleanup failure blocks open; then install immutable catalog.

Every enumerated directory is capped before allocation/sort: root max **5** entries; reservations root max **2**; Agent entity max **2**; Session entity max **3**; generation payload directory max **3**; each reservation/entity/generation collection max **1,000,000**. Unknown, alias, non-UTF-8, symlink and reparse entries count first and then fail. The scanner stops at cap+1 before unbounded allocation. All count/size/structural bound failures map to `DurableStateTooLarge`; malformed identity/layout/payload failures map to `DurableStateCorrupt`; a marker absent outside eligible bootstrap maps to `UnsupportedStoreFormat`. V1 has no automatic migration. Before a new reservation, the actor checks that the relevant reservation collection has fewer than 1,000,000 entries; exhaustion performs no write and burns no candidate. `StorageGeneration` exhaustion at 1,000,000 likewise performs no writes and maps internally to too-large/user-action-required.
...
For a **new Agent, Session, or Fork child**, the exact chronological order is:

1. create the markerless entity directory, then sync its `agents/` or `sessions/` collection parent when supported;
2. create `generations/`, then sync the entity directory;
3. create generation 1, then sync `generations/`;
4. create/write/sync `head.json` and `definition.json`; for Session/Fork also create/write/sync `conversation.jsonl`; sync their direct directories and exact-readback the candidate generation payload. Conversation Storage performs a bounded-memory streaming reread/semantic validation and returns an opaque `PreparedConversationProof` bound to child SessionId, same file identity/length, Header and selected path;
5. perform final cross-entity checks while the required private gate remains held;
6. cross `DurableCommitBarrier` immediately before `create_new(COMMITTED)`;
7. `create_new`/sync `COMMITTED`, sync generation 1, then exact-readback `COMMITTED`, `head.json` and `definition.json`;
8. `create_new`/sync `PUBLISHED`, sync the entity directory and entity-collection parent, then exact-readback the complete entity. For Session/Fork this binds the same conversation identity/length/Header to `PreparedConversationProof`; it does not allocate or reread the complete 1 GiB a second time;
9. install the immutable catalog head and fulfill completion.
