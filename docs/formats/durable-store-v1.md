...
状态：当前权威 physical format specification；M5.0 production recovery/root lease、private permanent reservation foundation、crate-private Agent Create与ordinary Session Create exact G1 COMMITTED/PUBLISHED publication，以及unloaded RecordedHistory + Genesis Session Fork tracer已实现；remaining Fork anchors/LiveSnapshot、update/CAS、public Runtime command及完整cross-platform native matrix pending
日期：2026-08-03
