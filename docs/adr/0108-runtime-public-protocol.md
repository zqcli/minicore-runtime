...
2. 公开领域identity为`AgentId → SessionId → TurnId → ItemId → RequestId`。不定义公开`RunId`、`WorkspaceId`或`SubmissionId`；`CommandId`负责协议命令correlation与in-flight去重，Submit的CommandId同时作为Turn创建前的process-local admission/cancel target。Turn创建后长期execution identity切换为TurnId。两者都不是额外Run entity；`execution_version`、provider attempt id、Tool executor route、EmergencyControl target generation等内部坐标不公开。
