...
> 2026-07-30：dispatch/query/snapshot/subscribe、typed outcomes和snapshot-first保留；TurnStarted、item_completed和Interaction events确认live mutation，不确认physical Session recording。
...
2. 公开领域identity为`AgentId → SessionId → TurnId → ItemId → RequestId`。不定义公开`RunId`、`WorkspaceId`或`SubmissionId`；`CommandId`负责协议命令correlation与in-flight去重，Submit的CommandId同时作为Turn创建前的process-local admission/cancel target。Turn创建后长期execution identity切换为TurnId。两者都不是额外Run entity；`execution_version`、provider transport internals、Tool executor route和EmergencyControl内部状态不公开。
3. Command在每个命令语义明确的线性化点返回typed `CommandOutcome`（如`TurnStarted`、`SteerQueued`、`FollowUpQueued`、`SessionForked`、`InteractionResolved`），不使用只有`accepted: bool`的通用acknowledgement。`Submit`在initiating UserMessage append/apply后返回`TurnStarted`；Steer/FollowUp在各自SessionIngress FIFO admission后返回Queued，但不承诺crash-safe delivery，restart后未append消息消失；后续append/start由Event表达。CommandId必须随机且不得复用；当前Runtime内相同CommandId和exact typed command的in-flight重试加入同一completion；同一in-flight CommandId携带不同command时返回`CommandConflict`。CommandId不持久化，不承诺restart后的命令重放；乐观并发用expected revision/status/TurnId表达。
...
5. Runtime scope与每个Session scope使用独立owner、Snapshot与event stream：`RuntimeSnapshot`只覆盖Runtime/Agent/Session summary与loaded membership，`SessionSnapshot`覆盖单个loaded Session的current Turn、按selected path entry顺序与assistant Reasoning/Text/ToolCall content顺序排列的active Items、pending Interaction与queues。不建立runtime-global event sequence，也不构造all-loaded stop-the-world barrier；SessionSnapshot从该Session immutable published view读取，不经过mutation/control queue，RuntimeSnapshot不等待所有SessionExecutor。
6. subscribe使用snapshot-first实时流：owner原子注册subscriber并捕获第一帧Snapshot，随后按发送顺序交付实时StateEvent；ProgressEvent可合并或丢弃。StateEvent本身不是durable log，可以描述committed-derived或process-local状态，两者的交付保证都只限当前subscription lifetime。AgentRun的AgentMessage/Reasoning started和delta属于ProgressEvent，ItemId在首个streamed content update分配后保持稳定；没有progress时在terminal normalization分配。CompactionSummary不创建ItemId。只有final candidate append/apply后才发布同ItemId的`item_completed` StateEvent。subscriber背压或disconnect时stream结束，调用方重新subscribe并从新Snapshot恢复；Runtime process restart先从JSONL replay并conservative terminalize unfinished Turn，再发布反映恢复结果的新Snapshot；durable-derived状态从projection重建，未append queue、旧load/readiness和旧phase不恢复。首版不公开cursor、Gap、event replay或跨restart续订。
7. SessionStorage拥有durable message/entry tree。Runtime通过`SessionQuery::GetHistoryTree`/`ListTurns`/`ListItems`暴露read model；MVP只要求持续增长的Session/Turn历史和大型catalog分页，GetTurn、turn-scoped ListItems与active Items完整返回。Turn/Item顺序由ordered Vec和new-Item StateEvent创建顺序表达，不公开DisplaySequence。通过 `SessionCommand::Fork`（`ForkAnchor` 使用 Genesis/UserMessage/FinalAgentMessage 语义，不接受裸 EntryId）创建新 Session branch。同一 Session 不提供原地 checkout/navigation mutation。
...
10. 公开面是in-process Rust interface加transport-neutral serde types；wire adapter只做serialization、correlation、连接生命周期、initialize/version协商与EventStream映射，不承担Session state、Snapshot publication、authorization、retry或storage truth。SessionExecutor、SessionExecutionHandle、SessionWriter、ModelGateway、PromptSet、Tool/Skill executor、credential、WorkspaceSnapshot和EmergencyControl等内部对象永远留在crate内部；外部宿主不能取得内部handle，只依赖facade。
...
- message tree只经Fork分支和只读read model暴露：`GetHistoryTree`返回完整compact branch topology，长期Turn timeline通过`ListTurns`分页；UI无法直接改写JSONL或leaf pointer。同一Session原地navigation/checkout留待真实产品需求出现后再设计。
