...
`AgentRuntime` 是下游 CLI、TUI 和 GUI 共用的稳定门面，负责命令、查询、事件、快照、工作区、会话目录和 `WorkspaceServices`。`WorkspaceServices` 中的共享 `ResourceManager` 管理级联资源快照；每个 `SessionRuntime` 固定 workspace cwd，并在 user turn 启动时捕获 `TurnResourceSnapshot`。`SessionRuntime` 是单个会话的产品级编排层，负责阶段控制、turn state、pending stable batch drafts、steering/follow-up/next-turn 队列、prompt/skill/prompt-template 调用、模型与思考等级切换、工具与活跃工具切换、资源更新、stream options、abort、`session_settled` 状态归约和事件映射；后期才在其拥有的安全点接入内部运行时 Hook。Rig 负责核心 agent loop，`SessionRuntime` 和 `Driver` 负责把它产品化为可被下游宿主稳定调用和观察的后端能力。
