...
4. **Same-origin is scheme + normalized host + effective port。** Planner同步解析call URL并与已安装canonical origin比较；host大小写、IDNA normalization与explicit/default `:443`按URL parser的canonical/effective语义比较，path与query不参与origin identity。foreign host、subdomain、scheme或port均为Denied。Malformed/oversize URL、userinfo或fragment是invalid arguments。userinfo检查在parse前后都执行：pre-parse semantic validation先要求raw `scheme://non-empty-authority` hierarchical form（scheme colon之后必须恰从两个forward slashes开始且authority非空），拒绝special-scheme parser recovery spellings（one slash、backslash、no slash、3+ slash导致empty authority），避免枚举追赶parser的每一种recovery spelling；随后在raw hierarchical authority上做literal `@` probe，因为WHATWG parser会把完全空的userinfo（`https://@host/`、`https://:@host/`）归一化为无userinfo而绕过post-parse的username/password检查；percent-encoded `%40`与path/query中的literal `@`不是userinfo。授权返回opaque、redacted、move-only request target，绑定exact parsed URL与exact origin client；executor不重新选择origin、不重新parse string，也不能访问origin集合。target本身拥有唯一consuming deep operation `send`：绑定exact URL/client构造并发送恰一个GET（固定ADR 0147 headers，body为空），executor只await response，不能改scheme/host/port/path/query/header，也从不接触raw `reqwest::Url`或`reqwest::Client`。
