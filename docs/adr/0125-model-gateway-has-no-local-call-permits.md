...
2. 共享`Arc<ModelGateway>`必须支持多个Session并发调用；实现不得用Gateway-wide mutex或其他长guard包围credential resolution、provider request或stream读取。每个Session最多一个current `RunningOperation`仍由SessionExecutor保证，但这不形成跨Session模型调用限制。
