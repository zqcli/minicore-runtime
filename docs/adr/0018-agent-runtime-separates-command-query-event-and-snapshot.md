...
`RuntimeQuery` 按领域分组为 runtime、session、settings、resources、command surface、models、usage 和 diagnostics。新增子查询时扩展对应领域 enum，不把所有查询平铺成一个巨型列表。query 必须只读、有大小边界、不消费 queue、不创建 turn、不发布业务事件，也不能为了读取目录或持久化详情而加载完整 `SessionRuntime`。

## 影响

`ListSessions`、资源详情、command catalog/suggestion、usage/context usage 和 settings/config read 从 `AgentCommand` 移到 `RuntimeQuery`。`CommandAck` 继续只表达命令是否被接收；query 结果不通过 event stream 广播，也不增加 event sequence。`RuntimeSnapshot` 保留为带 `last_event_sequence` 的粗粒度恢复读模型，不扩展成通用分页/详情容器。
