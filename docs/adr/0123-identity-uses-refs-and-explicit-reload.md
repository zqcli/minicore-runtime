...
6. Compaction operation持有同一个`Arc<CompactionPlan>`及其immutable settings、budget、scope和source，同时持有由该plan组装出的同一个`Arc<ModelCallRequest>`；exact rendered directive随request固定。operation result只能交回持有这两个Arc的current operation slot；append前验证exact source checkpoint、scope、boundaries、provenance、current Turn/version/control和actual typed entries。
7. `ConversationBoundary`只保存first/last `EntryId`。Fork remap全部target-local `EntryId`、`TurnId`、`ItemId`、`RequestId`及其nested references，preserve provider/transcript correlation `ToolCallId`和historical provenance，再对target执行完整structural replay validation；不要求source/target拥有跨ID相等的stable-unit hash。
...
10. `ToolExecutionStarted`通过assistant ToolCall/Item reference、resolved ToolName、exact frozen arguments、ToolRequirements和effective permissions记录truthful intent；不保存invocation/requirements/authorization hash。
...
16. `StoredTurnContext`删除Prompt/Tool/Skill/Workspace/Model/Execution fingerprint及SessionDefinition content hash，只保存exact durable refs、必要的safe execution values/provenance和diagnostics。MVP明确不从该entry重建旧execution environment。
17. `StoredCompaction`删除source transcript hash、stable-unit hash、plan/budget/assembled-context hash和summary content hash；保留exact source checkpoint、typed scope/boundaries/provenance、summary text、exact model ref、requested output、usage、finish reason、retry count和provider metadata。
