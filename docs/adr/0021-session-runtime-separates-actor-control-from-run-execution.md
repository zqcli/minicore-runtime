...
`Tools` 继续是 `SessionRuntime` actor 内部的 session-scoped 子系统。Actor 保留 active tools、approval mode、grants 和 decision/admin 能力；`RunTask` 只得到 `ToolBatchInvoker`，不能 set active tools、改变 approval mode 或直接提交 decision。`Tools::tool_batch_invoker()` 从当前 committed active-tool/policy/sandbox baseline 构造 immutable `ToolExecutionProfile`，并把共享 executor registry 与短锁 approval broker handle 封装进 invoker。MVP 在 `Turn` 中拒绝 model/thinking/stream/active-tool mutation；后续若支持 safe-point apply，必须同时替换 `PromptCallProfile` 与 `ToolBatchInvoker`，不能只 patch 一侧。
