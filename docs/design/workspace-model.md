...
状态：Accepted（已固化为 [ADR 0021](../adr/0021-workspace-is-single-instance-thin-boundary.md) 并同步各权威文档，BR-036 已关闭；本文作为设计推导背景材料保留）
...
- `OpenWorkspace { path }` 只在 `NoWorkspace` 状态创建 workspace。
- `Open` 状态下再次调用：canonical path 与当前 root 相同 → **幂等成功**（返回 accepted，不 teardown、不重载资源、不影响 loaded sessions）；不同 → **拒绝**，`CommandAck.accepted = false`，reason `WorkspaceAlreadyOpen`。
...
- 派生规则：`canonicalize(path)` → 平台规范化（Windows 剥离 `\\?\` verbatim 前缀、按平台语义做大小写规范化、去尾分隔符）→ 稳定 hash（如 SHA-256 截断）→ opaque id 编码。规范化后的路径称为 canonical root path，同时是 BR-064 中 `ResourceKey` / source info 路径展示规则的统一落点。
- session metadata 持久化 `{ workspace_id, workspace_root: PathBuf, cwd: PathBuf }`：`workspace_root` 为 canonical root path 冗余存储（诊断、迁移、id 算法演进时重算）；wire 协议上继续只traffic `WorkspaceId`。
...
- `OpenSession` 目标 session 的 `workspace_id` ≠ 当前 workspace → 拒绝，`SessionOutsideWorkspace`。
...
| 位置 | 处置 | 语义 |
| --- | --- | --- |
| `OpenWorkspace { path }` | 保留 | D1；path canonicalize 失败 / 不是目录 → 拒绝 |
| `Event.workspace_id` | 保留 | workspace/session/run 级事件必填，恒等于当前 workspace id（单实例下无歧义；坐标表已有的层级规则不变） |
...
| `ApprovalGrantScope::SameToolInWorkspace` | 保留 | 单实例下语义 = 本 runtime 生命周期内全部 session；grant key 已含 cwd，不需要额外 workspace 字段 |
| Workspace Lifecycle 状态机（events 文档） | 保留单实例 | `Open` 状态补两条自环/拒绝分支（D1）；`workspace_opened` 维持不作为公开事件（现状） |
...
- `WorkspaceId` 稳定性：同一 root 两次进程启动得到同一 id；大小写变体、尾分隔符、Windows `\\?\` 前缀、相对/绝对路径变体全部归一到同一 id。
...
7. `docs/review/system-blueprint-review-issues-round2.md`：BR-036 标记 Resolved，处理记录指向 ADR 0021。
