...
PromptSet仍是唯一producer。为保留role fidelity，目标shape使用ordered instructions，而不是提前压成单个system string：
...
如果Rig generic和provider-specific API都无法保留MiniCore要求的semantic content、tool identity或usage，spike必须阻止该provider adapter并提出targeted follow-up ADR；不能把Rig raw types泄漏给caller。直接重写provider HTTP client不属于本阶段accepted baseline。
