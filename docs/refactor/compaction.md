...
- SessionStorage仍是唯一durable truth；
- 首版只在active Turn的`NeedModel`安全点执行automatic compaction；
- trigger包括soft context pressure、Prompt local context overflow和provider context overflow；
- 首版不在final response后执行post-turn compaction，也不提供standalone/manual compaction；
...
由于retained range必须是连续suffix，首版不会摘要active Turn内部的早期ToolRound。
...
首版不会split current Turn、摘要或截断current UserMessage、删除ToolResult，或切换更大模型。hard overflow映射为TurnFailed；soft trigger仅在原调用仍通过Prompt validation时继续未压缩调用。
...
PromptSet仍是唯一模型上下文组装seam。目标input：
...
SummaryModel返回的reasoning不进入conversation。首版StoredCompaction不单独保存summary-call reasoning；实现不得保存或伪造provider未返回的hidden chain-of-thought。
...
`RequestOutcomeUnknown`或`StreamInterrupted`不能盲目重放。首版logical summary retry应有很小上限，不能通过改变summary directive伪装成同一次retry。
...
首版automatic active-Turn compaction约束：
...
`turn_id = None`和`model_call = None`保留给未来standalone/deterministic maintenance，不由首版流程产生。
...
MVP不把Compaction entry暴露成独立public fork anchor，但selected message path上的Compaction自然随path生效。
...
算法正确不代表summary质量足够。实现阶段建立可重复fixture，人工或grader检查：
...
优点是可以压缩超长active Turn；缺点是需要turn-prefix summary、current Turn continuation标记、额外cut/recovery/fork规则。首版选择明确报告`ProtectedSuffixTooLarge`。
...
可能更忠实保留provider-specific state，但artifact不可移植，且要求provider/model/auth continuity。首版以portable summary作为durable representation。

### Post-Run Eager Compaction

可以降低下一次Turn latency，但会为不再继续的Session支付成本，并使用旧Turn模型策略。首版只在真正NeedModel时执行。
...
任意丢弃旧message会破坏任务连续性。首版只允许有标记的summary-input ToolResult reduction，不把它当conversation compaction result。
