...
本文定义 MiniCore 阶段 9 的公开 Runtime interface，回答：
...
- 首版不提供同一 Session 内原地 checkout/navigation mutation；
...
- 首版实现 in-process Rust interface 和 transport-neutral serde types；具体 transport 使用薄 adapter。
...
| LangGraph Agent Server | Thread/Run资源、durable task queue、checkpoint、SSE join/cancel | 独立daemon出现后再评估Run resource和durable replay；首版不支付分布式队列成本 |
...
首版不提供同一Session原地history checkout。创建历史分支使用`Fork`，得到新的SessionId和独立definition revision序列。
...
阶段9保留旧设计中的无状态`CommandManager`、递归command tree、dynamic candidate、执行前重新resolve和UI-safe catalog。

阶段9删除旧`SessionRuntime.command: Command`长期facade。MiniCoreRuntime每次基于显式optional SessionId构造`CommandContext`；CommandManager不持有SessionExecutor handle或current Session。
...
首版CommandSurface不注册`/compact`。automatic compaction由SessionExecutor在NeedModel安全点内部触发。
...
首版只提供UI-safe catalog和diagnostics：
...
Public interface是阶段9 contract test surface。
...
### 首版公开 Manual CompactSession

优点：用户可主动降低上下文占用。

缺点：需要standalone Session maintenance state、admission/queue/cancel和模型选择规则；当前Compaction只在active Turn NeedModel安全点定义完整语义。

结论：首版不采用。未来出现真实需求后独立设计。
...
- [x] 确定首版不公开manual CompactSession。
- [x] 列出禁止公开的内部对象。
