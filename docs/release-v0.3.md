...
**IC-11 evidence decision:** CI uses structural counters and source/behavior assertions rather than absolute-millisecond thresholds. Existing evidence includes 100-turn create/load descriptor freezing with exactly 100 model starts, trusted acknowledgement over 10,000 entries, one conversation projection per normal plan plus the bounded changed-head compaction replan, one ContextDriver checked canonical sort seam, and concurrent SessionRuntimes with cancellation isolation. The unified B1–B9 wall-clock harness and benchmark artifact remain deferred; this release does not claim benchmark completion. IC-05 remains the approved benchmark-gated ValidationPlan exception, and IC-12 remains benchmark-gated because projection storage would change the durable append/copy ownership boundary.
