...
- Tool start继续通过`ToolStartGate`与EmergencyControl first-wins（start first-wins与Running best-effort cancellation均已落地：signal先赢→不调用start factory、PreExecution Cancelled；start先赢→slot经Settling继续await same run并truthful settle；首个production `read_file` adapter已提供owner-tracked、工作量/分配有界且不detach的cleanup（ADR 0143），但ordinary regular-file wall-clock settlement仍由OS/filesystem决定；其余production executor/adapter teardown仍后置）；Cancel/SecurityRevoked/Unload signal-first对全部unstarted calls（含pending UserQuestion binding）跳过并settle matching PreExecution Cancelled；
...
| INV-402 | production `read_file`的read授权在plan时经`WorkspaceAccessView::authorize_read`对已通过semantic parse的exact cwd-relative path同步签发；root path/无read grant/capability basis unavailable均collapse为frozen Denied且不构造start factory，absolute/dot/dotdot/platform-prefix等path在更早的`WorkspaceRelativePath` parse阶段归为invalid arguments；read_file Runtime的host invalidation先经Runtime-owned `WorkspaceReadAccessControl`永久revoke该Session read grant（idempotent、本Runtime lifetime内无unrevoke）再signal/re-resolve，recovery re-resolve只授予filesystem `None`、绝不恢复`ReadOnly` | [Workspace · WorkspaceAccessView](modules/workspace.md#workspaceaccessview) |
