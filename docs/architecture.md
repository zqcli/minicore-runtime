...
| V2 | 当前权威架构。ADR 0126–0137冻结async execution、conversation/wire、DurableState/Store V1/root lease与Tokio owner-tracked foundations。M5.0 durable foundation与exact historical definition resolution已实现；M5.1 target/proof、owner-tracked SessionRecorder physical append及全部七个 `slice = m5_1` Recorder fixture坐标已完成；M6.1 Workspace resolver/Snapshot、loaded Ready+Idle publication owner、Runtime-owned residency actor（single-flight Load、draining Unload、lifecycle exclusion、unified loaded/unloaded Workspace update）、Runtime-owned PromptService/initial PromptResourceView、Workspace Prompt candidate capture及replay/Recorder-backed Ready+Idle Load hydration已实现，并新增 crate-private Prompt candidate/profile/Text composition 与owner-bound empty view foundation；M6.2 scripted text-only Prompt assembly/ModelGateway foundation已实现，Runtime拥有empty gateway/initial catalog，exact request通过single adapter attempt产生ordered progress或validated terminal result，reload与cancel竞态保持Turn-pinned语义；Load fault-and-replay conformance已以crate-private deterministic seams覆盖admitted cancellation、replay worker faults、Recorder degradation、stale Workspace recheck与cold replay；public Runtime command/snapshot/event、complete shared-root publication、具体Prompt/Skill source adapter、Structured/Tool/Compaction assembly、production Rig adapter、完整Turn capture/ActiveTurnTask、remaining Fork anchors/LiveSnapshot、active-Turn grace Unload及完整cross-platform native matrix仍待实现；M5.2 tolerant semantic replay seam与corruption sidecars已实现；后续见[开发计划](development-plan.md)。 |
