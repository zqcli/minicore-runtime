...
| V2 | 当前权威架构。ADR 0126–0137冻结async execution、conversation/wire、DurableState/Store V1/root lease与Tokio owner-tracked foundations。M5.0 durable foundation与exact historical definition resolution已实现；M5.1 target/proof、owner-tracked SessionRecorder physical append及全部七个 `slice = m5_1` Recorder fixture坐标已完成；M5.2 tolerant semantic replay seam与corruption sidecars已实现；M6.1 Workspace resolver/Snapshot、loaded Ready+Idle publication owner、Runtime-owned residency actor（single-flight Load、draining Unload、lifecycle exclusion、unified loaded/unloaded Workspace update）、Runtime-owned PromptService/initial PromptResourceView、Workspace Prompt candidate capture及replay/Recorder-backed Ready+Idle Load hydration已实现，并新增 crate-private Prompt candidate/profile/Text composition 与owner-bound empty view foundation；M6.2 scripted text-only Prompt assembly/ModelGateway foundation已实现，Runtime拥有empty gateway/initial catalog，exact request通过single adapter attempt产生ordered progress或validated terminal result；M7 ordinary AgentRun已接通public Create/Load/Submit/Snapshot/Subscribe/Unload、immutable Turn context、Input/final Assistant live apply与inline record、single scripted Model attempt、terminal Event及cold replay，并覆盖context overflow、Recorder degradation和busy/Unload drain；M8.1最小Scripted Tool round-trip已接通ToolCall → ToolSet执行 → ordered ToolResult → 下一次Model → final Assistant；complete shared-root publication、具体Prompt/Skill source adapter、Tool policy/approval、Interaction/Cancel、Structured/Compaction assembly、production Rig adapter、remaining Fork anchors/LiveSnapshot、grace/cancel式active-Turn Unload及完整cross-platform native matrix仍待实现；后续见[开发计划](development-plan.md)。 |
