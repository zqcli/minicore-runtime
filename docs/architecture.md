...
- Tool start继续通过`ToolStartGate`与EmergencyControl first-wins（start first-wins与Running best-effort cancellation均已落地：signal先赢→不调用start factory、PreExecution Cancelled；start先赢→slot经Settling继续await same run并truthful settle；production `read_file`/`list_directory` adapters已提供owner-tracked、工作量/retained allocation有界且不detach的cleanup（ADR 0143/0144），但ordinary filesystem wall-clock settlement仍由OS/filesystem决定；其余production executor/adapter teardown仍后置）；Cancel/SecurityRevoked/Unload signal-first对全部unstarted calls（含pending UserQuestion binding）跳过并settle matching PreExecution Cancelled；
...
| INV-402 | production Workspace reads在plan时只经`WorkspaceAccessView::{authorize_read, authorize_read_directory}`对已semantic-parse的exact cwd-relative path同步签发opaque capability carrier，executor不得接收raw model path或重取ambient authority；file root target拒绝，directory empty target表示captured cwd，无read grant/capability basis unavailable均collapse为frozen Denied且不构造start factory；任一read opt-in的host invalidation先经同一个Runtime-owned `WorkspaceReadAccessControl`永久revoke该Session read grant再signal/re-resolve，recovery只授予filesystem `None`，future file/directory reads均Denied | [Workspace · WorkspaceAccessView](modules/workspace.md#workspaceaccessview) |
