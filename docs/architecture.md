...
- Tool start继续通过`ToolStartGate`与EmergencyControl first-wins（start gate first-wins已实现，Running Tool best-effort cancellation teardown仍pending）；
...
| INV-401 | Tool side-effect start由ToolStartGate与EmergencyControl first-wins；Running后只能truthful settle（start first-wins已落地，Running cancellation与完整slot仍pending） | [Turn / Item / Interaction · Tool Side-Effect Start](modules/turn-item-interaction.md#tool-side-effect-start) |
