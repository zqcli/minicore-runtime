...
| 12. Compaction | 实现手动压缩、summary message、context rebuild；再做 threshold 和 overflow recovery | `src/compaction.rs`、`src/session_runtime.rs`、`src/session_storage.rs` | `Compaction` 只生成 `CompactionSummaryMaterial`，`SessionRuntime` 构造唯一 `ModelCallRequest { purpose: CompactionSummary, tools: [] }`；`/compact` 使用 `DeferUntilPostRun`，idle 时立即执行、running 时进入唯一 pending action 且不 abort；pending action 可从 `QueueSnapshot` 恢复；`SessionEntry::Compaction` 重建上下文；overflow recovery 不污染重试上下文。 |
...
- `/compact` 在 `Compaction` 后端完成前只进入 catalog，状态为 disabled；启用后使用 `availability = Available` + `phase_policy = DeferUntilPostRun`。
