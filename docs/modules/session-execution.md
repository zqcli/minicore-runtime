...
- Prepared Tool不启动；
- Running Toolbest-effort cancel并等待truthful outcome或Abandoned（已实现：Running slot持有operation自己的`ToolCancellationHandle`，signal先赢后cancel该cancellation pair并经Settling继续await same run至executor cooperative cleanup/result，started run不因signal drop；production `read_file`保持await同一个owner-tracked blocking job且不detach，工作量/分配有界但ordinary regular-file wall-clock无上界承诺；generic ToolService与其余executor/adapter teardown仍后置，后续adapter必须证明有界工作量并等待可确认settlement）；
