...
状态：当前权威架构（ADR 0137后；crate-private loaded Ready+Idle `SessionExecutor`、Runtime-owned residency actor、single-flight Load、draining Unload、lifecycle exclusion、unified loaded/unloaded Workspace update，以及replay/Recorder-backed Ready+Idle Load hydration已实现；Load fault-and-replay conformance已覆盖admitted cancellation、replay worker faults、Recorder degradation、stale Workspace recheck与cold replay；public Snapshot/Event、active-Turn grace Unload、Turn admission与`ActiveTurnTask` pending）
日期：2026-07-31
...
当前crate-private foundation已经消费该合同：actor在spawn前安装publication slot/permit；changed Workspace先完成resolver candidate与empty-source Snapshot（production restricted authority不授权source），再调用DurableState；NoChange不调用resolver但仍经过authoritative durable stale/lifecycle recheck；actor在reap owner-tracked child后才安装new Snapshot、settle waiter并释放permit。request channel close会等待pre-close reserved permits和active publication；caller drop、close及post-commit-before-install barrier均由named deterministic tests覆盖；Load fault-and-replay conformance也以named barriers/faults证明admitted Load cancellation、replay worker spawn/panic/join failure、Recorder degraded initialization、stale candidate拒绝和append后的cold replay。actual Prompt/Skill source capture启用前，任何非空authorized source roots均作为internal invariant fail closed，不能静默发布source-empty Snapshot。
