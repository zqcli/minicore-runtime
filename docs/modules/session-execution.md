...
状态：当前权威架构（ADR 0137/0146后；loaded `SessionExecutor`、Runtime-owned residency、ordinary Turn、Tool/Interaction/Cancel、M9 control/observation与M10 Compaction均已接通。crate-private `ToolOperationSlot`完整生命周期现包含真实file mutation ownership：每loaded Session创建一个`SessionFileMutationQueue`；typed `ToolExecutionPlan::FileMutation`由round owner在任何start前按`call_index`串行prepare并reserve exact-request-bound ticket；ticket取得permit后才进入ToolStartGate；started mutation的permit由Running移动到Settling，只有exact outcome绑定并转Terminal后释放。UserQuestion仍先于mutation preparation完整settle。M14 production `ask_user`/`read_file`/`list_directory`/`write_file`均已实现；`write_file`使用两个owner-tracked blocking jobs和move-only prepared capability target，scheduled work不因signal detach。具体Prompt/Skill source、完整generic schema/hooks/policy/approval、generic ToolService、network/process及其他production adapters仍后置。）
日期：2026-07-31
...
- Prepared Tool不启动；
- Running Toolbest-effort cancel并等待truthful outcome或Abandoned（已实现：Running slot持有operation自己的`ToolCancellationHandle`，signal先赢后经Settling继续await same run；production `read_file`/`list_directory`/`write_file`保持await owner-tracked blocking job且不detach；started `write_file`还保持mutation permit至Terminal。generic ToolService与其余adapter teardown仍后置）；
