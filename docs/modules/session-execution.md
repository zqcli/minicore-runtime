...
Observer闭环补充：SessionExecutor保留既有32-slot state-only broadcast，并为`include_progress=true`订阅保留256-slot actor-owned State+Progress有序broadcast。provider callback仍不能直接发布public frame，也不再向unbounded `ExecutorCompletion`发送每个delta：每个active Turn拥有一个lossy mailbox，最多跟踪64个`content_index + public kind` key、保留8个pending event与65,536 raw UTF-8 text bytes；单个coalesced model delta segment至多32,000 bytes，只有mailbox从空转为非空时才`try_send`一次capacity-1 wake。普通delta在8-event容量内保持原segmentation；压力下只把同item的后续文本UTF-8-safe追加到最后segment，满后丢newest suffix，不驱逐已接受内容。没有progress-demand lease时仍维护ItemId、attempt-local delta与reasoning fallback事实，但不构造observer payload、不wake actor。actor normal priority固定为closing → prepare deadline → structurally-bounded owner completions → emergency → capacity-1 reliable progress → ordinary requests → lossy model wake；completion-first保留已queued terminal对actor emergency request的排序，exact Turn first-wins本身由EmergencyControl owner mutex内的private terminal/control commit gate冻结：worker在final live mutation前claim terminal，Cancel/Security/Unload/close在接受或signal前claim control；control先赢则不apply/record final，terminal先赢则control不得取消、Finishing或重标该Turn。Failed completion也在actor settle时claim同一gate，因而control先赢可truthful Interrupted，completion先赢则later control rejection。emergency仍不会等待progress或ordinary backlog。capacity-1 wake由successive Turns共享：消费stale Turn wake时actor同时检查并drain current active Turn mailbox，恢复因`try_send(Full)`只留下`wake_pending`的credit，避免新Turn永久静默。Steer safe point、phase/ToolInvocation refresh、retry marker与successful terminal都先drain retained model progress；Cancel/failed/interrupted/close关闭mailbox并丢pending model draft，late wake/callback只被忽略，不fatal-close Session。

ToolOutput与retry marker不进入lossy mailbox：ActiveTurnTask通过capacity-1 reliable progress lane发送并等待actor acknowledgement。每个AgentRun provider publisher捕获private attempt generation；retry在marker publication前先递增generation并清空attempt-local started/delta/fallback facts，保留logical ItemId map，旧attempt retained callback随后在任何identity/fallback/mailbox mutation前被拒绝。ToolOutput在live apply和inline record attempt后按原part顺序切为至多32,000 raw UTF-8 bytes的char-boundary chunks，逐chunk成功publication attempt后才进入下一chunk/Model；subscriber absence或lag不阻塞ack，close drain会ack-discard已接受命令以解除task waiter。该cap按safe Text最大2x JSON escaping与maximum-ID canonical V1 envelope证明完整ProgressEvent不超过65,536-byte limit；empty part无delta、chunk间无separator、stored/model-visible result不变。provider-neutral `ReasoningSummary | ReasoningText`仍映射同一key与stable ItemId；任一选定raw delta（即使observer payload因无demand/pressure未保留）阻止terminal summary fallback。subscriber lag关闭该stream为Backpressure；Unload/owner replacement为PublisherRestarted；Runtime shutdown为RuntimeClosing。
...
当前实现进度：M10已在M9 control/observation基础上接通完整Compaction；`SessionQueueView`继续只公开lane-local command identities。Tool round现有五种owner plan shape：Execute、Approval、UserQuestion、FileMutation、PreExecution（`PreparedFileMutation`是round-private prepared shape）。每个exact request仍先构造Prepared slot；UserQuestion按call_index hoist并逐个apply+record。其后remaining operations按call_index串行执行mutation preparation：pre-check exact emergency basis，调用move-only factory并等待scheduled preparation完整settle，post-settlement再次检查basis；只有仍current/unsignaled的Ready结果才同步reserve queue ticket。ordinary unstarted failure保持PreExecution truth，panic映射该slot的Abandoned RuntimeFailure。drive时waiting ticket与signal first-wins；permit先于ToolStartGate，started mutation由Running/Settling持permit至Terminal。production `read_file`/`list_directory`/`write_file`均owner-tracked且不detach；`fetch_url`的network operation由同一slot-owned future完整拥有，cancel drop exact send/body state后settle。ordinary filesystem wall-clock仍由OS/filesystem决定。generic ToolService、process及其他production adapters与完整generic schema/hooks/policy/approval仍后置。
...
- `Running`：只在typed `ToolStartedExecution` proof存在后进入，持有operation自己的`ToolCancellationHandle`与still-owned started run future；file mutation另持有`Some(SessionFileMutationPermit)`，ordinary operation为`None`；
- `Settling`：signal先赢时先cancel自己的pair并继续await same run；mutation permit随variant从Running移动到Settling，直到exact outcome绑定后消费Settling进入Terminal才drop；
- `Terminal`：绑定exact identity-bound `ToolExecutionOutcome`，唯一extraction路径。

执行细节：`Execute`/`Approval.allowed`直接携带move-only start factory；`FileMutation`携带final permissions、move-only preparation factory与exact Session queue。round owner把它转换为`PreparedFileMutation { ticket, start }`、truthful PreExecution或frozen Abandoned后才drive。signal-before preparation不调用factory；signal在scheduled preparation期间到达时仍等待same preparation settle，随后不reserve ticket；waiting ticket signal-first则移除ticket、不start；permit-first后才reserve/start gate。`run_started_execution`复验typed proof exact capture后构造future；signal after start只cancel observer并经Settling await same run。serial/parallel、panic isolation与call_index result order保持不变。Turn/Item只投影`Started | Completed | Abandoned`，recording不是Tool start marker。
...
- Prepared Tool不启动；
- Running Toolbest-effort cancel并等待truthful outcome或Abandoned（已实现：Running slot持有operation自己的`ToolCancellationHandle`，signal先赢后经Settling继续await same run；production `read_file`/`list_directory`/`write_file`保持await owner-tracked blocking job且不detach；started `write_file`还保持mutation permit至Terminal；`fetch_url`在same owner future内drop exact send/body state并返回Cancelled。generic ToolService与process/其余adapter teardown仍后置）；
