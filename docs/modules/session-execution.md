...
Observer闭环补充：SessionExecutor保留既有state-only broadcast，并新增只供`include_progress=true`订阅的actor-owned State+Progress有序broadcast；ActiveTurnTask与provider callback不能直接发布public frame，只能把typed progress completion送回actor。模型delta、ToolInvocation snapshot refresh、ToolOutputDelta、下一次模型delta与Turn terminal因此共享同一owner lane；ToolOutput在live apply和inline record attempt后发送，每个non-empty Text part按原part顺序切为至多32,000 raw UTF-8 bytes的char-boundary chunks，逐chunk等待actor接受后才进入下一次Model。该cap按safe Text最大2x JSON escaping与maximum-ID canonical V1 envelope证明完整ProgressEvent不超过65,536-byte limit；empty part无delta、chunk间无separator、stored/model-visible result不变。provider-neutral `ReasoningSummary | ReasoningText`都映射到同一`content_index + Reasoning` key，因此OpenAI installation选定的单一reasoning channel复用stable ItemId；任一raw delta会阻止该item的terminal summary fallback，而没有选定raw delta时现有safe summary fallback仍可发布一次。final assistant在terminal前触发一次opt-in observation refresh，确保最终Item identity可校正provisional progress。subscriber lag关闭该stream为Backpressure；Unload/owner replacement为PublisherRestarted；Runtime shutdown为RuntimeClosing。
