...
状态：当前权威架构（ADR 0137后；loaded Ready+Idle `SessionExecutor`、Runtime-owned residency actor、single-flight Load、draining Unload、lifecycle exclusion、unified loaded/unloaded Workspace update、Workspace Prompt candidate capture及replay/Recorder-backed hydration已实现；M7 ordinary Turn、M8 Tool/Interaction/Cancel与crate-private `ToolOperationSlot`完整生命周期（Prepared→Running→Settling→Terminal：per-slot first-wins start gate、typed started proof、Running cancellation pair与truthful settle）、M9.1–M9.21当前control/observation范围及M10完整Compaction vertical slice已接通，包括queue/Steer/FollowUp、logical retry、EmergencyControl/SecurityRevoked、public command/outcome、Starting/Running/Finishing Snapshot、`session_execution_changed`与terminal StateEvent；current Turn/active Item/Pending Interaction安全摘要、usage read model与Degraded recording/diagnostics projection已接通；crate-private scripted approval/UserQuestion控制seam亦已完成（typed `ToolExecutionPlan::{Approval, UserQuestion}`拆分、Session-private concrete `ToolExecutionControl`、Tools-owned move-only `UserQuestionAnswerBinding`、hoisted exclusive question调度与signal-first settlement）；production `ask_user` builtin（ToolName/schema与answer→model-visible ToolResult text/render格式已由ADR 0142冻结并实现，closed/default-off/`with_ask_user_tool()` opt-in、零permission、仅UserQuestion/frozen PreExecution plans、deterministic compact JSON answer）以及OS-backed production `read_file`/`list_directory` builtins（ADR 0143/0144：closed/default-off、per-admission Workspace-bound、共享ReadOnly authority/revocation、capability-relative file read/direct directory enumeration、owner-tracked no-detach settlement）均已实现；具体Prompt/Skill source adapter、完整schema/hooks/policy/approval enforcement、generic ToolService、其余production executor/adapters（返回前须提供有界工作量并等待可确认settlement；wall-clock bound须由各adapter另证）与Session-local mutation queue/mutation permit attachment to Settling仍后置）
日期：2026-07-31
...
当前实现进度：M10已在M9 control/observation基础上接通proactive/Prompt/provider overflow、CompactionSummary至多一次logical retry、exact control/plan/request arbitration、live Replace、same-Arc inline record、post-compaction Steer safe point、`Compacting` phase及Snapshot usage/recording refresh。M9.18 的 `SessionQueueView` 仍保留 lane-local FIFO、exact `CommandId`/expected `TurnId` 和 `acceptingInput`，不携带 queued intent 正文。current Turn、active Items 与 Pending Interaction 已以最小安全摘要接入 Session Snapshot/Wire V1。crate-private `ToolOperationSlot`完整生命周期已实现：assistant ToolCall live apply后、record await前在同一no-await步骤为每个exact `ToolExecutionRequest`（ItemId + same `Arc<ToolCall>`）构造`Prepared` slot（绑定exact request + round EmergencyControl handle/observation + per-slot `ToolStartGate`），reservation在EmergencyControl owner mutex内对exact unsignaled target/epoch执行lock-free CAS（与`signal`同一mutex线性化，first-wins），move-only `ToolStartPermit`→typed `ToolStartedExecution` proof后`ToolSet::run_started_execution`复验exact capture并调用move-only `ToolExecutionStart` factory构造future；signal/stale先赢→不调用factory、产出matching PreExecution Cancelled ToolResult apply+inline record后Turn Interrupted且不发起下一次Model；start先赢→signal只触发operation cancellation observer，slot进入Settling并继续await same run至executor cooperative cleanup/result后truthful settle（started run不因signal drop），exact Executed/Abandoned；serial/parallel按call_index order；parent-owned join_all与per-boundary panic isolation使panic恰好映射Abandoned。crate-private scripted approval/UserQuestion控制正确性seam亦已完成：typed `ToolExecutionPlan::{Approval, UserQuestion}`结构拆分（旧generic Interaction plan删除）、Session-private concrete `ToolExecutionControl`复用既有Interaction actor/wire/storage owner（无public trait冻结）、Tools-owned move-only/redacted `UserQuestionAnswerBinding`（仅truthful PreExecution+Succeeded为answer、malformed/panic fail closed）、UserQuestion按typed plan shape hoisted到全部ordinary sibling之前（call_index串行、至多一个pending、不reserve/start ToolStartGate且不涉及mutation ticket、每个question outcome先apply+inline record再继续）、signal-first（Cancel/SecurityRevoked/Unload）跳过binding并settle全部unstarted calls为matching PreExecution Cancelled、abandoned question对remaining无副作用（known preflight保留、其余unstarted为PreExecution Failed）。production `read_file`与`list_directory` executors已实现工作量/retained allocation有界、owner-tracked且不detach的settlement（ordinary filesystem open/read/enumeration wall-clock仍由OS/filesystem决定）；generic ToolService、其余production executor/adapters、Session-local mutation queue/mutation permit attachment to Settling与完整schema/hooks/policy/approval enforcement仍后置。
...
→ 在同一no-await步骤为每个exact request构造`ToolOperationSlot::Prepared`（绑定exact request + round EmergencyControl handle/observation + per-slot `ToolStartGate`）
...
上述enum即当前实现（implementation-only字段——`emergency_control`/`observation`/`gate`/`run`——省略，变体形态与生命周期语义不变）。Session Execution是`ToolOperationSlot`的唯一type owner；每个exact request恰好一个slot。

- `Prepared`：assistant ToolCall live apply后、record await前构造，绑定exact request、round的`EmergencyControlHandle` + observation与slot自己的`ToolStartGate`（sole operation owner，不存在第二层round-level gate）；
- `Running`：只在typed `ToolStartedExecution` proof存在后进入，持有operation自己的`ToolCancellationHandle`与still-owned started run future（observer已move进该run）；
- `Settling`：当前等待点——signal先赢时先cancel自己的cancellation pair、在executor result前进入Settling并继续await same run至executor cooperative cleanup/result（started future绝不因signal被drop），自然完成则先绑定result再短暂经过Settling；
- `Terminal`：绑定exact identity-bound `ToolExecutionOutcome`，唯一extraction路径。

执行细节：typed `ToolExecutionPlan`拆分为`Execute{permissions, start: ToolExecutionStart}`/`Approval{permissions, request, allowed: ToolExecutionStart, denied}`/`UserQuestion{request, answer: UserQuestionAnswerBinding}`/`PreExecution(result)`（旧generic Interaction plan删除）；`Execute`与`Approval.allowed`携带move-only `ToolExecutionStart` factory，前两者各自携带唯一final class-level permission事实。`ToolSet::run_started_execution`先复验typed proof的exact capture（foreign proof fail-closed为identity-bound Abandoned RuntimeFailure）再调用factory构造future。signal-before-start不调用factory（matching PreExecution Cancelled）；start先赢后signal只触发operation cancellation observer，slot进入Settling并继续await same run，executor返回truthful Executed/Abandoned。serial后续未启动calls各自PreExecution Cancelled；parallel Running calls各自收到cancel，parent-owned join_all、call_index order；per-slot catch_unwind使panic恰好映射exact Abandoned。future mutation permit step（Session-local mutation queue/mutation permit attachment to Settling）仍后置，不属于当前slot。Turn/Item只投影`Started | Completed | Abandoned`。pre-execution exact outcome允许`Prepared → Terminal(Completed)`、只有possible-start且outcome unknown才能进入`Terminal(Abandoned)`。Tool start遵守INV-401，recording不是Tool start marker。

### UserQuestion排他调度

UserQuestion由typed plan shape（`ToolExecutionPlan::UserQuestion`）识别：round按call_index规划每个Prepared slot恰好一次（planner panic按slot隔离、只冻结同一slot的outcome），question hoisted到全部ordinary sibling之前、按call_index串行驱动、至多一个pending；每个question outcome（answer或owner cancellation）由ActiveTurnTask先apply live+inline record attempt，再呈现下一个question或启动ordinary sibling；question绝不预留`ToolStartGate`或mutation ticket、绝不构造start factory，等待期间phase为`WaitingForUserInput`，ordinary sibling在question pending期间绝不factory/poll。valid answer经move-only `UserQuestionAnswerBinding::bind`产生identity-bound `PreExecution + Succeeded` ToolResult；Cancel/SecurityRevoked/Unload signal-first跳过binding并settle全部unstarted calls为matching PreExecution Cancelled；presentation/resolution/binding permit-first各阶段settle truthfully；abandoned question对remaining无副作用（known preflight保留、其余unstarted settle为`PreExecution Failed`）。
...
Interaction request candidate（`InteractionRequestCandidate`）绑定exact ItemId + 已started ToolCallId + request body（mismatch在reducer apply与EntryId allocation前以typed error拒绝）；Approval request tool name必须与exact request call name一致（`matches_request`，mismatch fail closed为identity-bound Abandoned、绝不present）；host resolution经`InteractionResolutionPermit`在Emergency owner mutex内与signal/close first-wins后才apply（signal-first不apply并settle pending interaction为对应cancellation）；append-before-notify与resolution-before-resume语义不变。
...
- Prepared Tool不启动；
- Running Toolbest-effort cancel并等待truthful outcome或Abandoned（已实现：Running slot持有operation自己的`ToolCancellationHandle`，signal先赢后cancel该cancellation pair并经Settling继续await same run至executor cooperative cleanup/result，started run不因signal drop；production `read_file`/`list_directory`保持await同一个owner-tracked blocking job且不detach，工作量/retained allocation有界但ordinary filesystem wall-clock无上界承诺；generic ToolService与其余executor/adapter teardown仍后置，后续adapter必须证明有界工作量并等待可确认settlement）；
- Pending Interaction在live state中Cancelled并resume waiter（含UserQuestion：signal-first跳过binding，全部unstarted calls settle matching PreExecution Cancelled）；
...
- UserQuestion hoisted exclusive scheduling（call_index串行、至多一个pending、question outcome先apply+record再继续）、binding/answer与signal first-wins、signal-first全部unstarted settle为PreExecution Cancelled；
