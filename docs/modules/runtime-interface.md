...
`MiniCoreRuntimeConfig`包含host配置的dedicated durable root、Runtime-global `CompactionSettings`与现有runtime options；它不是wire DTO。closed production Tool opt-ins（`with_ask_user_tool()`与`with_read_file_tool()`，均default-off、idempotent、相互独立）也由该config capture，`open`把它们freeze为单一production Tool config；read_file opt-in同时选择read-only Workspace resolution authority与per-Session永久read revocation control（见下文Production read_file builtin与[Tools](tools.md#production-read_file-builtin)）。`CompactionSettings`默认启用，并可通过`with_compaction_settings(...)`替换；`open`在启动task或打开storage前验证summary min/max，失败返回redacted `InvalidConfiguration`。`open`只使用显式传入的cloned Handle，不调用`Handle::current()`；owner-tracked timer probe把missing runtime/time driver panic或join failure映射为`RuntimeDependencyUnavailable`，DurableState open failures保持一对一分类（该open-time cause与loaded Session readiness的`RuntimeDependencyUnavailable` cause相互独立：后者唯一producer是loaded Turn admission读pinned historical AgentRevisionRef时的transient StorageUnavailable，不是host global bool，也不是`ReloadSharedResources`）。`RuntimeInitializationError`的`Debug`/`Display`同样redacted，不暴露path、OS source或durable bytes。`shutdown`是idempotent host-only non-wire seam，不是第五个protocol entry point。Facade `Drop`只发best-effort Closing signal且不阻塞；host必须await shutdown才能观察全部join、root lease release与Closed。
...
Production read_file builtin：`MiniCoreRuntimeConfig::with_read_file_tool()`是default-off、idempotent、与`with_ask_user_tool`相互独立的opt-in；它同时是Tool安装与read-only authority ceiling（每个declared root的authority ceiling恰为`ReadOnly` filesystem access——requested `ReadWrite`收窄为`ReadOnly`、绝不`ReadWrite`——Prompt/Skill source ceilings保持false）。`open`不选择任何静态ToolSet：每次Turn admission对exact captured Workspace snapshot materialize workspace-bound ToolSet（single `read_file` builtin，或与`ask_user`的frozen composition——恰`ask_user` then `read_file`两个name、一个two-route planner、`FilesystemRead` sandbox contract），`ReloadSharedResources`保留该frozen config不变。ToolName恰为`read_file`，closed schema（required `path` string、`maxLength: 4096`、`additionalProperties: false`），permission恰为`FilesystemRead`，只读cwd-relative path（opaque `AuthorizedWorkspaceReadPath`、capability-relative open；parse/semantic失败、授权Denied、读取失败分别settle frozen Text：`tool arguments are invalid`/`workspace file access is denied`/`file could not be read`/`file is not valid UTF-8`/`file is too large`/`file read was cancelled`）。诚实边界：每次调用work/bytes有界（一次nonblocking capability open + 至多65,537 bytes读取，成功内容恰一个≤65,536 bytes Text part），FIFO等special file被nonblocking open + regular-file check拒绝而绝不挂起；但regular-file wall-clock I/O取决于OS/filesystem，Runtime不fabricate timeout。cancellation truthful：start后、blocking job创建前signal赢→零I/O证明的`Completed + Cancelled`；job运行中signal→保持await同一tracked job并保留truthful result；只有`RuntimeTaskError`→`Abandoned { RuntimeFailure }`。该opt-in不新增wire command/capability；read-only authority的per-Session永久revocation与host security invalidation先revoke再signal/re-resolve、subsequent read denied语义在[Tools](tools.md#production-read_file-builtin)冻结（read_file opt-in下`invalidate_session_workspace_authority`先发布permanent read revocation再signal）。
