...
Production Workspace read builtins：`with_read_file_tool()`与`with_list_directory_tool()`均default-off、idempotent，并与`with_ask_user_tool()`及彼此独立；任一read opt-in同时是Tool安装与共享read-only authority ceiling（每个declared root的authority ceiling恰为`ReadOnly` filesystem access——requested `ReadWrite`收窄为`ReadOnly`、绝不`ReadWrite`——Prompt/Skill source ceilings保持false）。`open`不选择静态Workspace-bound ToolSet：每次Turn admission对exact captured Workspace snapshot materialize enabled selection，fixed order为`ask_user → read_file → list_directory`，任一read route开启时outer sandbox恰为`FilesystemRead`。`read_file`合同仍是cwd-relative bounded UTF-8 regular-file read（opaque `AuthorizedWorkspaceReadPath`、最多65,537 bytes读取、一个≤65,536-byte Text part、fixed failure/cancel texts）。`list_directory`由ADR 0144冻结为cwd-relative direct enumeration：empty path表示cwd，opaque `AuthorizedWorkspaceReadDirectory` capability open，不递归/不读content/不跟随entry symlink，type为`file|directory|symlink|other`，最多256项、retained name bytes≤8,192、compact sorted JSON≤65,536 bytes；fixed texts为`workspace directory access is denied`/`directory could not be listed`/`directory contains an unsupported entry name`/`directory listing is too large`/`directory listing was cancelled`。二者都使用owner-tracked blocking job；scheduling前cancel证明零I/O，scheduled后等待同一job并保留truthful result，`RuntimeTaskError`→`Abandoned { RuntimeFailure }`。这些opt-ins不新增wire command/capability；per-Session永久revocation先revoke再signal/re-resolve，subsequent file/directory reads均Denied。
