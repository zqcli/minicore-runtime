...
同进程Session progress闭环补充：`SubscriptionRequest { scope = Session, include_progress = true }`使用Session actor单点写入的State+Progress有序流；首帧仍是原子捕获的Session Snapshot，随后交付模型文本、选定reasoning progress、ToolInvocation observation、真实ToolResult Text与terminal StateEvent。每个opt-in subscription持有private progress-demand lease；`include_progress = false`继续只进入32-slot state publisher，不增加model progress payload/wake负载。provider model delta先进入Turn-owned bounded mailbox（64 keys、8 pending events、65,536 raw text bytes、32,000-byte UTF-8 segment、capacity-1 wake），pressure只合并same-item连续suffix或丢newest，不进入unbounded owner completion lane。actor按closing/deadline/owner completion/emergency/reliable progress/ordinary/lossy wake优先级单写public stream；owner completion在emergency前保留live terminal与Cancel/Security first-wins，emergency仍在progress与ordinary前。消费stale Turn wake会drain current active mailbox以恢复shared capacity-1 wake credit；phase、Steer safe point、retry与successful terminal形成drain fence，failed/interrupted/cancel关闭并丢pending model draft，late callback/wake benign。ToolResult每个non-empty Text part仍按part顺序切成至多32,000 raw UTF-8 bytes的char-boundary chunks，并经独立capacity-1 reliable lane逐chunk等待actor publication acknowledgement；same V1 shape/identity与canonical frame bound不变，顺序拼接恢复exact原文，subscriber absence/lag不阻塞ack，close drain会settle waiter。每次AgentRun以`content_index + public content kind`绑定stable ItemId并在final `StoredAssistantContent`复用；每个provider publisher捕获private attempt generation，logical retry先使旧generation失效并清attempt-local facts、保留ItemId map，再由actor ack marker，旧attempt callback在任何identity/fallback/mailbox mutation前drop。OpenAI installation默认summary-only；显式`RawText`仍只发布raw channel，observer drop不允许回退混发summary。Anthropic thinking、encrypted/signature、refusal与function-call argument delta继续不公开。该实现不改变Progress/Wire V1 shape。
...
- `CancelAccepted`只表示target仍可取消且sticky cancel epoch已经发布。Input live apply前它使正在进行的capture/composition future失效并取消candidate；Input已apply但`TurnStarted`尚未发布时，它绑定同一Turn并阻止task spawn；active Turn中与private terminal/control commit gate first-wins：Final Assistant在live mutation前claim terminal，Cancel在接受/signal前claim control，双方共用exact Turn EmergencyControl owner mutex。control先赢时final不apply/record并最终Interrupted；terminal先赢时Cancel返回既有`TurnTerminal` rejection且final只出现一次。worker已计算Failed但actor尚未settle时同一gate仍裁决accepted→Interrupted或completion→rejection；绝不在final live commit后仅重写terminal label。Tool清理和Turn terminal通过后续StateEvent/Snapshot观察；
...
- 可以按SessionId/TurnId/ItemId合并连续delta；Runtime只在private 8-event mailbox受压时UTF-8-safe合并same-item newest suffix，单segment最多32,000 raw bytes；
- model mailbox最多保留65,536 raw text bytes并跟踪64个key；满后丢newest model suffix，不驱逐已接受event，`ItemStarted`与首个retained delta作为同一acceptance且started始终先发布；
- `include_progress=false`不构造model observer payload或发送wake，但ItemId、delta-seen与reasoning fallback事实仍更新；opt-in stream在receive error或State projection failure关闭时立即释放progress-demand lease，即使Host继续持有closed `EventStream`也不再产生model observer payload；
- `ToolOutputDelta`对每个non-empty ToolResult Text part按原part顺序发布UTF-8-safe chunks；单chunk最多32,000 raw bytes，empty part无event，chunk间无separator，拼接必须得到exact原文；ToolOutput走capacity-1 reliable lane，accepted chunk不合并/丢弃且actor ack先于下一chunk/Model；
- queue满时可以丢弃model progress；ToolOutput不使用该lossy queue；
...
- Cancel/Security/Unload/failure关闭当前Turn mailbox并丢pending未finalize model state；logical `model_retry_scheduled`经reliable fence在previous retained delta后、next attempt前发布，要求Host清除上一attempt临时view；Runtime在marker前先递增attempt generation，旧publisher不能再改变ItemId、fallback或mailbox；
- successful phase/final observation与terminal前drain retained model progress；failed/interrupted terminal丢pending model progress。Host收到同ItemId的`item_completed`或Turn terminal后忽略迟到started/delta；late callback/wake在Runtime侧同样benign drop，不fatal-closeSession；
