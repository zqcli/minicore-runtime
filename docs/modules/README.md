...
本目录是MiniCore V2当前权威module设计。ADR 0126–0137冻结async conversation/public wire、DurableState/Store V1/root lease与Tokio owner-tracked foundations。M5.0 durable foundation与exact historical definition resolution已实现；M6.1 Workspace resolver/Snapshot、loaded Ready+Idle publication owner、Runtime-owned residency actor/Load/Unload/lifecycle exclusion，以及replay/Recorder-backed Ready+Idle Load hydration已实现；public Runtime command/query/snapshot/event、remaining Fork anchors/LiveSnapshot、active-Turn grace Unload、完整cross-platform native matrix、完整SessionExecutor Turn control/ActiveTurnTask、M8 public DTO、M10 compaction与provider/Tool adapter行为仍待实现。
