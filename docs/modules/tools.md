...
`ToolPermissionSet::restricted_candidate(candidate)`是ADR 0140的closed typed restricted-option seam：接受exact `ToolPermissionSet` candidate，只允许等于或窄于current ceiling（equal/subset/empty均成功并返回该candidate），新增任何capability class返回closed typed `ToolPermissionRestrictionError::ElevatesCapabilities`。`ToolSandboxContract::admit(final_permissions)`只在Sandbox available且`final ⊆ enforceable`时返回move-only proof；否则返回`Unavailable`或携带exact `final − enforceable`的`CapabilityGap`。两类失败都只转换为固定、bounded、non-secret的`PreExecution + Denied`文本，missing classes不进入model-visible content。M13.3后per-request planner只返回一个plan；`Execute.permissions`与`Approval.permissions`是final class-level事实唯一owner。Direct Execute在plan离开Tools owner前admit；Approval先present host，`AllowOnce`重新admit同一ceiling，`AllowWith(candidate)`在approval settlement中经`restricted_candidate` typed seam于release code证明candidate不宽于ceiling后再admit candidate；typed elevation失败仍以CapabilityGap settle固定PreExecution Denied，绝不reserve gate。只有成功才进入ToolStartGate reservation；失败直接settle PreExecution Denied。该合同不代表resource-level path/host/process grants或production Sandbox已实现。
...
production `read_file`是closed、default-off、Runtime-owned builtin，唯一实现位置`src/tools/read_file.rs`；该module向parent composer提供frozen `definition/plan`，standalone `build_tool_set/sandbox`仅为`cfg(test)` focused fixture。`ProductionToolConfig`与`TurnToolResources::Production`是唯一production selection/composition与per-admission materialization路径：
...
ADR 0144冻结的production `list_directory`是closed、default-off、Runtime-owned builtin，唯一实现位置`src/tools/list_directory.rs`；该module向parent composer提供frozen `definition/plan`，standalone `build_tool_set/sandbox`仅为`cfg(test)` focused fixture，production安装只走`ProductionToolConfig`：
...
ADR 0147冻结的production `fetch_url`是closed、default-off、Runtime-owned network builtin，唯一实现位置`src/tools/fetch_url.rs`；该module向parent composer提供frozen `definition/plan`，standalone `build_tool_set/sandbox`仅为`cfg(test)` focused fixture。production安装由Runtime在`open`时materialize authority后传入`ProductionToolConfig`：
...
同进程progress订阅中，assistant ToolCall完成live apply与inline record attempt后，Session actor先drain前序retained model progress，再发布携带Started ToolInvocation的`session_execution_changed` snapshot；任何Tool start都发生在该observation acknowledgement之后。exact `ToolExecutionOutcome::Completed`随后先apply/record，再按`ToolResultContent.parts()`顺序把每个non-empty真实Text part切成至多32,000 raw UTF-8 bytes的char-boundary chunks。ToolOutput不进入lossy model mailbox或unbounded owner completion lane：ActiveTurnTask经capacity-1 reliable progress lane逐chunk发送并等待actor acknowledgement，accepted chunk不合并/丢弃；subscriber absence/lag只影响observer receipt，不阻塞actor ack。empty part不发布，part/chunk之间不插入separator，non-lagged subscriber按顺序拼接恢复exact原文。32,000-byte canonical V1 frame证明、stored/model-visible result与`result: None`均不变。Abandoned没有synthetic output delta。close drain会ack-discard已接受reliable command使task退出，不会卡住shutdown；ordinary active Turn保持ToolInvocation → ToolOutputDelta chunk1..N → next model progress → terminal的actor单写顺序。
...
- [x] production `read_file` builtin（closed/default-off/Runtime-owned、`MiniCoreRuntimeConfig::with_read_file_tool()` idempotent opt-in与read-only authority ceiling、`WorkspaceResolver::new_with_read_access` + owner-held `WorkspaceReadAccessControl` per-Session永久read revocation（host security invalidation先revoke再signal）、`ProductionToolConfig` closed composition与per-admission materialization、cwd-relative opaque `AuthorizedWorkspaceReadPath`授权、capability-relative nonblocking open（symlink escape/特殊文件fail closed）、regular-file/UTF-8/65,536-byte有界读取、owner-tracked blocking execution与truthful cooperative cancellation）；
