...
- **四路frozen selection与per-admission materialization**：`ProductionToolConfig::new(ask_user, read_file)`在`open`时冻结exact base一次（empty default或single ask_user builtin），恰有四路selection：`(off, off)`→empty default ToolSet；`(on, off)`→single ask_user builtin；`(off, on)`→single read_file builtin（pinned到exact captured Workspace tool context与Runtime task context）；`(on, on)`→一个immutable composed ToolSet，恰含`ask_user` then `read_file`两个definition/spec（frozen顺序，prompt projection同序）、一个frozen two-route planner（每个valid call路由到恰一个builtin planner，unknown name经正常ToolSet lookup不可用）、outer sandbox contract为read_file的FilesystemRead contract。`TurnToolResources::Production`在每次admission经`for_workspace`对exact captured Workspace snapshot materialize恰一次（consumes self），不存在单一static ToolSet selection；`ReloadSharedResources`保留该frozen config不变（per-admission materialization对未来snapshot生效）。
...
2. production builtin Tool最小集合（`ask_user`已由ADR 0142冻结并实现，`read_file`已由production read_file milestone冻结并实现；是否增加其他builtin仍开放）；
...
- [x] production `read_file` builtin（closed/default-off/Runtime-owned、`MiniCoreRuntimeConfig::with_read_file_tool()` idempotent opt-in与read-only authority ceiling、`WorkspaceResolver::new_with_read_access` + owner-held `WorkspaceReadAccessControl` per-Session永久read revocation（host security invalidation先revoke再signal）、`ToolSet::read_file_builtin()`与`ProductionToolConfig`四路frozen selection/per-admission materialization、cwd-relative opaque `AuthorizedWorkspaceReadPath`授权、capability-relative nonblocking open（symlink escape/特殊文件fail closed）、regular-file/UTF-8/65,536-byte有界读取、owner-tracked blocking execution与truthful cooperative cancellation）；
