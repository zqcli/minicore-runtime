...
状态：M5.0 design gate 已冻结；foundation implementation pending（ADR 0136、0137）
日期：2026-08-03
...
MVP support is claimed only when host configuration supplies a dedicated user-private coherent root on APFS, NTFS, ext4 or XFS and the native platform suite passes. MiniCore validates observable invariants; it does not pretend to identify every mount technology perfectly. NFS, SMB, WebDAV, FUSE, overlay filesystems, mapped remote drives and hostile concurrent filesystem mutation are unsupported host configurations. The root itself must not be a symlink/reparse point. On Unix, the root and every recognized descendant must be owned by the effective user, directories must be exactly `0700`, regular files exactly `0600`, and every descendant must have the root device identity. The Windows adapter requires one root volume identity and rejects recognized reparse/cross-volume descendants. Root ancestors and a user-private Windows ACL are host trust preconditions. The advisory lease is cooperative, not a security boundary. Unix creation targets use directories `0700` and files `0600`; Windows relies on the trusted inherited user-private ACL.

Every recognized component must have exact ASCII spelling. Recovery rejects case-fold aliases (also on case-insensitive volumes), lossy/non-UTF-8 recognized names, recognized symlinks, Windows reparse points, and unexpected recognized namespace entries. It enumerates then sorts canonical parsed names; `read_dir` order is never semantic.
