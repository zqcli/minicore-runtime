...
当前 foundation已由同一actor中的concrete Agent Create、ordinary Session Create、unloaded RecordedHistory + Genesis Session Fork tracer及Agent status/definition/metadata、Session metadata G2 CAS与Session lifecycle existing-head action tracers消费：sealed lifecycle attempt在reservation成功后生成exact G1 payload；Session Create在private Agent lifecycle gate内读取current Enabled exact ref并持有同一gate至owner-tracked publication child完成；Genesis Fork先以opaque same-open source lease验证published source Header/file observation并释放source lease，再在同一Agent gate/certainty路径重编码child Header、固定captured retained Agent revision、发布COMMITTED/PUBLISHED、安装catalog并完成close/reopen recovery。**没有 standalone production `reserve` API、reservation token/receipt 或 caller-visible reservation waiter**。Agent status CAS在exclusive Agent gate内执行expected-status recheck、stale-before-no-op、Deleted terminal与status-only G+1 COMMITTED publication；Agent definition CAS在同一exclusive gate内执行expected-revision recheck、stale-before-no-op、Deleted terminal与definition-bearing G+1 COMMITTED publication；Agent metadata CAS保留name Keep/Set与description Keep/Set/Clear patch intent，在gate内按expected metadata revision→Deleted→canonical no-op顺序解析并发布metadata-only G+1。三者均在complete full-chain readback后先install catalog、settle当前请求再释放gate；Session metadata CAS在独立per-Session gate内保留name/description Keep/Set/Clear patch intent，发布metadata-only G+1且不触碰conversation。Session lifecycle existing-head action在同一per-Session gate内依据authoritative current lifecycle执行Archive/Unarchive/Delete closed matrix，无expected lifecycle token，发布head-only G+1并保留definition、metadata、fork provenance、createdAt与conversation physical identity；DurableState不拥有residency，future Runtime owner必须从Unloaded检查到durable completion持续排除并发Load。non-Genesis/LiveSnapshot semantic streaming、Session definition mutation与Session lifecycle Runtime integration仍待后续slice消费同一基础。
