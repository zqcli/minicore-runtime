...
`ExclusiveWritableConversationLease` has **no public or caller-visible constructor**. `DurableState` now issues a `PublishedConversationTarget` after exact initial Header and same-open-file physical validation; Conversation Storage consumes that target to create the paired opaque writable proof from a cloned same-open handle. The existing `#[cfg(test)]` constructor remains scanner-test-only. Recorder Load/open/write failures with root lease intact remain local to that Session and create `RecordingHealth::Degraded`; loss of root lease/lock-file identity is global poison/close, never Session Degraded.
