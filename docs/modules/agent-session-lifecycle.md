...
- [x] M5.0冻结DurableState、Durable Store V1、root lease、permanent reservations（new-entity Create/Fork complete-or-invisible、existing-head update old-or-new）与Tokio/deterministic test seams；production recovery/root lease/owner-tracked actor、private permanent reservation foundation与crate-private Agent Create exact G1 COMMITTED/PUBLISHED publication已实现；public Runtime Agent command接入、Session Create/Fork/update/CAS、完整cross-platform native matrix、Recorder/replay仍pending。
