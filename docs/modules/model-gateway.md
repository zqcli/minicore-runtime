...
两个adapter只共享private connection client construction、artifact-derived固定产品User-Agent、bounded body drain、numeric Retry-After、event-stream content-type check与bounded SSE framing；不能共享会抹平OpenAI/Anthropic request、terminal、typed envelope、metadata或delivery差异的generic response model。M14已把shared transport固定为exact `reqwest = 0.13.4`的`json + rustls + stream`最小features，显式`retry::never()`、no redirect/no proxy与`minicore-runtime/<crate-version>`产品身份，并由真实Rust 1.85冷编译、OpenAI 36个与Anthropic 43个local loopback/focused tests验证；两个protocol parser仍完全独立。
...
Anthropic visible thinking可以没有signature：adapter将其truthfully保留为`text=Some, signature=None`，但该artifact不是可回传provider的signed reasoning。Anthropic replay只编码exact text+signature pair、signature-only或明确的redacted artifact；unsigned text-only reasoning自然省略，绝不合成signature或改写为普通assistant text。若late `signature_delta`建立non-empty signature，则最终artifact恢复为可按existing exact规则回放的signed reasoning。
...
- 首个production baseline只支持`OpenAiResponses | AnthropicMessages`；两个protocol adapter、dynamic credential resolution与catalog/model source installation均已完成。默认Runtime仍为空catalog；只有trusted host显式安装的definition可解析。explicit ignored live smoke harness已实现，2026-08-12两个public Runtime-path real-credential release smoke均通过（ADR 0145）；
...
- [x] 在显式release环境执行并记录OpenAI Responses与Anthropic Messages real-credential public Runtime smoke；固定产品User-Agent、Anthropic unsigned thinking与omitted start stops由ADR 0145冻结，默认tests继续ignored/offline。
- [ ] 激活public structured requester/Wire/SessionDefinition字段与Runtime/ActiveTurnTask structured调用（显式cache/continuation保持ADR 0141 omission，见下节）。
