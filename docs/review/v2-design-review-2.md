...
`ToolAuthorization` 在 approval 之后、`record_execution_start` 之前用 `enforceable()` 与最终 PermissionSet 求差：差集非空 → PreExecution 拒绝。未声明的 class 视为不可强制（fail closed 默认）。`SandboxEnforcementCapabilities` 进入 `ToolSetFingerprint` 覆盖范围（它影响哪些调用能执行）。
...
3. *ContentHash 规范化*：算法定为 SHA-256；输入必须是确定性 canonical encoding——固定字段顺序的专用序列化（不是 serde JSON 默认输出），集合先按 stable identity 排序，禁止 HashMap 迭代顺序参与；每类 fingerprint 声明自己的 coverage + algorithm version 字段（capture/estimator/canonicalization version 已有先例，补齐其余）。产出 golden vector 测试文件作为冻结物。
   - 后续修订：[ADR 0122](../adr/0122-workspace-fingerprints-are-runtime-local.md)将Workspace及其窄view fingerprint明确排除在跨Runtime ContentHash freeze之外；这些值是Runtime-local opaque identity，不要求canonical encoding、algorithm version或golden vector。O14/O15等真正持久内容identity仍需单独冻结。
...
1. *Skill 弱一致例外标注*：`turn-execution-context.md` 不变量「active Turn 不读取这些 Service 的 future current value」句尾加「（唯一例外：尚未 lazy-load 的 Skill entry 按 captured location 读取当前文件内容，见 C4 决议的弱一致语义）」；该例外同时进入 R6 的 INV-201 条目文本。
