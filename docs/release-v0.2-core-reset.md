...
The production crate contains 55 Rust source files and approximately 16,518 production lines. Its canonical top-level owners are `agent`, `config`, `error`, `event`, `ids`, `model`, `prompt`, `runtime`, `session`, `tools`, and `workspace`. `agent` and `prompt` are private. The only accepted multi-module dependency cycle is `{agent, prompt, session}`; every other module SCC is a singleton.
