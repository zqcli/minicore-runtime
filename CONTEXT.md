...
- M0–M11已完成并通过统一review/check；M11关闭Session Fork command/storage、durable Agent/Session catalog/Fork provenance query、Runtime Session membership/lifecycle StateEvent、public Agent definition/metadata CAS、public Session metadata CAS、ordinary Session definition CAS、Agent revision upgrade、Ready-state `ReloadWorkspace`、Workspace/Prompt/Agent/Model/selected Prompt/RuntimeDependency loaded readiness与恢复、shared-resource reload recovery/fanout与complete shared-root publication、active-Turn graceful Unload、host security Workspace authority invalidation（`invalidate_session_workspace_authority` host-only非wire seam、`workspace_preparing`→Preparing、owner-tracked re-resolve recovery、exactly-once waiter settlement）、public manifest无pending closure及full recovery scenario/fixture closure；最终`./scripts/check.sh`与`./scripts/check-msrv.sh`在current HEAD通过748个library tests（3 ignored）、152个integration tests、Clippy、format、docs与全部fixtures。M1 Wire foundation/owner semantic spine亦已通过Fast、MSRV与heavy gates；自动化Linux/macOS/Windows native matrix acceptance已通过（全部七个`platform_m5_0`坐标均有对应的production行为与测试覆盖；GitHub Actions run 31433810296四个job全部通过：Ubuntu Rust stable、Ubuntu Rust 1.85.0、cargo test macos-latest、cargo test windows-latest；本地`./scripts/check.sh`与Rust 1.85门禁已通过748个library tests（3 ignored）、152个integration tests、Clippy、format、docs与全部fixtures）、crate-private `ToolOperationSlot`完整生命周期已实现（Prepared→Running→Settling→Terminal：per-slot first-wins gate、typed started proof、Running cancellation pair与truthful settle；production executor/adapter teardown仍pending）、Structured output foundation已实现而public activation与provider-native schema mapping、具体Prompt/Skill source adapter、production ask-user builtin ToolName/schema与answer→model-visible ToolResult text/render格式、完整Tool policy/approval enforcement、Session-local mutation queue/mutation permit attachment to Settling、production ToolService/executor/adapters、public Tool DTO与具体Skill composition/source、production Rig adapter仍pending；
