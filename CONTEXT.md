...
当前开发分支是`dev`。M12 Production Provider Gate（V4-P1-3）已完成；换机后执行`git switch dev && git pull --ff-only origin dev`，再用`git status --short --branch`确认工作树干净。与本任务无关的`stash@{0}: On refactor/async-loop-eventual-session-log: wip: paused bounded JSON implementation before dev merge`仍保留，不要自动apply/drop。
...
- 首版`ProviderProtocol`只支持OpenAI Responses与Anthropic Messages；M14 production adapters通过前都不显示为available，OpenAI Chat Completions/Gemini等未经验证protocol不接受；
- exact `rig-core = 0.40.0`仅为dev-dependency，Rig类型只出现在`tests/m12_*`，不进入`src/`、Prompt、Conversation或Runtime public DTO；
...
ModelGateway private seam，只执行具体provider request/stream/cancellation/response mapping。M12只批准OpenAI Responses与Anthropic Messages baseline；SDK automatic retry固定为0，protocol terminal与allowlisted metadata通过private `HttpClientExt` wrapper保存。production adapter仍属于M14，Rig type不得越过private seam。
...
- 第四轮评审：全部普通V4-P0/P1已关闭；V4-P1-3由M12/ADR 0138关闭，仅conditional V4-C0-1仍开放；
- M0–M12已完成并通过统一review/check；最终`./scripts/check.sh`与`./scripts/check-msrv.sh`在M12 HEAD通过748个library tests（3 ignored）、184个integration tests、Clippy、format、current/archive docs、Wire V1 144 active/0 pending与Durable Store fixtures；
- M12只冻结OpenAI Responses/Anthropic Messages production baseline、Rig terminal/metadata seam和delivery/error contract，不实现production provider adapter；M14仍需实现private `RigProviderAdapter`、credential/client factory、connection/cache/continuation、provider-native Structured mapping及production contract suite；
- M13/V4-C0-1：production Tool/Sandbox adapter前关闭enforcement fail-closed合同；Structured public activation、具体Prompt/Skill source、production ask-user builtin、Session-local mutation permit attachment、完整Tool policy/Sandbox与public Tool DTO继续pending。
