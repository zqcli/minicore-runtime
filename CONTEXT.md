...
- M0–M10已完成，M11 Session Fork command/storage、durable Agent/Session catalog/Fork provenance query、Runtime Session membership/lifecycle StateEvent、public Agent definition/metadata CAS及public manifest无pending closure已完成；M1 Wire foundation/owner semantic spine已通过Fast、MSRV与heavy gates。自动化Linux/macOS/Windows native matrix、M11 remaining Session mutation/readiness/full recovery closure、complete shared-root publication、具体Prompt/Skill source adapter、Structured assembly、production Rig adapter与active-Turn grace Unload仍pending；
