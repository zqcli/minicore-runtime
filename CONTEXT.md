...
ModelGateway private seam，只执行具体provider request/stream/cancellation/response mapping。M12只批准OpenAI Responses与Anthropic Messages baseline；adapter/private HTTP client automatic retry固定为0，protocol terminal与allowlisted metadata由`OpenAiResponsesProviderAdapter`或`AnthropicMessagesProviderAdapter`在wire owner内保存。production adapter仍属于M14；Rig只存在于standalone evidence harness，provider/transport type不得越过private seam。
...
- 第四轮评审：全部普通V4-P0/P1已关闭；V4-P1-3由M12/ADR 0138/0139关闭，仅conditional V4-C0-1仍开放；
- M0–M12已完成并通过统一review/check；`./scripts/check.sh`通过主crate748个library tests（3 ignored）、160个integration tests及provider-gate 25个tests，`./scripts/check-msrv.sh`用真实Rust 1.85通过主crate748个library tests（3 ignored）与160个integration tests；Clippy、format、current/archive docs、Wire V1 144 active/0 pending与Durable Store fixtures通过；
