...
- `conversation`: canonical entries, semantic validation, proof-gated load, paged replay, restart repair, append coordination, confirmed projection, canonical prompt-history and compaction-candidate proofs, transcript, and one crate-private `DurabilityClass` seam for log/commit known, unknown, and consistency outcomes. Transcript caller/temporary/closed dispositions remain operation-specific.
...
- `model`, `tools`, `context`, `compaction`: host-neutral Ports and checked DTOs bound immutably for the loaded lifetime. Their private drivers share one crate-private deadline selector whose equal-deadline rule conservatively chooses the Turn source. The separate crate-private `port_call` module centralizes bounded one-shot cancellation/deadline/panic/drop control only for Context, Compaction, ToolPolicy, and Tool execution.
...
- `agent::tool_driver` and `agent::runner_protocol`: private P5-B execution modules owning frozen-spec policy evaluation, typed approval/input suspension, panic-safe Tool execution, child cancellation, session output bounds, direct preservation of in-bound completed ToolOutputs, and lossy progress. They never append, spawn, or own SessionRuntime/log authority.
...
- `compaction::driver`: private P5-D/P5-E2 module binding zero or one CompactionStrategy, a checked timeout/summary snapshot, candidate validation and test pause outside the shared call, then immediate entry into `run_port_call`; its closure first resolves completed-boundary/strategy availability to `Ok(None)` or invokes the strategy as `Ok(Some(...))`. Invalid effective deadlines map to Compaction `DeadlineExceeded`; it has no conversation mutation, log, model, tool, context, workspace, or owner authority.
...
Local work for this phase uses Python architecture/docs/source checks and `git diff --check`. Remote evidence includes the regenerated 37-record lockfile, stable `scripts/check.sh`, MSRV `scripts/check-msrv.sh`, and warnings-denied rustdoc; the cleaned root library run reported 305 passing tests, with integration and provider-gate suites also passing. The complete evidence map is [docs/acceptance-v0.3.md](docs/acceptance-v0.3.md), and publication status is [docs/release-v0.3.md](docs/release-v0.3.md).
