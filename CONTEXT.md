...
- 首版`ProviderProtocol`只支持OpenAI Responses与Anthropic Messages；两个direct adapter及local contract suite均已完成，但在dynamic credential/catalog installation与release smoke通过前仍不显示为available；OpenAI Chat Completions/Gemini等未经验证protocol不接受；
...
ModelGateway private seam，只执行具体provider request/stream/cancellation/response mapping。M12只批准OpenAI Responses与Anthropic Messages baseline；adapter/private HTTP client automatic retry固定为0，protocol terminal与allowlisted metadata由`OpenAiResponsesProviderAdapter`或`AnthropicMessagesProviderAdapter`在wire owner内保存。M14已实现两个direct adapter/local contract slices；dynamic credential/catalog installation与live opt-in smoke仍pending。Rig只存在于standalone evidence harness，provider/transport type不得越过private seam。
...
- M12冻结OpenAI Responses/Anthropic Messages production baseline、terminal/metadata与delivery/error contract，并明确拒绝Rig进入Rust 1.85 production baseline；M14已实现两个direct private adapters、provider-native Structured strict mapping及默认离线production contract suites，仍需dynamic credential/catalog installation、connection/cache/continuation与live opt-in smoke；
- M14：继续实现dynamic provider credential/catalog installation、显式live opt-in smoke与production Tool/Sandbox adapters；Structured public activation、具体Prompt/Skill source、production ask-user builtin、Session-local mutation permit attachment、resource-level Tool policy/Sandbox与public Tool DTO继续pending。
