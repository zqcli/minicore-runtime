...
- M0–M10已完成，M11 Session Fork command/storage、durable Agent/Session catalog/Fork provenance query、Runtime Session membership/lifecycle StateEvent、public Agent definition/metadata CAS、public Session metadata CAS、ordinary Session definition CAS、Agent revision upgrade、Ready-state `ReloadWorkspace`、Workspace/Prompt Unavailable loaded readiness与ReloadWorkspace恢复及Agent readiness fan-out与ModelUnavailable及selected PromptUnavailable load/definition projection、public manifest无pending closure已完成；M1 Wire foundation/owner semantic spine已通过Fast、MSRV与heavy gates。自动化Linux/macOS/Windows native matrix、shared-resource reload recovery/fanout与complete shared-root publication与active-Turn graceful Unload及host security Workspace authority invalidation（`invalidate_session_workspace_authority` host-only非wire seam、`workspace_preparing`→Preparing、owner-tracked re-resolve recovery、exactly-once waiter settlement）已实现，RuntimeDependencyUnavailable与full recovery scenarios、具体Prompt/Skill source adapter、Structured assembly、production Rig adapter仍pending；
