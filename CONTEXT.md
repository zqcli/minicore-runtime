...
当前进入M13 Production Tool/Sandbox Gate（V4-C0-1）。M13.1已实现closed四类capability set、final `ToolPermissionSet` restricted candidate、adapter available/enforceable contract、exact差集、Sandbox unavailable fail-closed与fixed `PreExecution + Denied` mapping；M13.2已让per-request direct Execute plan在任何ToolStartGate reservation/start factory poll前admit，gap/unavailable丢弃未调用factory并变成frozen PreExecution Denied。approval resume revalidation与production adapter仍pending。后续继续用fake backend关闭approval不可提升、SecurityRevoked/Sandbox unavailable/Running truthful settlement；Session-local file alias FIFO仍等待真实file Tool/canonical target consumer，不提前造无consumer queue。关闭M13后再进入M14 production adapters。任何新Tool adapter必须保持：无raw `tokio::spawn`脱离owner、started run不因signal drop、返回前提供有界可确认cleanup、Emergency mutex内只允许固定非阻塞exact reservation/transition。
