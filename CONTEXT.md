...
- M14：继续执行并记录显式real-credential live smoke，随后实现production OS-backed Tool/Sandbox adapters；Structured public activation、具体Prompt/Skill source、Session-local mutation permit attachment、resource-level Tool policy/Sandbox与public Tool DTO继续pending（production `ask_user` builtin已由ADR 0142实现）。
