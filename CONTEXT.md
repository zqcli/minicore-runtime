...
- M0–M10已完成，M11 Session Fork command/storage、durable Agent/Session catalog/Fork provenance query及Runtime Session membership StateEvent纵向切片已完成；M1 Wire foundation/owner semantic spine已通过Fast、MSRV与heavy gates；M2仍按owning behavior slice增量激活remaining DTO。自动化Linux/macOS/Windows native matrix、M11 remaining public family/event/full recovery closure、complete shared-root publication、具体Prompt/Skill source adapter、Structured assembly、production Rig adapter与active-Turn grace Unload仍pending；
