...
当前M14 production `list_directory` milestone已完成完整本地门禁：`./scripts/check.sh`运行主crate library 935 passed、3 ignored，integration 159 passed、3 ignored（其中2个ignored是默认off live provider smoke），以及standalone provider-gate 25/25；`./scripts/check-msrv.sh`显式锁定真实Rust 1.85 compiler并在隔离target运行主crate全部targets，结果同为library 935 passed、3 ignored与integration 159 passed、3 ignored。Clippy、format、current/archive docs、Wire V1 144 active/0 pending与Durable Store fixtures全部通过。两个live smoke harness在两套toolchain中均被编译且默认ignored，不读取env或访问network；实际real-credential run未执行。read_file slice的历史提交保持不变；list_directory milestone由五个提交组成：authorize workspace directory reads、add production list_directory builtin、install production list_directory tool、bound directory entry name copies、freeze production list_directory contract。最近remote CI仍是M13 closure SHA `0951b12a9584c5f26f4757bf84cefb21e824d1dd`的GitHub Actions run `31518732896`，已在Ubuntu stable、Rust 1.85、macOS与Windows四个job全部success。
