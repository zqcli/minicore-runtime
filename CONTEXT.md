...
- M14：继续执行并记录显式real-credential live smoke，随后实现其余production OS-backed Tool/Sandbox adapters（write/network/process、其它file adapter；首个narrow `read_file` slice已完成并由ADR 0143冻结）；Structured public activation、具体Prompt/Skill source、Session-local mutation permit attachment、resource-level Tool policy/Sandbox与public Tool DTO继续pending（production `ask_user` builtin已由ADR 0142实现、production `read_file` builtin已由ADR 0143实现）。
