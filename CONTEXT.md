...
ActiveTurnTask注入ToolSet的crate-private interface，用于approval、UserQuestion和ToolStartGate。它不暴露Session state。完整trait（`request_approval`/`request_user_question`/`reserve_execution_start`）仍是future target；当前start gate由Session Execution的`RoundToolStartGate`（绑定exact EmergencyControl handle/observation与每request一个`ToolStartGate`的concrete narrow seam）+ Tools的`ToolStartPermit`/`ToolStartedExecution`组合实现，approval/UserQuestion lane仍pending。
