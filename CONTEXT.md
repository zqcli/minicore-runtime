...
- M0、M1、M2 minimal Snapshot/Event、M3.1、M3.2和M4已完成；M1 Wire foundation/owner semantic spine已完成并通过Fast、MSRV与heavy gates；M2仍按slice增量推进，已完成Protocol V1 bootstrap router、incremental public manifest conformance gate、initial typed Wire roots、M7 Create/Load/Unload/Submit/Cancel command codec、TurnStarted/CommandOutput/typed rejection completion，以及minimal loaded-ready-idle SessionSnapshot与Runtime/Turn terminal StateEvent codec；M3.1 exact line codec与M3.2 physical scanner已完成；M5.0 durable foundation与exact historical definition resolution已完成；M6.1 resolver/Snapshot、loaded Ready+Idle publication owner及Runtime-owned residency foundation已完成，覆盖single-flight Load、draining Unload、per-Session typed exclusion、Archive/Delete loaded check到durable completion、unified loaded/unloaded Workspace update、caller cancellation、pre-close reserved request、child reap、gate cleanup与shutdown owner retention；Runtime现拥有PromptService/initial PromptResourceView与empty ModelGateway/initial ModelCatalogView，Workspace Prompt candidate capture已接入Load和loaded Idle publication并保持candidate failure old-snapshot semantics；M6.2 scripted text-only assembly/request/single-attempt/validation/cancellation vertical slice已完成，reload后old TurnModelSnapshot继续调用retained old adapter；Agent/Session Create/Fork Unix process-abort前后PUBLISHED tracer已在macOS本地验证，自动化Linux/macOS/Windows native matrix、remaining Fork anchors/LiveSnapshot、complete shared-root publication、具体Prompt/Skill source adapter、Structured/Tool/Compaction assembly、production Rig adapter、public Runtime command/snapshot/event、active-Turn grace Unload pending；M5.2 tolerant semantic replay seam与corruption sidecars及replay/Recorder-backed Ready+Idle Load hydration已实现；M4已完成Prompt-owned opaque `ModelMessage`、exact `ConversationRevision`/`EntryIdGenerator`、`LiveSessionState` reducer与Compaction stable units/source/replacement subset。M5拥有tolerant recorded-marker replay，M10才完成full Compaction；M4 read scope仍为crate-private snapshot，public Item/Interaction DTO随M8激活；
