...
- M14：provider real-credential smoke项已由ADR 0145关闭，首个file-mutation adapter与Session-local queue/permit已由ADR 0146关闭；继续实现其余production OS-backed Tool/Sandbox adapters（network/process及其他未实现adapter）、generic ToolService/policy、Structured public activation、具体Prompt/Skill source与public Tool DTO（production `ask_user`/`read_file`/`list_directory`/`write_file` builtins已由ADR 0142/0143/0144/0146实现；Public Wire V1与Durable Store V1均为closed exact schemas，Structured activation若进入SessionDefinition必须走独立protocol minor与storage migration）。
