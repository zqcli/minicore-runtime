...
- `context::driver` and `prompt::builder`: private P5-C modules owning one-provider context deadlines/panic isolation, the crate-private canonical `ValidatedContextBundle` seam, consumption of conversation-owned prompt proofs, deterministic mapping, stable context headers, exact frozen tools, and exact serialized-request output-reserved budgeting. ContextDriver is the sole production validator/constructor for the checked context bundle; PromptBuilder consumes it without revalidation. They invoke no model, tool, log, workspace, or owner.
