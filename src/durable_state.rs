...
use crate::agent_session_lifecycle::{
    ...
    SealedSessionCreateAttempt, SealedSessionForkAttempt, SealedSessionMetadataAttempt,
...
fn session_agent_basis(
    entry: &DurableAgentCatalogEntry,
    pinned: AgentRevisionRef,
) -> Option<DurableAgentCatalogEntry> {
    if !entry.definition_index.contains_key(&pinned.revision()) {
        return None;
    }
    Some(DurableAgentCatalogEntry {
        ...
        definition_index: Arc::clone(&entry.definition_index),
    })
}
...
pub(crate) enum DurableSessionForkError {
    ...
    #[error("durable state exceeds its selected size limit")]
    DurableStateTooLarge,
    #[error("Fork child identity is unavailable")]
    IdentityUnavailable,
    #[error("Fork child identity allocation attempts are exhausted")]
    CollisionAttemptsExhausted,
    #[error("Fork child identity conflicts with its source")]
...
#[derive(Clone, Copy, Debug, Eq, Error, PartialEq)]
pub(crate) enum DurableAgentMetadataError {
    #[error("durable state is closing")]
    Closing,
    #[error("Agent was not found")]
    AgentNotFound,
...
#[derive(Clone, Debug)]
pub(crate) enum DurableAgentMetadataOutcome {
    NoChange(Arc<DurableAgentHead>),
    Updated(Arc<DurableAgentHead>),
}
...
impl DurableAgentMetadataOutcome {
    pub(crate) fn head(&self) -> &Arc<DurableAgentHead> {
        match self {
            Self::NoChange(head) | Self::Updated(head) => head,
        }
...
#[derive(Clone, Copy, Debug, Eq, Error, PartialEq)]
pub(crate) enum DurableSessionMetadataError {
    #[error("durable state is closing")]
    Closing,
...
struct SetAgentMetadataRequest {
    attempt: SealedAgentMetadataAttempt,
    response:
        Option<oneshot::Sender<Result<DurableAgentMetadataOutcome, DurableAgentMetadataError>>>,
    closing: CancellationToken,
    permit: Option<AgentMutationPermit>,
}

impl SetAgentMetadataRequest {
    fn settle(&mut self, outcome: Result<DurableAgentMetadataOutcome, DurableAgentMetadataError>) {
        if let Some(response) = self.response.take() {
            let _ = response.send(outcome);
        }
    }

    fn reject(&mut self, error: DurableAgentMetadataError) {
        self.settle(Err(error));
    }
}

impl Drop for SetAgentMetadataRequest {
    fn drop(&mut self) {
        let error = if self.closing.is_cancelled() {
            DurableAgentMetadataError::Closing
        } else {
...
struct SetAgentMetadataWaiter {
    response: oneshot::Receiver<Result<DurableAgentMetadataOutcome, DurableAgentMetadataError>>,
}

impl SetAgentMetadataWaiter {
    async fn wait(self) -> Result<DurableAgentMetadataOutcome, DurableAgentMetadataError> {
        self.response
            .await
            .unwrap_or(Err(DurableAgentMetadataError::InternalDispatchUnavailable))
    }
}
...
impl ForkSessionRequest {
    fn settle(&mut self, outcome: Result<Arc<DurableSessionHead>, DurableSessionForkError>) {
        if let Some(response) = self.response.take() {
            let _ = response.send(outcome);
        }
    }

    fn reject(&mut self, error: DurableSessionForkError) {
        self.settle(Err(error));
    }
}

impl Drop for ForkSessionRequest {
    fn drop(&mut self) {
        let error = if self.closing.is_cancelled() {
            DurableSessionForkError::Closing
        } else {
            DurableSessionForkError::InternalDispatchUnavailable
        };
        self.reject(error);
    }
}

struct ForkSessionWaiter {
    response: oneshot::Receiver<Result<Arc<DurableSessionHead>, DurableSessionForkError>>,
}

impl ForkSessionWaiter {
    async fn wait(self) -> Result<Arc<DurableSessionHead>, DurableSessionForkError> {
        self.response
            .await
            .unwrap_or(Err(DurableSessionForkError::InternalDispatchUnavailable))
    }
}

struct SetSessionMetadataRequest {
    attempt: SealedSessionMetadataAttempt,
    response:
        Option<oneshot::Sender<Result<DurableSessionMetadataOutcome, DurableSessionMetadataError>>>,
    closing: CancellationToken,
    permit: Option<SessionMutationPermit>,
}

impl SetSessionMetadataRequest {
    fn settle(
        &mut self,
        outcome: Result<DurableSessionMetadataOutcome, DurableSessionMetadataError>,
    ) {
        if let Some(response) = self.response.take() {
            let _ = response.send(outcome);
        }
    ...
    fn reject(&mut self, error: DurableSessionMetadataError) {
        self.settle(Err(error));
    }
}

impl Drop for SetSessionMetadataRequest {
    fn drop(&mut self) {
        let error = if self.closing.is_cancelled() {
            DurableSessionMetadataError::Closing
        } else {
...
struct SetSessionMetadataWaiter {
    response: oneshot::Receiver<Result<DurableSessionMetadataOutcome, DurableSessionMetadataError>>,
}

impl SetSessionMetadataWaiter {
    async fn wait(self) -> Result<DurableSessionMetadataOutcome, DurableSessionMetadataError> {
        self.response.await.unwrap_or(Err(
            DurableSessionMetadataError::InternalDispatchUnavailable,
        ))
    }
...
/// The private per-Session lifecycle gate used by existing-head Session mutations. It is kept
/// separate from the Agent gate because Session metadata CAS linearizes against Session lifecycle
/// changes, not against Agent publication epochs.
#[derive(Default)]
struct SessionLifecycleGateRegistry {
    gates: Mutex<BTreeMap<SessionId, Arc<tokio::sync::RwLock<()>>>>,
    #[cfg(test)]
    mutation_waiting: Notify,
}

/// Typed exclusive Session mutation epoch. The raw lock is intentionally not exposed outside the
/// registry and the permit may be cloned only for the owner-tracked publication worker.
#[derive(Clone)]
struct SessionMutationPermit {
    _guard: Arc<SessionMutationGuard>,
}
...
impl SessionLifecycleGateRegistry {
    fn gate(&self, session_id: SessionId) -> Arc<tokio::sync::RwLock<()>> {
        let mut gates = lock(&self.gates);
        Arc::clone(
            gates
                .entry(session_id)
    ...
    async fn acquire_mutation(&self, session_id: SessionId) -> SessionMutationPermit {
        #[cfg(test)]
        self.mutation_waiting.notify_one();
        SessionMutationPermit {
            _guard: Arc::new(SessionMutationGuard {
                _guard: self.gate(session_id).write_owned().await,
            }),
        }
    }
    ...
    #[cfg(test)]
    #[allow(dead_code)]
    async fn wait_for_session_mutation_for_test(&self) {
        self.mutation_waiting.notified().await;
    }
}
...
struct DurableStateActor {
    ...
    session_lifecycle_gates: Arc<SessionLifecycleGateRegistry>,
    recovered_agents: Arc<BTreeMap<AgentId, DurableAgentCatalogEntry>>,
    recovered_sessions: Arc<BTreeMap<SessionId, DurableSessionCatalogEntry>>,
...
enum DurableStateActorRequest {
    ...
    SetAgentMetadata(SetAgentMetadataRequest),
    SetAgentDefinition(SetAgentDefinitionRequest),
    CreateSession(CreateSessionRequest),
    SetSessionMetadata(SetSessionMetadataRequest),
...
impl DurableStateActorRequestEnvelope {
    ...
    fn reject_closing(&mut self) {
        ...
        match &mut self.request {
            ...
            DurableStateActorRequest::SetAgentMetadata(request) => {
                request.reject(DurableAgentMetadataError::Closing);
            }
            ...
            DurableStateActorRequest::SetSessionMetadata(request) => {
                request.reject(DurableSessionMetadataError::Closing);
            }
            DurableStateActorRequest::ForkSession(request) => {
                request.reject(DurableSessionForkError::Closing);
            }
            #[cfg(test)]
    ...
    fn reject_unavailable(&mut self) {
        ...
        match &mut self.request {
            ...
            DurableStateActorRequest::SetAgentMetadata(request) => {
                request.reject(DurableAgentMetadataError::InternalDispatchUnavailable);
            }
            DurableStateActorRequest::SetAgentDefinition(request) => {
            ...
            DurableStateActorRequest::SetSessionMetadata(request) => {
                request.reject(DurableSessionMetadataError::InternalDispatchUnavailable);
            }
            DurableStateActorRequest::ForkSession(request) => {
                request.reject(DurableSessionForkError::InternalDispatchUnavailable);
            }
            #[cfg(test)]
            DurableStateActorRequest::Probe(_) => {}
            #[cfg(test)]
    ...
    fn probe_signals(&self) -> Option<(Arc<tokio::sync::Notify>, Arc<tokio::sync::Notify>)> {
        match &self.request {
            ...
            | DurableStateActorRequest::SetAgentMetadata(_)
            | DurableStateActorRequest::SetAgentDefinition(_)
            | DurableStateActorRequest::CreateSession(_)
            | DurableStateActorRequest::SetSessionMetadata(_)
...
impl DurableStateActorHandle {
    ...
    async fn enqueue_agent_metadata(
        &self,
        attempt: SealedAgentMetadataAttempt,
    ) -> SetAgentMetadataWaiter {
        let (response, waiter) = oneshot::channel();
        let request = SetAgentMetadataRequest {
            attempt,
            response: Some(response),
            closing: self.closing.clone(),
            permit: None,
        ...
        let mut envelope = DurableStateActorRequestEnvelope {
            request: DurableStateActorRequest::SetAgentMetadata(request),
            #[cfg(test)]
            response: None,
            #[cfg(test)]
            drop_error: ActorRequestError::Unavailable,
        ...
        let permit = tokio::select! {
            ...
            _ = self.closing.cancelled() => {
                envelope.reject_closing();
                return SetAgentMetadataWaiter { response: waiter };
            }
            permit = self.sender.reserve() => match permit {
                Ok(permit) => permit,
                Err(_) => {
        ...
        permit.send(envelope);
        SetAgentMetadataWaiter { response: waiter }
    }
    ...
    async fn enqueue_session_metadata(
        &self,
        attempt: SealedSessionMetadataAttempt,
    ) -> SetSessionMetadataWaiter {
        let (response, waiter) = oneshot::channel();
        let request = SetSessionMetadataRequest {
            attempt,
            response: Some(response),
            closing: self.closing.clone(),
            permit: None,
        ...
        let mut envelope = DurableStateActorRequestEnvelope {
            request: DurableStateActorRequest::SetSessionMetadata(request),
            #[cfg(test)]
            response: None,
            #[cfg(test)]
            drop_error: ActorRequestError::Unavailable,
        ...
        let permit = tokio::select! {
            ...
            _ = self.closing.cancelled() => {
                envelope.reject_closing();
                return SetSessionMetadataWaiter { response: waiter };
            }
            permit = self.sender.reserve() => match permit {
                Ok(permit) => permit,
                Err(_) => {
        ...
        permit.send(envelope);
        SetSessionMetadataWaiter { response: waiter }
    }
    ...
    #[cfg(test)]
    async fn enqueue_session_fork_with_test_candidates(
        &self,
        attempt: SealedSessionForkAttempt,
        candidates: impl IntoIterator<Item = Result<SessionId, ()>>,
        candidate_calls: Arc<std::sync::atomic::AtomicUsize>,
    ...
    async fn enqueue_session_fork_request(
        &self,
        mut fork: ForkSessionRequest,
    ) -> ForkSessionWaiter {
        let (response, waiter) = oneshot::channel();
        fork.response = Some(response);
        let mut envelope = DurableStateActorRequestEnvelope {
            request: DurableStateActorRequest::ForkSession(fork),
            #[cfg(test)]
            response: None,
            #[cfg(test)]
            drop_error: ActorRequestError::Unavailable,
        ...
        let permit = tokio::select! {
            ...
            _ = self.closing.cancelled() => {
                envelope.reject_closing();
                return ForkSessionWaiter { response: waiter };
            }
            permit = self.sender.reserve() => match permit {
                Ok(permit) => permit,
                Err(_) => {
        ...
        // The permit is the exact request-admission linearization point. From here the actor or
        // its drain path owns settlement, including a permanently reserved child identity.
        permit.send(envelope);
        ForkSessionWaiter { response: waiter }
    }
...
impl DurableStateActor {
                ...
                request = self.receiver.recv() => match request {
                    ...
                    Some(mut request) => {
                        ...
                        match &mut request.request {
                            ...
                            DurableStateActorRequest::SetAgentMetadata(metadata) => {
                                match self.process_agent_metadata(metadata).await {
                                    AgentMetadataMutationWorkResult::NoChange(head) => {
                                        metadata.settle(Ok(DurableAgentMetadataOutcome::NoChange(
                                            head,
                                        )));
                                        metadata.permit.take();
                                    }
                                    AgentMetadataMutationWorkResult::Published {
                                        entry,
                                        close_required,
                                    } => {
                                        let head = Arc::clone(&entry.current_head);
                                        lock(&self.published_agents)
                                            .insert(head.agent_id(), entry);
                                        metadata.settle(Ok(
                                            DurableAgentMetadataOutcome::Updated(head),
                                        ));
                                        if close_required {
                                            self.task_context.request_closing();
                                            self.closing.cancel();
                                    ...
                                    AgentMetadataMutationWorkResult::Ordinary {
                                        error,
                                        close_required,
                                    } => {
                                        metadata.settle(Err(error));
                                        if close_required {
                                            self.task_context.request_closing();
                                            self.closing.cancel();
                                            metadata.permit.take();
                                    ...
                                    AgentMetadataMutationWorkResult::Poisoned => {
                                        metadata.settle(Err(
                                            DurableAgentMetadataError::InternalDispatchUnavailable,
                                        ));
                                        self.task_context.request_closing();
                            ...
                            DurableStateActorRequest::SetSessionMetadata(metadata) => {
                                match self.process_session_metadata(metadata).await {
                                    SessionMetadataMutationWorkResult::NoChange(head) => {
                                        metadata.settle(Ok(DurableSessionMetadataOutcome::NoChange(
                                            head,
                                        )));
                                        metadata.permit.take();
                                    }
                                    SessionMetadataMutationWorkResult::Published {
                                        entry,
                                        close_required,
                                    } => {
                                        let head = Arc::clone(&entry.current_head);
                                        lock(&self.published_sessions)
                                            .insert(head.session_id(), entry);
                                        metadata.settle(Ok(
                                            DurableSessionMetadataOutcome::Updated(head),
                                        ));
                                        if close_required {
                                            self.task_context.request_closing();
                                            self.closing.cancel();
                                    ...
                                    SessionMetadataMutationWorkResult::Ordinary {
                                        error,
                                        close_required,
                                    } => {
                                        metadata.settle(Err(error));
                                    ...
                                    SessionMetadataMutationWorkResult::Poisoned => {
                                        metadata.settle(Err(
                                            DurableSessionMetadataError::InternalDispatchUnavailable,
                                        ));
                                        self.task_context.request_closing();
                            ...
                            DurableStateActorRequest::ForkSession(fork) => {
                                match self.process_fork_session(fork).await {
                                    SessionForkPublicationWorkResult::Published {
                                        entry,
                                        close_required,
                                    } => {
                                        let head = Arc::clone(&entry.current_head);
                                    ...
                                    SessionForkPublicationWorkResult::Poisoned => {
                                        fork.settle(Err(
                                            DurableSessionForkError::InternalDispatchUnavailable,
                                        ));
                                        self.poison_session_create_admission().await;
    ...
    async fn process_agent_metadata(
        &mut self,
        request: &mut SetAgentMetadataRequest,
    ) -> AgentMetadataMutationWorkResult {
        let agent_id = request.attempt.agent_id();
        let exists = lock(&self.published_agents).contains_key(&agent_id)
            || self.recovered_agents.contains_key(&agent_id);
        if !exists {
            return AgentMetadataMutationWorkResult::Ordinary {
                error: DurableAgentMetadataError::AgentNotFound,
                close_required: false,
            };
        }
        let mutation_permit = tokio::select! {
            ...
            _ = self.closing.cancelled() => {
                return AgentMetadataMutationWorkResult::Ordinary {
                    error: DurableAgentMetadataError::Closing,
                    close_required: false,
                };
            }
            permit = self.agent_lifecycle_gates.acquire_mutation(agent_id) => permit,
        ...
        let Some(current) = current else {
            return AgentMetadataMutationWorkResult::Poisoned;
        };
        let decision = match request.attempt.decide(
            current.current_head.status(),
            current.current_head.metadata(),
        ) {
            Ok(decision) => decision,
            Err(AgentMetadataDecisionError::StaleRevision) => {
                return AgentMetadataMutationWorkResult::Ordinary {
                    error: DurableAgentMetadataError::StaleRevision,
                    close_required: false,
                };
            }
            Err(AgentMetadataDecisionError::AgentDeleted) => {
                return AgentMetadataMutationWorkResult::Ordinary {
                    error: DurableAgentMetadataError::AgentDeleted,
                    close_required: false,
                };
            }
            Err(AgentMetadataDecisionError::RevisionExhausted) => {
                return AgentMetadataMutationWorkResult::Ordinary {
                    error: DurableAgentMetadataError::DurableStateTooLarge,
                    close_required: false,
                };
            }
        };
        let AgentMetadataDecision::Publish(next_metadata) = decision else {
            return AgentMetadataMutationWorkResult::NoChange(Arc::clone(&current.current_head));
        };
        let next_generation_value = match current
            .current_head
            .storage_generation()
        ...
        let head = match DurableAgentHead::new(
            ...
            Ok(head) => Arc::new(head),
            Err(_) => return AgentMetadataMutationWorkResult::Poisoned,
        };
        ...
        map_agent_metadata_existing_generation_result(
            publish_agent_existing_generation(
                self.task_context.clone(),
                Arc::clone(&self.lease),
                Arc::clone(&self.filesystem),
                ...
                request
                    .permit
                    .as_ref()
                    .expect("the metadata request retains its exclusive permit")
                    .clone(),
                self.closing.clone(),
                Arc::clone(&self.active_commit_barrier),
            )
    ...
    async fn process_session_metadata(
        &mut self,
        request: &mut SetSessionMetadataRequest,
    ) -> SessionMetadataMutationWorkResult {
        let session_id = request.attempt.session_id();
        let exists = lock(&self.published_sessions).contains_key(&session_id)
            || self.recovered_sessions.contains_key(&session_id);
        if !exists {
            return SessionMetadataMutationWorkResult::Ordinary {
                error: DurableSessionMetadataError::SessionNotFound,
                close_required: false,
            };
        }
        let mutation_permit = tokio::select! {
            ...
            _ = self.closing.cancelled() => {
                return SessionMetadataMutationWorkResult::Ordinary {
                    error: DurableSessionMetadataError::Closing,
                    close_required: false,
                };
            }
            permit = self.session_lifecycle_gates.acquire_mutation(session_id) => permit,
        };
        request.permit = Some(mutation_permit);
        let current = {
            let published = lock(&self.published_sessions);
            published
                .get(&session_id)
                .cloned()
                .or_else(|| self.recovered_sessions.get(&session_id).cloned())
        };
        let Some(current) = current else {
            return SessionMetadataMutationWorkResult::Poisoned;
        };
        let decision = match request.attempt.decide(
            current.current_head.lifecycle(),
            current.current_head.metadata(),
        ) {
            Ok(decision) => decision,
            Err(SessionMetadataDecisionError::StaleRevision) => {
                return SessionMetadataMutationWorkResult::Ordinary {
                    error: DurableSessionMetadataError::StaleRevision,
                    close_required: false,
                };
            }
            Err(SessionMetadataDecisionError::SessionDeleted) => {
                return SessionMetadataMutationWorkResult::Ordinary {
                    error: DurableSessionMetadataError::SessionDeleted,
                    close_required: false,
                };
            }
            Err(SessionMetadataDecisionError::RevisionExhausted) => {
                return SessionMetadataMutationWorkResult::Ordinary {
                    error: DurableSessionMetadataError::DurableStateTooLarge,
                    close_required: false,
                };
            }
        };
        let SessionMetadataDecision::Publish(next_metadata) = decision else {
            return SessionMetadataMutationWorkResult::NoChange(Arc::clone(&current.current_head));
        };
        let next_generation_value = match current
            .current_head
            .storage_generation()
        ...
        let head = match DurableSessionHead::new(
            ...
            next_metadata,
            current.current_head.lifecycle(),
            current.current_head.fork_provenance().cloned(),
            current.current_head.created_at(),
        ) {
            Ok(head) => Arc::new(head),
            Err(_) => return SessionMetadataMutationWorkResult::Poisoned,
        };
        let pinned_agent = current.current_definition.agent();
        let agent_id = pinned_agent.agent_id();
        let agent = {
            let published = lock(&self.published_agents);
            published
                .get(&agent_id)
                .or_else(|| self.recovered_agents.get(&agent_id))
                .and_then(|entry| session_agent_basis(entry, pinned_agent))
        };
        let Some(agent) = agent else {
            return SessionMetadataMutationWorkResult::Poisoned;
        };
        let agents = BTreeMap::from([(agent_id, agent)]);
        if validate_session_generation_transition(
            session_id,
            next_generation,
            &current.current_head,
            &current.current_definition,
        ...
        map_session_metadata_existing_generation_result(
            publish_session_existing_generation(
                self.task_context.clone(),
                Arc::clone(&self.lease),
                Arc::clone(&self.filesystem),
                self.root.clone(),
                ...
                request
                    .permit
                    .as_ref()
                    .expect("the Session metadata request retains its exclusive permit")
                    .clone(),
                self.closing.clone(),
                Arc::clone(&self.active_commit_barrier),
            )
    ...
    async fn process_fork_session(
        &mut self,
        request: &mut ForkSessionRequest,
    ) -> SessionForkPublicationWorkResult {
        let child_session_id = match self.reserve_session_for_fork(request).await {
            Ok(session_id) => session_id,
        ...
        let source_session_id = request.attempt.source_session_id();
        let source = {
            let published = lock(&self.published_sessions);
            published
                .get(&source_session_id)
                .cloned()
                .or_else(|| self.recovered_sessions.get(&source_session_id).cloned())
        };
        let Some(source) = source else {
            return SessionForkPublicationWorkResult::Ordinary {
                error: DurableSessionForkError::SourceNotFound,
                close_required: false,
            };
        };
        if source.current_head.lifecycle() == SessionLifecycle::Deleted {
            return SessionForkPublicationWorkResult::Ordinary {
                error: DurableSessionForkError::SourceDeleted,
                close_required: false,
            };
        }
        ...
        let captured_agent = source.current_definition.agent();
        let agent_id = captured_agent.agent_id();
        let agent_exists = lock(&self.published_agents).contains_key(&agent_id)
            || self.recovered_agents.contains_key(&agent_id);
        if !agent_exists {
            return SessionForkPublicationWorkResult::Poisoned;
        }
        let agent_gate = tokio::select! {
            ...
            permit = self.agent_lifecycle_gates.acquire_publication(agent_id) => permit,
        };
        let agent = {
            let published = lock(&self.published_agents);
            published
                .get(&agent_id)
                .or_else(|| self.recovered_agents.get(&agent_id))
                .and_then(|entry| session_agent_basis(entry, captured_agent))
        };
        let Some(agent) = agent else {
            return SessionForkPublicationWorkResult::Poisoned;
        };
        match agent.current_head.status() {
            ...
            AgentStatus::Disabled => {
                return SessionForkPublicationWorkResult::Ordinary {
                    error: DurableSessionForkError::AgentDisabled,
                    close_required: false,
                };
            }
            AgentStatus::Deleted => {
                return SessionForkPublicationWorkResult::Ordinary {
                    error: DurableSessionForkError::AgentDeleted,
                    close_required: false,
                };
            }
        }

        let (definition, metadata, provenance) = match request
            .attempt
            .materialize(child_session_id, &source.current_definition)
        {
            Ok(parts) => parts,
        ...
        let generation = match StorageGeneration::new(1) {
            Some(generation) => generation,
            None => return SessionForkPublicationWorkResult::Poisoned,
        };
        let head = match DurableSessionHead::new(
            child_session_id,
            generation,
        ...
        let agents = BTreeMap::from([(agent_id, agent)]);
        let sessions = BTreeMap::from([(source_session_id, source)]);
        if validate_session_generation_one(
            child_session_id,
            generation,
            &head,
            &definition,
        ...
        .is_err()
            || definition.agent() != captured_agent
        {
            return SessionForkPublicationWorkResult::Poisoned;
        }
        ...
        map_session_publication_for_fork(
            publish_session_generation(
                self.task_context.clone(),
                Arc::clone(&self.lease),
                Arc::clone(&self.filesystem),
    ...
    async fn reserve_session_for_fork(
        &mut self,
        request: &mut ForkSessionRequest,
    ) -> Result<SessionId, DurableSessionForkError> {
        if self.recovered_session_reservations.len() + self.new_session_reservations.len()
            >= SESSION_RESERVATION_ENTRY_CAP
        {
            return Err(DurableSessionForkError::DurableStateTooLarge);
        }
        let mut collisions = 0;
        loop {
            if self.closing.is_cancelled() {
                return Err(DurableSessionForkError::Closing);
            }
            let candidate = {
                #[cfg(test)]
                {
                    if let Some(candidates) = request.candidates.as_mut() {
                        ...
                        candidates
                            .pop_front()
                            .unwrap_or(Err(()))
                            .map_err(|_| DurableSessionForkError::IdentityUnavailable)?
                    } else {
                        request
                            .attempt
                            .generate_candidate()
            ...
            if candidate == request.attempt.source_session_id() {
                return Err(DurableSessionForkError::SourceIdentityConflict);
            }
            let was_present = self.recovered_session_reservations.contains(&candidate)
                || self.new_session_reservations.contains(&candidate);
            let observation = self
                ...
                .await
                .map_err(|error| match error {
                    CreateReservationAttemptError::Closing => DurableSessionForkError::Closing,
                    CreateReservationAttemptError::StorageUnavailable => {
                        DurableSessionForkError::StorageUnavailable
                    }
                    CreateReservationAttemptError::Poisoned => {
                        DurableSessionForkError::InternalDispatchUnavailable
                    }
            ...
            match observation {
                ...
                ReservationPhysicalObservation::AlreadyExistsCanonical if was_present => {
                    collisions += 1;
                    if collisions == RESERVATION_COLLISION_ATTEMPT_CAP {
                        return Err(DurableSessionForkError::CollisionAttemptsExhausted);
                    }
                }
                ReservationPhysicalObservation::CreateErrorAbsent if !was_present => {
                    return Err(DurableSessionForkError::StorageUnavailable);
                }
                ReservationPhysicalObservation::CreateErrorPresent if was_present => {
                    return Err(DurableSessionForkError::StorageUnavailable);
                }
    ...
    async fn run_create_reservation_attempt(
        ...
        let job = self.task_context.spawn_blocking_tracked(move || {
            let _lease = lease;
            match attempt_local_reservation(filesystem.as_ref(), &path, directory_sync, &job_stage)
            {
...
#[derive(Clone)]
enum SessionExistingGenerationWorkResult {
    Published {
        entry: DurableSessionCatalogEntry,
        close_required: bool,
    },
    Ordinary {
        error: SessionExistingGenerationError,
        close_required: bool,
    },
    Poisoned,
}
...
enum AgentMetadataMutationWorkResult {
    NoChange(Arc<DurableAgentHead>),
    Published {
        entry: DurableAgentCatalogEntry,
        close_required: bool,
...
#[derive(Clone, Copy)]
enum SessionExistingGenerationError {
    Closing,
    DurableStateTooLarge,
    StorageUnavailable,
}

enum SessionMetadataMutationWorkResult {
    NoChange(Arc<DurableSessionHead>),
    Published {
        entry: DurableSessionCatalogEntry,
        close_required: bool,
    ...
    Ordinary {
        error: DurableSessionMetadataError,
        close_required: bool,
    },
    Poisoned,
}
...
#[derive(Clone)]
enum SessionForkPublicationWorkResult {
    Published {
        entry: DurableSessionCatalogEntry,
        close_required: bool,
    },
...
async fn validate_recorded_genesis_fork_source(
    ...
    tokio::select! {
        biased;
        _ = closing.cancelled() => {
            match job_wait.await {
                Ok(_) => Err(DurableSessionForkError::Closing),
                Err(error) => Err(map_fork_source_task_error(error)),
            }
        }
...
fn validate_recorded_genesis_fork_source_blocking(
    ...
    let current_generation = source.current_head.storage_generation();
    let current_generation_path = source_entity
        .join("generations")
        .join(current_generation.directory_name());
    let current_head_bytes = DurableStoreV1Codec::encode_session_head(&source.current_head)
        .map_err(|_| DurableSessionForkError::InternalDispatchUnavailable)?;
    let current_definition_bytes =
        DurableStoreV1Codec::encode_session_definition(&source.current_definition)
            .map_err(|_| DurableSessionForkError::InternalDispatchUnavailable)?;
    let current_definition_path = source_entity
        .join("generations")
        .join(
            source
                .current_head
    ...
    for (path, expected) in [
        (source_entity.join("PUBLISHED"), b"".as_slice()),
        (current_generation_path.join("COMMITTED"), b"".as_slice()),
        (
            current_generation_path.join("head.json"),
            current_head_bytes.as_slice(),
        ),
        (current_definition_path, current_definition_bytes.as_slice()),
    ] {
        if exact_readback_state(filesystem, &path, expected) != PublicationCertainty::Published {
            return Err(DurableSessionForkError::InternalDispatchUnavailable);
        }
    }
    let initial_definition_path =
        source_entity.join("generations/00000000000000000001/definition.json");
    let initial_definition_bytes = filesystem
        .read_document(&initial_definition_path)
        .map_err(map_recorded_fork_source_definition_error)?;
    let initial_definition =
        DurableStoreV1Codec::decode_session_definition(&initial_definition_bytes)
            .map_err(map_durable_document_codec_error)
            .map_err(map_recorded_fork_source_definition_error)?;
    if initial_definition.session_id() != source_session_id
        || initial_definition.revision().get() != 1
        || initial_definition.agent().agent_id() != source.current_definition.agent().agent_id()
        || initial_definition.created_at() != source.current_head.created_at()
    {
        return Err(DurableSessionForkError::InternalDispatchUnavailable);
    }
    let expected_header = SessionHeader::reconstruct(
        1,
        source_session_id,
        source.current_head.created_at(),
        initial_definition.agent(),
        initial_definition.revision(),
    );
    let conversation_path = source_entity.join("conversation.jsonl");
    let initial_path_metadata =
        metadata_without_following(&conversation_path).map_err(map_recorded_fork_open_error)?;
    ...
    if initial_path_metadata.len() > MAX_CONVERSATION_FILE_BYTES {
        return Err(DurableSessionForkError::SourceConversationTooLarge);
    }
    let file = filesystem
        .open_file(&conversation_path)
        .map_err(|_| DurableSessionForkError::StorageUnavailable)?;
    let opened_handle_metadata = file
        .metadata()
        .map_err(|_| DurableSessionForkError::StorageUnavailable)?;
    ...
    validate_open_file_observation(
        &initial_path_metadata,
        &opened_handle_metadata,
        &opened_path_metadata,
    )
    .map_err(map_recorded_fork_open_error)?;
    let declared_file_bytes = initial_path_metadata.len();
    let mut lease = RecordedForkConversationLease::from_durable_state(
        source_session_id,
        declared_file_bytes,
        file,
    );
    if lease.source_session_id() != source_session_id
...
const fn map_recorded_fork_open_error(error: DurableOpenError) -> DurableSessionForkError {
    match error {
        ...
        | DurableOpenError::StoreInUse => DurableSessionForkError::SourceConversationCorrupt,
        DurableOpenError::StorageUnavailable => DurableSessionForkError::StorageUnavailable,
    }
}

const fn map_recorded_fork_source_definition_error(
    error: DurableOpenError,
) -> DurableSessionForkError {
    let _ = error;
    DurableSessionForkError::InternalDispatchUnavailable
}

const fn map_fork_source_task_error(error: RuntimeTaskError) -> DurableSessionForkError {
    match error {
        RuntimeTaskError::OwnerClosing => DurableSessionForkError::Closing,
        RuntimeTaskError::WorkerUnavailable => DurableSessionForkError::StorageUnavailable,
        RuntimeTaskError::OperationPanicked => DurableSessionForkError::InternalDispatchUnavailable,
    }
}

fn map_session_publication_for_fork(
    ...
    match result {
        ...
        SessionPublicationWorkResult::Ordinary {
            error: DurableSessionCreateError::StorageUnavailable,
            close_required,
        } => SessionForkPublicationWorkResult::Ordinary {
            error: DurableSessionForkError::StorageUnavailable,
            close_required,
        ...
        | SessionPublicationWorkResult::Poisoned => SessionForkPublicationWorkResult::Poisoned,
        SessionPublicationWorkResult::Ordinary { .. } => SessionForkPublicationWorkResult::Poisoned,
    }
}
...
#[allow(
    clippy::too_many_arguments,
    reason = "the Session existing-generation publisher binds one exact old/new head and Agent basis"
)]
async fn publish_session_existing_generation(
    task_context: RuntimeTaskContext,
    lease: Arc<RootLease>,
    filesystem: Arc<dyn DurableFilesystem>,
    root: PathBuf,
    directory_sync: DirectorySync,
    current: DurableSessionCatalogEntry,
    head: Arc<DurableSessionHead>,
    agents: Arc<BTreeMap<AgentId, DurableAgentCatalogEntry>>,
    mutation_permit: SessionMutationPermit,
    closing: CancellationToken,
    active_barrier: Arc<Mutex<Option<Arc<DurableCommitBarrier>>>>,
) -> SessionExistingGenerationWorkResult {
    let barrier = DurableCommitBarrier::new();
    *lock(&active_barrier) = Some(Arc::clone(&barrier));
    let worker_barrier = Arc::clone(&barrier);
    // Keep the Session gate and the root lease alive in the owner-tracked worker even if the
    // actor is aborted after COMMITTED and the caller later joins the worker.
    let worker_mutation_permit = mutation_permit.clone();
    let job = task_context.spawn_blocking_tracked(move || {
        let _lease = lease;
        let _worker_mutation_permit = worker_mutation_permit;
        publish_session_existing_generation_blocking(
            filesystem.as_ref(),
            &root,
            directory_sync,
            current,
    ...
    let result = tokio::select! {
        ...
        _ = closing.cancelled() => {
            ...
            let result = job_wait.await;
            match result {
                Ok(result) if cancellation_won => {
                    force_session_existing_generation_closing(result)
                }
                Ok(result) => force_session_existing_generation_close(result),
                Err(error) if cancellation_won => force_session_existing_generation_closing(
                    classify_session_existing_generation_task_error(error, &barrier),
                ),
                Err(error) => force_session_existing_generation_close(
                    classify_session_existing_generation_task_error(error, &barrier),
                ),
            }
        }
        result = &mut job_wait => match result {
            Ok(result) => result,
            Err(error) => classify_session_existing_generation_task_error(error, &barrier),
        },
    };
    *lock(&active_barrier) = None;
    // The request-owned permit remains in the concrete request until the actor installs the
...
fn classify_session_existing_generation_task_error(
    error: RuntimeTaskError,
    barrier: &DurableCommitBarrier,
) -> SessionExistingGenerationWorkResult {
    match barrier.current_stage() {
        DurableCommitStage::Crossed => SessionExistingGenerationWorkResult::Poisoned,
        DurableCommitStage::Pending | DurableCommitStage::Cancelled => match error {
            RuntimeTaskError::OwnerClosing => SessionExistingGenerationWorkResult::Ordinary {
                error: SessionExistingGenerationError::Closing,
                close_required: false,
            },
            RuntimeTaskError::WorkerUnavailable => SessionExistingGenerationWorkResult::Ordinary {
                error: SessionExistingGenerationError::StorageUnavailable,
                close_required: false,
            },
            RuntimeTaskError::OperationPanicked => SessionExistingGenerationWorkResult::Poisoned,
        },
    }
}

fn force_session_existing_generation_closing(
    result: SessionExistingGenerationWorkResult,
) -> SessionExistingGenerationWorkResult {
    match result {
        SessionExistingGenerationWorkResult::Published { entry, .. } => {
            SessionExistingGenerationWorkResult::Published {
                entry,
                close_required: true,
            }
        }
        SessionExistingGenerationWorkResult::Ordinary { .. } => {
            SessionExistingGenerationWorkResult::Ordinary {
                error: SessionExistingGenerationError::Closing,
                close_required: false,
            }
        }
        SessionExistingGenerationWorkResult::Poisoned => {
            SessionExistingGenerationWorkResult::Poisoned
        }
    }
}

fn force_session_existing_generation_close(
    result: SessionExistingGenerationWorkResult,
) -> SessionExistingGenerationWorkResult {
    match result {
        SessionExistingGenerationWorkResult::Published { entry, .. } => {
        ...
        SessionExistingGenerationWorkResult::Ordinary { error, .. } => {
            SessionExistingGenerationWorkResult::Ordinary {
                error,
                close_required: true,
            }
...
fn map_agent_metadata_existing_generation_result(
    result: AgentExistingGenerationWorkResult,
) -> AgentMetadataMutationWorkResult {
    match result {
        AgentExistingGenerationWorkResult::Published {
        ...
        AgentExistingGenerationWorkResult::Ordinary {
            ...
            error: match error {
                AgentExistingGenerationError::Closing => DurableAgentMetadataError::Closing,
                AgentExistingGenerationError::DurableStateTooLarge => {
                    DurableAgentMetadataError::DurableStateTooLarge
                }
                AgentExistingGenerationError::StorageUnavailable => {
                    DurableAgentMetadataError::StorageUnavailable
                }
            },
            close_required,
        },
        AgentExistingGenerationWorkResult::Poisoned => AgentMetadataMutationWorkResult::Poisoned,
    }
}
...
fn map_session_metadata_existing_generation_result(
    result: SessionExistingGenerationWorkResult,
) -> SessionMetadataMutationWorkResult {
    match result {
        SessionExistingGenerationWorkResult::Published {
        ...
        SessionExistingGenerationWorkResult::Ordinary {
            ...
            error: match error {
                SessionExistingGenerationError::Closing => DurableSessionMetadataError::Closing,
                SessionExistingGenerationError::DurableStateTooLarge => {
                    DurableSessionMetadataError::DurableStateTooLarge
                }
                SessionExistingGenerationError::StorageUnavailable => {
...
fn publish_agent_existing_generation_blocking(
    ...
    precommit_step!(exact_optional_definition_readback_result(
        filesystem,
        &definition_path,
        definition_bytes.as_deref(),
        OptionalDefinitionObservationStage::Staged,
    ));
        ...
        let readback_failed = !operation_failed
            && (exact_readback_state(filesystem, &committed_path, b"")
                ...
                || exact_readback_state(filesystem, &head_path, &head_bytes)
                    != PublicationCertainty::Published
                || exact_optional_definition_readback(
                    filesystem,
                    &definition_path,
                    definition_bytes.as_deref(),
                    OptionalDefinitionObservationStage::Committed,
                ) != PublicationCertainty::Published);
...
#[allow(
    clippy::too_many_arguments,
    reason = "the Session existing-generation worker binds the fixed physical paths and exact facts"
)]
fn publish_session_existing_generation_blocking(
    filesystem: &dyn DurableFilesystem,
    root: &Path,
    directory_sync: DirectorySync,
    current: DurableSessionCatalogEntry,
    ...
    #[cfg(test)]
    {
        ...
        assert_eq!(
            agents.len(),
            1,
            "Session metadata carries exactly one Agent basis into publication"
        );
        let basis = agents
            .get(&pinned_agent.agent_id())
            .expect("the Session metadata Agent basis matches the pinned Agent");
        assert!(
            basis
                .definition_index
                .contains_key(&pinned_agent.revision()),
            "the Session metadata Agent basis contains the pinned revision"
        );
    }
    ...
    .is_err()
    {
        return SessionExistingGenerationWorkResult::Poisoned;
    }
    let head_bytes = match DurableStoreV1Codec::encode_session_head(&head) {
        Ok(bytes) => bytes,
        Err(DurableStoreCodecError::DocumentTooLarge | DurableStoreCodecError::JsonStructure) => {
            return SessionExistingGenerationWorkResult::Ordinary {
                error: SessionExistingGenerationError::DurableStateTooLarge,
                close_required: false,
            };
        }
        Err(_) => return SessionExistingGenerationWorkResult::Poisoned,
    };
    macro_rules! precommit_step {
        ($operation:expr) => {{
            if barrier.is_cancelled() {
                return reconcile_session_existing_generation_precommit_failure(
                    filesystem,
                    root,
                    directory_sync,
                    &current,
                    generation,
                    agents,
                    SessionExistingGenerationError::Closing,
                    &barrier,
                );
            }
            if $operation.is_err() {
    ...
    let committed_file = match filesystem.create_new_private_file(&committed_path) {
        Ok(file) => Some(file),
        Err(()) => match reconcile_session_existing_generation_committed_failure(
            filesystem,
            root,
            directory_sync,
            &current,
            ...
            SessionExistingCommittedResolution::Continue => {
                close_required = true;
                None
            }
            SessionExistingCommittedResolution::Return(result) => return result,
        },
    };
    if let Some(committed_file) = committed_file {
        let operation_failed = validate_new_file_mode(&committed_file).is_err()
        ...
        if operation_failed || readback_failed {
            match reconcile_session_existing_generation_committed_failure(
                filesystem,
                root,
                directory_sync,
                &current,
                ...
                SessionExistingCommittedResolution::Continue => close_required = true,
                SessionExistingCommittedResolution::Return(result) => return result,
            }
        }
    }

    match recover_published_session_existing_generation(
        filesystem,
        &entity,
        &current,
        &head,
        ...
        Ok(entry) => SessionExistingGenerationWorkResult::Published {
            entry,
            close_required,
        },
        Err(_) => SessionExistingGenerationWorkResult::Poisoned,
    }
}

#[allow(
    clippy::too_many_arguments,
    reason = "the Session existing-generation precommit reconciliation binds the old Session entry, Agent basis, and exact cleanup policy"
)]
fn reconcile_session_existing_generation_precommit_failure(
    filesystem: &dyn DurableFilesystem,
    root: &Path,
    directory_sync: DirectorySync,
    current: &DurableSessionCatalogEntry,
    ...
    match cleanup_operation_owned_session_trailing(
        filesystem,
        root,
        directory_sync,
        current,
        ...
        Ok(()) => SessionExistingGenerationWorkResult::Ordinary {
            error,
            close_required: false,
        },
        Err(StagingCleanupError::CloseRequired) => SessionExistingGenerationWorkResult::Ordinary {
            error,
            close_required: true,
        },
        Err(StagingCleanupError::Poisoned) => SessionExistingGenerationWorkResult::Poisoned,
    }
}

enum SessionExistingCommittedResolution {
    Continue,
    Return(SessionExistingGenerationWorkResult),
}

#[allow(
    clippy::too_many_arguments,
    reason = "the Session committed reconciliation binds one exact old/new generation and Agent basis"
)]
fn reconcile_session_existing_generation_committed_failure(
    filesystem: &dyn DurableFilesystem,
    root: &Path,
    directory_sync: DirectorySync,
    current: &DurableSessionCatalogEntry,
    ...
    agents: &BTreeMap<AgentId, DurableAgentCatalogEntry>,
) -> SessionExistingCommittedResolution {
    let generation_path = definition_path
        .parent()
        .expect("Session definition has its generation directory parent");
    let committed_path = generation_path.join("COMMITTED");
    match exact_readback_state(filesystem, &committed_path, b"") {
        PublicationCertainty::Published => {
            ...
            if !payloads_valid {
                return SessionExistingCommittedResolution::Return(
                    SessionExistingGenerationWorkResult::Poisoned,
                );
            }
        ...
        PublicationCertainty::NotPublished => {
            let result = match cleanup_operation_owned_session_trailing(
                filesystem,
                root,
                directory_sync,
                current,
            ...
            SessionExistingCommittedResolution::Return(result)
        }
        PublicationCertainty::CommittedCorruptPoisoned
        | PublicationCertainty::IndeterminatePoisoned => {
            SessionExistingCommittedResolution::Return(
                SessionExistingGenerationWorkResult::Poisoned,
            )
        }
    }
...
#[allow(
    clippy::too_many_arguments,
    reason = "the Session trailing-generation cleanup binds the old Session entry, Agent basis, and exact operation subset"
)]
fn cleanup_operation_owned_session_trailing(
    filesystem: &dyn DurableFilesystem,
    root: &Path,
    directory_sync: DirectorySync,
    current: &DurableSessionCatalogEntry,
    ...
    let entity = root
        .join(SESSIONS_DIRECTORY)
        .join(current.current_head.session_id().to_string());
    let generation_path = entity.join("generations").join(generation.directory_name());
    match fs::symlink_metadata(&generation_path) {
        Err(error) if error.kind() == io::ErrorKind::NotFound => return Ok(()),
        Ok(_) => {}
    ...
    let scanned = scan_session_entity(&entity, GENERATION_ENTRY_CAP)
        .map_err(|_| StagingCleanupError::Poisoned)?;
    let ScannedSessionEntity::Published(scan) = scanned else {
        return Err(StagingCleanupError::Poisoned);
    };
    let recovered =
        recover_session_generation_chain(scan, current.current_head.session_id(), agents)
            .map_err(|_| StagingCleanupError::Poisoned)?;
    if recovered.entry.current_head != current.current_head
        || recovered.entry.current_definition != current.current_definition
    {
    ...
    let cleanup =
        deferred_session_generation_cleanup(root, current.current_head.session_id(), markerless);
    finalize_deferred_generation_cleanup(&cleanup, directory_sync, filesystem).map_err(|error| {
        match error {
            DurableOpenError::StorageUnavailable => StagingCleanupError::CloseRequired,
            _ => StagingCleanupError::Poisoned,
...
fn recover_published_agent_existing_generation(
    ...
    let definition_state = exact_optional_definition_readback(
        filesystem,
        &generation_path.join("definition.json"),
        definition_bytes,
        OptionalDefinitionObservationStage::Final,
    );
...
fn recover_published_session(
    ...
    match recovered.entry.current_head.fork_provenance() {
        None => {
            let expected_agent = AgentRevisionRef::new(
                pinned_agent.agent_id(),
                agent.current_head.current_definition_revision(),
            );
            if pinned_agent != expected_agent {
                return Err(PublicationCertainty::CommittedCorruptPoisoned);
            }
        }
        Some(provenance)
            if provenance.source_session_id() != session_id
                && sessions.contains_key(&provenance.source_session_id()) => {}
        Some(_) => return Err(PublicationCertainty::CommittedCorruptPoisoned),
    }
...
#[allow(
    clippy::too_many_arguments,
    reason = "existing Session readback binds the fixed entity, exact next generation, and Agent basis"
)]
fn recover_published_session_existing_generation(
    filesystem: &dyn DurableFilesystem,
    entity: &Path,
    current: &DurableSessionCatalogEntry,
    expected_head: &DurableSessionHead,
    head_bytes: &[u8],
    agents: &BTreeMap<AgentId, DurableAgentCatalogEntry>,
) -> Result<DurableSessionCatalogEntry, PublicationCertainty> {
    let published_state = exact_readback_state(filesystem, &entity.join("PUBLISHED"), b"");
    ...
    if recovered.entry.current_definition.as_ref() != current.current_definition.as_ref() {
        return Err(PublicationCertainty::CommittedCorruptPoisoned);
    }
    let pinned_agent = recovered.entry.current_definition.agent();
    let Some(agent) = agents.get(&pinned_agent.agent_id()) else {
        return Err(PublicationCertainty::CommittedCorruptPoisoned);
    };
    if !agent
        .definition_index
    ...
    Ok(DurableSessionCatalogEntry {
        current_head: recovered.entry.current_head,
        current_definition: Arc::clone(&current.current_definition),
    })
}
...
impl DurableState {
    ...
    async fn open_with_filesystem(
        ...
        let session_lifecycle_gates = Arc::new(SessionLifecycleGateRegistry::default());
        let actor = match DurableStateActorHandle::start(
            ...
            *directory_sync,
            ...
            Arc::clone(&session_lifecycle_gates),
            Arc::clone(agents),
            Arc::clone(sessions),
    ...
    pub(crate) async fn update_agent_metadata(
        &self,
        attempt: SealedAgentMetadataAttempt,
    ) -> Result<DurableAgentMetadataOutcome, DurableAgentMetadataError> {
        self.actor
            .enqueue_agent_metadata(attempt)
            .await
            .wait()
            .await
    }
    ...
    pub(crate) async fn update_session_metadata(
        &self,
        attempt: SealedSessionMetadataAttempt,
    ) -> Result<DurableSessionMetadataOutcome, DurableSessionMetadataError> {
        self.actor
            .enqueue_session_metadata(attempt)
            .await
            .wait()
            .await
    }
    ...
    #[allow(
        dead_code,
        reason = "the production Session command surface consumes this durable Fork seam"
    )]
    /// Publishes the current unloaded-source RecordedHistory + Genesis tracer.
    ///
    /// The future Runtime command caller must linearize source residency and invoke this seam only
    /// with an unloaded source; loaded sources require `LiveSnapshot` and are not accepted through
    /// this tracer. Keeping the method crate-private prevents that precondition becoming a public
    /// production promise before Session residency ownership exists.
    ...
    #[cfg(test)]
    async fn fork_session_with_test_candidates(
        &self,
        attempt: SealedSessionForkAttempt,
        candidates: impl IntoIterator<Item = Result<SessionId, ()>>,
    ) -> Result<Arc<DurableSessionHead>, DurableSessionForkError> {
        let calls = Arc::new(std::sync::atomic::AtomicUsize::new(0));
        self.actor
            .enqueue_session_fork_with_test_candidates(attempt, candidates, calls)
            .await
            .wait()
            .await
    }
...
#[derive(Clone, Copy)]
enum OptionalDefinitionObservationStage {
    Staged,
    Committed,
    Final,
}

fn read_optional_definition_document(path: &Path) -> Result<Option<Vec<u8>>, DurableOpenError> {
...
fn exact_optional_definition_readback(
    filesystem: &dyn DurableFilesystem,
    path: &Path,
    expected: Option<&[u8]>,
    stage: OptionalDefinitionObservationStage,
) -> PublicationCertainty {
    match filesystem.observe_optional_definition(path, stage) {
...
fn exact_optional_definition_readback_result(
    filesystem: &dyn DurableFilesystem,
    path: &Path,
    expected: Option<&[u8]>,
    stage: OptionalDefinitionObservationStage,
) -> Result<(), ()> {
    if exact_optional_definition_readback(filesystem, path, expected, stage)
        == PublicationCertainty::Published
    {
        Ok(())
    } else {
...
trait DurableFilesystem: Send + Sync {
    ...
    fn observe_optional_definition(
        ...
        let _ = stage;
        read_optional_definition_document(path)
    }
    #[cfg(test)]
...
mod tests {
    ...
    use crate::agent_session_lifecycle::{
        ...
        SealedAgentDefinitionAttempt, SealedAgentMetadataAttempt, SealedAgentStatusAttempt,
        SealedSessionCreateAttempt, SealedSessionForkAttempt, SealedSessionMetadataAttempt,
        SessionForkProvenance, SessionLifecycle, SessionMetadata, SessionMetadataDescriptionPatch,
    ...
    use crate::runtime_task::{RuntimeTaskContext, RuntimeTaskError};
    use crate::wire::{
        AgentId, AgentMetadataRevision, AgentRevision, ItemId, SessionDefinitionRevision,
        SessionId, SessionMetadataRevision, Timestamp,
    };
    use crate::workspace::{
    ...
    impl DurableFilesystem for DeterministicPersistentFaultFilesystem {
        ...
        fn observe_optional_definition(
            &self,
            path: &Path,
            stage: super::OptionalDefinitionObservationStage,
        ) -> Result<Option<Vec<u8>>, DurableOpenError> {
            let operation = match stage {
                super::OptionalDefinitionObservationStage::Staged => {
                    PublicationOperation::ReadbackStagedDefinition
                }
                super::OptionalDefinitionObservationStage::Committed => {
                    PublicationOperation::ReadbackCommittedDefinition
                }
                super::OptionalDefinitionObservationStage::Final => {
            ...
            let bytes = super::read_optional_definition_document(path)?;
            self.complete_publication_operation(operation);
            if matches!(fault, Some(PublicationFault::Corrupt(_))) {
                fs::write(path, b"corrupt").map_err(|_| DurableOpenError::StorageUnavailable)?;
                return Err(DurableOpenError::DurableStateCorrupt);
    ...
    fn run_native_abort_session_fork_child(root: &Path, candidate: SessionId, point: &str) {
        ...
        command
            .arg("--exact")
            .arg("durable_state::tests::native_process_abort_session_fork_child")
            .arg("--ignored")
            .arg("--nocapture")
            .env("MINICORE_TEST_SESSION_FORK_CRASH_CHILD", "v1")
            .env("MINICORE_TEST_CRASH_ROOT", root)
            .env("MINICORE_TEST_CRASH_SESSION", candidate.to_string())
            .env("MINICORE_TEST_CRASH_POINT", point)
            .env("MINICORE_TEST_CRASH_PROOF", proof_file.path())
        ...
        let mut child = command
            .spawn()
            .expect("the native Session Fork crash child starts");
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(30);
        let status = loop {
            match child.try_wait() {
                Ok(Some(status)) => break status,
                ...
                Err(_) => {
                    assert!(
                        kill_and_reap_native_child(&mut child),
                        "the unobservable Session Fork crash child is killed and reaped"
                    );
                    panic!("the native Session Fork crash child could not be observed safely");
                }
            }
            if std::time::Instant::now() >= deadline {
                assert!(
                    kill_and_reap_native_child(&mut child),
                    "the timed-out Session Fork crash child is killed and reaped"
                );
                panic!("the native Session Fork crash child exceeded its bounded deadline");
            }
            std::thread::sleep(std::time::Duration::from_millis(10));
        };
        let proof_bytes = fs::read(proof_file.path());
        ...
        assert_eq!(
            ...
            proof.as_bytes(),
            "an unrelated signal cannot impersonate the Session Fork abort hook"
        );
    }
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn agent_metadata_cas_publishes_exact_g2_retains_definition_and_reopens() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        let agent_id = AgentId::from_str(AGENT_ONE).unwrap();
        let expected_revision = AgentMetadataRevision::new(NonZeroU64::new(1).unwrap());
        let owner_timestamp: Timestamp = "2026-08-03T10:00:00.123Z".parse().unwrap();
        let state = open(root.path()).await.expect("the Enabled Agent opens");
        let outcome = state
            ...
            .await
            .expect("the metadata CAS publishes");
        assert!(outcome.changed());
        assert_eq!(outcome.head().storage_generation().get(), 2);
        assert_eq!(outcome.head().metadata().revision().get(), 2);
        assert_eq!(outcome.head().metadata().name(), "Planner revised");
        assert_eq!(outcome.head().metadata().description(), None);
        assert_eq!(outcome.head().metadata().updated_at(), owner_timestamp);
        assert_eq!(outcome.head().current_definition_revision().get(), 1);
        assert_eq!(
            outcome.head().current_definition_storage_generation().get(),
            1
        );
        assert_eq!(outcome.head().status(), AgentStatus::Enabled);
        assert_eq!(outcome.head().created_at(), owner_timestamp);
        let catalog_head = state.agent_head(agent_id).unwrap();
        assert!(Arc::ptr_eq(outcome.head(), &catalog_head));
        assert_eq!(
            state
        ...
        let reopened = open(root.path()).await.expect("the metadata G2 reopens");
        let reopened_head = reopened.agent_head(agent_id).unwrap();
        assert_eq!(&*reopened_head, &**outcome.head());
        assert_eq!(
            reopened
                .agent_current_definition(agent_id)
                .unwrap()
        ...
        assert!(
            reopened.contains_agent_definition(
                agent_id,
                AgentRevision::new(NonZeroU64::new(1).unwrap())
            )
        );
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn agent_metadata_cas_orders_stale_before_noop_and_deleted_and_allows_disabled_updates() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        let agent_id = AgentId::from_str(AGENT_ONE).unwrap();
        let revision_one = AgentMetadataRevision::new(NonZeroU64::new(1).unwrap());
        let owner_timestamp: Timestamp = "2026-08-03T10:00:05.000Z".parse().unwrap();
        ...
        assert_eq!(
            state
                ...
                .await
                .unwrap_err(),
            super::DurableAgentMetadataError::StaleRevision
        );
        ...
        let no_change = state
            ...
            .await
            .expect("identical canonical metadata is a no-op");
        assert!(!no_change.changed());
        assert_eq!(no_change.head().storage_generation().get(), 1);
        assert_eq!(no_change.head().metadata().revision().get(), 1);
        assert_eq!(
            no_change.head().metadata().updated_at(),
            "2026-08-03T10:00:00.123Z".parse::<Timestamp>().unwrap()
        );
        assert!(
        ...
        let disabled = state
            .update_agent_metadata(
                SealedAgentMetadataAttempt::new(
                    ...
                    revision_one,
                    Some("Planner revised".to_owned()),
                    AgentMetadataDescriptionPatch::set("Disabled update").unwrap(),
                    owner_timestamp,
                )
                .unwrap(),
            )
            .await
            .expect("Disabled still permits metadata updates");
        assert!(disabled.changed());
        assert_eq!(disabled.head().status(), AgentStatus::Disabled);
        assert_eq!(disabled.head().storage_generation().get(), 3);
        assert_eq!(disabled.head().metadata().revision().get(), 2);
        ...
        assert_eq!(
            state
                ...
                .unwrap_err(),
            super::DurableAgentMetadataError::StaleRevision,
            "stale metadata is rejected before the terminal status check"
        );
        assert_eq!(
            state
                .update_agent_metadata(
                ...
                .await
                .unwrap_err(),
            super::DurableAgentMetadataError::AgentDeleted,
            "Deleted remains terminal even when metadata is identical"
        );
        state.close().await;
    }

    #[tokio::test(flavor = "current_thread")]
    async fn agent_definition_then_metadata_publications_retain_complete_definition_index() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        let agent_id = AgentId::from_str(AGENT_ONE).unwrap();
        ...
        let definition = state
            ...
            .await
            .expect("the definition publication succeeds");
        assert_eq!(definition.head().storage_generation().get(), 2);

        let metadata = state
            .update_agent_metadata(
                SealedAgentMetadataAttempt::new(
                    agent_id,
                    AgentMetadataRevision::new(NonZeroU64::new(1).unwrap()),
                    Some("Planner revised".to_owned()),
                    AgentMetadataDescriptionPatch::keep(),
            ...
            .await
            .expect("the metadata publication succeeds after definition");
        assert_eq!(metadata.head().storage_generation().get(), 3);
        assert_eq!(metadata.head().current_definition_revision().get(), 2);
        assert_eq!(
            metadata
                .head()
                .current_definition_storage_generation()
        ...
        {
            let active = super::lock(&state.published_agents);
            let active_entry = active
                .get(&agent_id)
                .expect("the active published Agent overlay contains the mutation");
            let expected_index = BTreeMap::from([
                (
                    AgentRevision::new(NonZeroU64::new(1).unwrap()),
                ...
                (
                    AgentRevision::new(NonZeroU64::new(2).unwrap()),
                    StorageGeneration::new(2).unwrap(),
                ),
            ]);
            assert_eq!(
                &*active_entry.definition_index, &expected_index,
                "metadata publication retains the complete historical definition map"
            );
        }
        ...
        assert_eq!(fs::read(generation.join("head.json")).unwrap(), g3_head);
        assert!(!generation.join("definition.json").exists());
        state.close().await;

        let reopened = open(root.path()).await.expect("the chain reopens");
        assert_eq!(
            reopened
                .agent_head(agent_id)
                .unwrap()
        ...
        for revision in 1..=2 {
            assert!(reopened.contains_agent_definition(
                agent_id,
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn session_agent_basis_arc_clones_complete_definition_index() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        ...
        let state = open(root.path()).await.expect("the historical Agent opens");
        let agent_id = AgentId::from_str(AGENT_ONE).unwrap();
        let source = state
            .agents
            .get(&agent_id)
            .expect("the recovered Agent source entry exists");
        let basis = super::session_agent_basis(
            source,
            AgentRevisionRef::new(agent_id, AgentRevision::new(NonZeroU64::new(1).unwrap())),
        )
        .expect("the source retains the pinned historical revision");
        assert_eq!(basis.definition_index.len(), 2);
        assert!(
            basis
                .definition_index
                .contains_key(&AgentRevision::new(NonZeroU64::new(1).unwrap()))
        );
        assert!(
            basis
                .definition_index
                .contains_key(&AgentRevision::new(NonZeroU64::new(2).unwrap()))
        );
        assert!(Arc::ptr_eq(
            &basis.definition_index,
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn agent_metadata_catalog_waits_for_final_definition_absence_readback() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        let filesystem = Arc::new(DeterministicPersistentFaultFilesystem::new([]));
        ...
        let state_for_update = state.clone();
        let mut update = Box::pin(tokio::spawn(async move {
            state_for_update
                .update_agent_metadata(
                    SealedAgentMetadataAttempt::new(
                        agent_id,
        ...
        assert_eq!(
            state
                ...
                .get(),
            1,
            "the catalog remains old while final absence is blocked"
        );
        assert_eq!(
            state
                .agent_head(agent_id)
        ...
        let generation = agent_path(root.path(), AGENT_ONE)
            .join("generations")
            .join(GENERATION_TWO);
        assert!(generation.join("COMMITTED").is_file());
        assert!(!generation.join("definition.json").exists());
        assert!(
            poll_once_pending(update.as_mut()).await,
            "the waiter/task stays unsettled behind final definition-absence readback"
        );

        release
            .send(())
            .expect("the final metadata definition-absence readback resumes");
        let outcome = update
            .await
            .expect("the metadata update task joins")
            .expect("the metadata update settles");
        assert!(outcome.changed());
        assert_eq!(outcome.head().storage_generation().get(), 2);
        assert_eq!(
            state
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn agent_metadata_noop_performs_zero_publication_io_and_preserves_head() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        let filesystem = Arc::new(
        ...
        assert!(filesystem.publication_operations().is_empty());
        let debug = format!("{no_change:?}");
        assert!(!debug.contains(AGENT_ONE));
        ...
        let error = state
            ...
            .await
            .unwrap_err();
        assert_eq!(error, super::DurableAgentMetadataError::StorageUnavailable);
        let error_text = error.to_string();
        assert!(!error_text.contains(AGENT_ONE));
        assert!(!error_text.contains("Planner"));
        assert!(!error_text.contains("10:00:05"));
        state.close().await;
    }
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn agent_metadata_exclusive_gate_linearizes_before_queued_delete_and_session_create() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        let agent_id = AgentId::from_str(AGENT_ONE).unwrap();
        ...
        let delete = state
            .actor
            .enqueue_agent_status(SealedAgentStatusAttempt::delete(
                agent_id,
                AgentStatus::Enabled,
            ))
            .await;
        let session_id = session_candidate(104);
        let actor = state.actor.clone();
        let session = tokio::spawn(async move {
            actor
                .enqueue_session_create_with_test_candidates(
        ...
        let metadata = metadata
            .wait()
            .await
            .expect("the metadata CAS wins the exclusive epoch");
        assert!(metadata.changed());
        assert_eq!(metadata.head().metadata().name(), "Planner revised");
        assert_eq!(metadata.head().storage_generation().get(), 2);

        let delete = delete.wait().await.expect("the queued delete settles");
        assert!(delete.changed());
        assert_eq!(delete.head().status(), AgentStatus::Deleted);
        assert_eq!(delete.head().storage_generation().get(), 3);

        assert_eq!(
            session.await.expect("the queued Session task joins"),
            Err(super::DurableSessionCreateError::AgentDeleted),
            "the queued Create observes the completed delete epoch"
        );
        assert!(!session_path(root.path(), &session_id.to_string()).exists());
        state.close().await;
    }
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn session_metadata_cas_publishes_exact_g2_retains_definition_and_conversation() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        let session_id = SessionId::from_str(SESSION_ONE).unwrap();
        let conversation_path = session_path(root.path(), SESSION_ONE).join("conversation.jsonl");
        let conversation_before =
            fs::read(&conversation_path).expect("the Session conversation exists");
        let conversation_metadata_before =
            fs::symlink_metadata(&conversation_path).expect("the conversation metadata exists");
        let state = open(root.path()).await.expect("the Session store opens");
        let original_definition = state
            .session_current_definition(session_id)
            .expect("the current definition is catalogued");
        let outcome = state
            .update_session_metadata(SealedSessionMetadataAttempt::new(
                session_id,
                SessionMetadataRevision::new(NonZeroU64::new(1).unwrap()),
                SessionMetadataNamePatch::set("Project session").unwrap(),
                SessionMetadataDescriptionPatch::keep(),
                "2026-08-03T10:01:05.000Z".parse().unwrap(),
            ))
            .await
            .expect("the Session metadata CAS publishes");
        assert!(outcome.changed());
        assert_eq!(outcome.head().storage_generation().get(), 2);
        assert_eq!(outcome.head().metadata().revision().get(), 2);
        assert_eq!(outcome.head().metadata().name(), Some("Project session"));
        assert_eq!(outcome.head().metadata().description(), None);
        assert_eq!(
            outcome.head().metadata().updated_at(),
            "2026-08-03T10:01:05.000Z".parse::<Timestamp>().unwrap()
        );
        assert_eq!(outcome.head().lifecycle(), SessionLifecycle::Open);
        assert_eq!(
            outcome.head().created_at(),
            "2026-08-03T10:01:00.456Z".parse::<Timestamp>().unwrap()
        );
        assert!(Arc::ptr_eq(
            outcome.head(),
            &state
                .session_head(session_id)
                .expect("the new head is catalogued")
        ));
        assert!(Arc::ptr_eq(
            &original_definition,
            &state
                .session_current_definition(session_id)
                .expect("the old definition remains catalogued")
        ));

        let generation = session_path(root.path(), SESSION_ONE)
            .join("generations")
            .join(GENERATION_TWO);
        assert_eq!(
            fs::read(generation.join("head.json")).unwrap(),
        ...
        let conversation_metadata_after =
            fs::symlink_metadata(&conversation_path).expect("the conversation remains");
        assert_eq!(
            conversation_metadata_after.len(),
            conversation_metadata_before.len()
        );
        #[cfg(unix)]
        {
            use std::os::unix::fs::MetadataExt;
            assert_eq!(
                conversation_metadata_after.dev(),
                conversation_metadata_before.dev()
            );
            assert_eq!(
                conversation_metadata_after.ino(),
                conversation_metadata_before.ino()
            );
        }
        state.close().await;

        let reopened = open(root.path()).await.expect("the metadata G2 reopens");
        let reopened_head = reopened.session_head(session_id).expect("the head reopens");
        assert_eq!(reopened_head.as_ref(), outcome.head().as_ref());
        assert_eq!(
            &*reopened
                .session_current_definition(session_id)
                .expect("the definition reopens"),
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn session_metadata_on_fork_retains_provenance_and_conversation() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        ...
        let session_id = SessionId::from_str(SESSION_TWO).unwrap();
        ...
        let state = open(root.path()).await.expect("the fork Session opens");
        ...
        let outcome = state
            .update_session_metadata(SealedSessionMetadataAttempt::new(
                session_id,
                SessionMetadataRevision::new(NonZeroU64::new(1).unwrap()),
                SessionMetadataNamePatch::set("Fork session").unwrap(),
                SessionMetadataDescriptionPatch::keep(),
                "2026-08-03T10:04:05.000Z".parse().unwrap(),
            ))
            .await
            .expect("fork metadata publication succeeds");
        assert!(outcome.changed());
        assert_eq!(outcome.head().storage_generation().get(), 2);
        assert_eq!(outcome.head().fork_provenance(), provenance.as_ref());
        assert!(
            !session_path(root.path(), SESSION_TWO)
                .join("generations")
                .join(GENERATION_TWO)
                .join("definition.json")
                .exists()
        ...
        let reopened = open(root.path())
            .await
            .expect("the fork metadata chain reopens");
        assert_eq!(
            reopened.session_head(session_id).unwrap().fork_provenance(),
            provenance.as_ref()
        );
        assert_eq!(fs::read(&conversation_path).unwrap(), conversation_before);
        reopened.close().await;
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn session_metadata_after_definition_retains_current_definition_and_conversation() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_agent_generation(
        ...
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        let session_definition_g2 = replace_fixture(
        ...
        let state = open(root.path())
            .await
            .expect("the definition-bearing Session opens");
        ...
        let outcome = state
            ...
            .await
            .expect("metadata follows a definition publication");
        assert!(outcome.changed());
        assert_eq!(outcome.head().storage_generation().get(), 3);
        assert_eq!(outcome.head().current_definition_revision().get(), 2);
        assert_eq!(
            outcome.head().current_definition_storage_generation().get(),
            2
        ...
        assert!(Arc::ptr_eq(
            &definition,
            &state.session_current_definition(session_id).unwrap()
        ));
        let generation = session_path(root.path(), SESSION_ONE)
            .join("generations")
            .join(GENERATION_THREE);
        ...
        assert_eq!(fs::read(&conversation_path).unwrap(), conversation_before);
        state.close().await;
        let reopened = open(root.path()).await.expect("the metadata chain reopens");
        let head = reopened.session_head(session_id).unwrap();
        assert_eq!(head.storage_generation().get(), 3);
        assert_eq!(head.current_definition_revision().get(), 2);
        assert_eq!(head.current_definition_storage_generation().get(), 2);
        assert_eq!(
            reopened
                .session_current_definition(session_id)
                .unwrap()
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn session_metadata_after_definition_cleans_up_and_retries_with_historical_agent_basis() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_agent_generation(
        ...
        let session_id = SessionId::from_str(SESSION_ONE).unwrap();
        let make_attempt = || {
            SealedSessionMetadataAttempt::new(
                session_id,
                SessionMetadataRevision::new(NonZeroU64::new(1).unwrap()),
                SessionMetadataNamePatch::set("Project session").unwrap(),
                SessionMetadataDescriptionPatch::keep(),
        ...
        assert_eq!(
            state
                .update_session_metadata(make_attempt())
                .await
                .unwrap_err(),
            super::DurableSessionMetadataError::StorageUnavailable
        );
        assert!(
            !session_path(root.path(), SESSION_ONE)
                .join("generations")
                .join(GENERATION_THREE)
                .exists(),
            "pre-COMMITTED cleanup removes the historical-chain staging generation"
        );
        let retry = state
            .update_session_metadata(make_attempt())
            .await
            .expect("the retry publishes after exact cleanup");
        assert_eq!(retry.head().storage_generation().get(), 3);
        assert_eq!(
            state
                .session_current_definition(session_id)
                .unwrap()
        ...
        state.close().await;
        let reopened = open(root.path()).await.expect("the retry chain reopens");
        assert_eq!(
            reopened
                .session_head(session_id)
                .unwrap()
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn session_metadata_noop_performs_zero_publication_io_and_preserves_head() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        ...
        let state = open_with_reservation_filesystem(root.path(), Arc::clone(&filesystem)).await;
        let session_id = SessionId::from_str(SESSION_ONE).unwrap();
        let original_head = state.session_head(session_id).unwrap();
        let original_definition = state.session_current_definition(session_id).unwrap();
        let conversation_path = session_path(root.path(), SESSION_ONE).join("conversation.jsonl");
        let conversation_before = fs::read(&conversation_path).unwrap();
        ...
        assert!(!attempt_debug.contains(SESSION_ONE));
        let no_change = state
            .update_session_metadata(attempt)
            .await
            .expect("the armed publication fault is bypassed by the no-op");
        assert!(!no_change.changed());
        assert!(Arc::ptr_eq(no_change.head(), &original_head));
        ...
        assert_eq!(fs::read(&conversation_path).unwrap(), conversation_before);
        assert!(filesystem.publication_operations().is_empty());
        let outcome_debug = format!("{no_change:?}");
        assert!(!outcome_debug.contains(SESSION_ONE));
        assert!(!outcome_debug.contains("10:01:05"));
        let error = state
            .update_session_metadata(SealedSessionMetadataAttempt::new(
                session_id,
                SessionMetadataRevision::new(NonZeroU64::new(1).unwrap()),
        ...
        let error_text = error.to_string();
        assert!(!error_text.contains(SESSION_ONE));
        assert!(!error_text.contains("Project session"));
        state.close().await;
    }

    #[tokio::test(flavor = "current_thread")]
    async fn session_metadata_cas_allows_archived_and_orders_stale_before_deleted() {
        let archived_root = TempRoot::existing();
        create_marked_empty_store(archived_root.path());
        create_valid_g1_agent(archived_root.path());
        ...
        let session_id = SessionId::from_str(SESSION_ONE).unwrap();
        let archived = open(archived_root.path())
            .await
            .expect("the archived Session opens");
        assert_eq!(
            archived.session_head(session_id).unwrap().lifecycle(),
        ...
        assert_eq!(
            archived
                .update_session_metadata(SealedSessionMetadataAttempt::new(
                    session_id,
                    SessionMetadataRevision::new(NonZeroU64::new(2).unwrap()),
                    SessionMetadataNamePatch::keep(),
                    SessionMetadataDescriptionPatch::keep(),
                    "2026-08-03T10:01:05.000Z".parse().unwrap(),
                ))
                .await
                .unwrap_err(),
            super::DurableSessionMetadataError::StaleRevision
        );
        let archived_update = archived
            .update_session_metadata(SealedSessionMetadataAttempt::new(
                session_id,
                SessionMetadataRevision::new(NonZeroU64::new(1).unwrap()),
                SessionMetadataNamePatch::set("Archived session").unwrap(),
                SessionMetadataDescriptionPatch::keep(),
                "2026-08-03T10:01:05.000Z".parse().unwrap(),
            ))
            .await
            .expect("Archived permits metadata updates");
        assert!(archived_update.changed());
        assert_eq!(
            archived_update.head().lifecycle(),
            SessionLifecycle::Archived
        );
        assert_eq!(archived_update.head().storage_generation().get(), 3);
        archived.close().await;
        ...
        let deleted = open(deleted_root.path())
            .await
            .expect("the deleted Session opens");
        assert_eq!(
            deleted
                .update_session_metadata(SealedSessionMetadataAttempt::new(
                    session_id,
        ...
        assert_eq!(
            deleted
                ...
                .await
                .unwrap_err(),
            super::DurableSessionMetadataError::SessionDeleted
        );
        deleted.close().await;
    }

    #[tokio::test(flavor = "current_thread")]
    async fn session_metadata_gate_linearizes_same_expected_mutations_and_direct_contender() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        ...
        let gate = state
            ...
            .gate(session_id)
            .try_write_owned()
            .expect("the direct contender owns the idle Session gate");
        let (release_gate, wait_gate) = std::sync::mpsc::channel();
        let gate_holder = std::thread::spawn(move || {
            let _gate = gate;
            wait_gate
                .recv()
                .expect("the test releases the direct Session gate contender");
        });
        let first = state
            .actor
            .enqueue_session_metadata(SealedSessionMetadataAttempt::new(
                session_id,
                SessionMetadataRevision::new(NonZeroU64::new(1).unwrap()),
                SessionMetadataNamePatch::set("First winner").unwrap(),
                SessionMetadataDescriptionPatch::keep(),
                "2026-08-03T10:01:05.000Z".parse().unwrap(),
            ))
            .await;
        state
            .session_lifecycle_gates
            .wait_for_session_mutation_for_test()
            .await;
        assert_eq!(
            state
                .session_head(session_id)
                ...
                .get(),
            1,
            "the direct gate contender keeps the catalog old"
        );
        let second = state
            .actor
            .enqueue_session_metadata(SealedSessionMetadataAttempt::new(
                session_id,
                SessionMetadataRevision::new(NonZeroU64::new(1).unwrap()),
                SessionMetadataNamePatch::set("Second loser").unwrap(),
                SessionMetadataDescriptionPatch::keep(),
                "2026-08-03T10:01:06.000Z".parse().unwrap(),
            ))
            .await;
        release_gate
            .send(())
            .expect("the direct Session gate contender releases");
        gate_holder.join().expect("the direct contender joins");
        let first = first.wait().await.expect("the first mutation publishes");
        assert!(first.changed());
        assert_eq!(first.head().metadata().name(), Some("First winner"));
        assert_eq!(
            second.wait().await.unwrap_err(),
            super::DurableSessionMetadataError::StaleRevision
        );
        assert_eq!(
            state.session_head(session_id).unwrap().metadata().name(),
            Some("First winner")
        );
        state.close().await;
    }
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn session_metadata_catalog_waits_for_final_definition_absence_readback() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        ...
        release
            .send(())
            .expect("the final definition absence readback resumes");
        let outcome = update
            .await
            .expect("the update task joins")
            .expect("the Session metadata update settles");
        assert_eq!(outcome.head().metadata().revision().get(), 2);
        assert_eq!(
            state.session_head(session_id).unwrap().metadata().name(),
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn session_metadata_faults_reconcile_definition_absence_at_each_stage() {
        let ordinary_cases = [
            (
                "head-write-after",
        ...
        for (name, fault, publishes, closes) in ordinary_cases {
            let root = TempRoot::existing();
            create_marked_empty_store(root.path());
            create_valid_g1_agent(root.path());
            create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
            ...
            let first = state.update_session_metadata(make_attempt()).await;
            if publishes {
                let outcome = first.unwrap_or_else(|error| panic!("{name}: {error:?}"));
                assert!(outcome.changed(), "{name}");
                assert_eq!(outcome.head().metadata().revision().get(), 2, "{name}");
                assert_eq!(
                    outcome.head().metadata().name(),
                    Some("Project session"),
                    "{name}"
                );
                let generation = session_path(root.path(), SESSION_ONE)
                    .join("generations")
                    .join(GENERATION_TWO);
                assert!(!generation.join("definition.json").exists(), "{name}");
                ...
                if closes {
                    assert_eq!(
                        state
                            ...
                            .await
                            .unwrap_err(),
                        super::DurableSessionMetadataError::Closing,
                        "{name} settles success before remembered close"
                    );
                }
            } else {
                assert_eq!(
                    first.unwrap_err(),
                    super::DurableSessionMetadataError::StorageUnavailable,
                    "{name} is an ordinary pre-COMMITTED failure"
                );
                assert!(
                    !session_path(root.path(), SESSION_ONE)
                        .join("generations")
                ...
                assert_eq!(retry.head().metadata().revision().get(), 2, "{name}");
            }
            state.close().await;
            let reopened = open(root.path())
                .await
            ...
            assert_eq!(
                reopened
                    ...
                    .get(),
                2,
                "{name} reopens the retained metadata"
            );
            reopened.close().await;
        }
        ...
        let poisoned_cases = [
            (
                "committed-definition-indeterminate",
                PublicationFault::Indeterminate(PublicationOperation::ReadbackCommittedDefinition),
                false,
            ),
            (
                "committed-definition-corrupt",
                PublicationFault::Corrupt(PublicationOperation::ReadbackCommittedDefinition),
                true,
            ),
            (
                "final-definition-before",
                PublicationFault::Before(PublicationOperation::ReadbackPublishedDefinition),
                false,
            ),
            (
                "final-definition-after",
                PublicationFault::After(PublicationOperation::ReadbackPublishedDefinition),
                false,
            ),
            (
                "final-definition-indeterminate",
                PublicationFault::Indeterminate(PublicationOperation::ReadbackPublishedDefinition),
                false,
            ),
            (
                "final-definition-corrupt",
                PublicationFault::Corrupt(PublicationOperation::ReadbackPublishedDefinition),
                true,
            ),
        ];
        for (name, fault, corrupt) in poisoned_cases {
            let root = TempRoot::existing();
            create_marked_empty_store(root.path());
            create_valid_g1_agent(root.path());
            create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
            ...
            assert!(
                matches!(
                    result,
                    Err(super::DurableSessionMetadataError::InternalDispatchUnavailable)
                ),
                "{name} poisons after committed ambiguity or final proof failure"
            );
            let definition = session_path(root.path(), SESSION_ONE)
                .join("generations")
                .join(GENERATION_TWO)
                .join("definition.json");
            if corrupt {
                assert_eq!(fs::read(&definition).unwrap(), b"corrupt", "{name}");
            } else {
                assert!(!definition.exists(), "{name} retains definition absence");
            }
            state.close().await;
            if corrupt {
                assert!(matches!(
                ...
                let reopened = open(root.path())
                    ...
                    .unwrap_or_else(|error| panic!("{name}: {error:?}"));
                let head = reopened.session_head(session_id).unwrap();
                assert_eq!(head.storage_generation().get(), 2, "{name}");
                assert_eq!(head.metadata().revision().get(), 2, "{name}");
                reopened.close().await;
            }
        }
    }
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn session_metadata_actor_abort_after_committed_keeps_session_gate_and_lease() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        ...
        assert!(
            state
                ...
                .try_write_owned()
                .is_err(),
            "the owner-tracked post-COMMITTED child retains the Session gate"
        );
        let mut close = Box::pin(state.close());
        assert!(
            poll_once_pending(close.as_mut()).await,
            "shutdown retains the metadata publication child and root lease"
        );
        assert!(matches!(
            open(root.path()).await,
        ...
        release
            .send(())
            .expect("the committed Session metadata publication resumes");
        close.await;
        let reopened = open(root.path())
            .await
            .expect("post-COMMITTED metadata actor loss reopens");
        let head = reopened.session_head(session_id).unwrap();
        assert_eq!(head.storage_generation().get(), 2);
        assert_eq!(head.metadata().revision().get(), 2);
        assert_eq!(head.metadata().name(), Some("Project session"));
        reopened.close().await;
    }

    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_publishes_exact_child_g1_and_reopens() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        ...
        let child = SessionId::from_str(SESSION_THREE).unwrap();
        let attempt = SealedSessionForkAttempt::recorded_genesis(
            source,
        ...
        let returned_head = state
            ...
            .await
            .expect("the recorded Genesis Fork publishes");
        let catalog_head = state
            .session_head(child)
            .expect("the child head is installed before completion");
        assert!(Arc::ptr_eq(&returned_head, &catalog_head));
        assert_eq!(
        ...
        let child_entity = session_path(root.path(), SESSION_THREE);
        ...
        assert_eq!(fs::read(generation.join("COMMITTED")).unwrap(), b"");
        assert_eq!(fs::read(child_entity.join("PUBLISHED")).unwrap(), b"");
        assert_eq!(
            fs::read(child_entity.join("conversation.jsonl")).unwrap(),
            include_bytes!("../docs/fixtures/durable-store-v1/genesis-fork-child.jsonl")
        );
        let definition = state
            .session_current_definition(child)
            .expect("the child definition is installed");
        assert_eq!(definition.revision().get(), 1);
        assert_eq!(definition.workspace().revision().get(), 1);
        assert_eq!(definition.agent().revision().get(), 1);
        assert!(state.session_head(source).is_some());
        state.close().await;
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_rejects_missing_and_deleted_sources_after_reservation() {
        for (name, install_source, expected) in [
            (
                "missing",
                false,
                super::DurableSessionForkError::SourceNotFound,
            ),
            (
                "deleted",
                true,
                super::DurableSessionForkError::SourceDeleted,
            ),
        ] {
            let root = TempRoot::existing();
            create_marked_empty_store(root.path());
            ...
            let state = open(root.path())
                .await
                .unwrap_or_else(|error| panic!("the {name} source store opens: {error:?}"));
            assert_eq!(
                state
                    .fork_session_with_test_candidates(
                        SealedSessionForkAttempt::recorded_genesis(
                            source,
                            "2026-08-03T10:03:00.000Z".parse().unwrap(),
                        ),
                        [Ok(child)],
                    ...
                    .await,
                Err(expected),
                "the {name} source is classified exactly"
            );
            assert!(
                root.path()
                    .join(RESERVATIONS_DIRECTORY)
                    ...
                    .join(SESSION_THREE)
                    .is_file(),
                "the child identity is permanently reserved before source resolution"
            );
            assert!(!session_path(root.path(), SESSION_THREE).exists());
            state.close().await;
        }
    }

    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_allows_archived_source_and_preserves_captured_agent_revision() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_agent_generation(
        ...
        let source = SessionId::from_str(SESSION_ONE).unwrap();
        let child = SessionId::from_str(SESSION_THREE).unwrap();
        let state = open(root.path()).await.expect("the archived source opens");
        assert_eq!(
            state
                ...
                .get(),
            2,
            "the Agent current definition is newer than the source capture"
        );
        state
            .fork_session_with_test_candidates(
                SealedSessionForkAttempt::recorded_genesis(
            ...
            .await
            .expect("an archived source remains forkable while the Agent is Enabled");
        let child_definition = state.session_current_definition(child).unwrap();
        assert_eq!(child_definition.agent().revision().get(), 1);
        assert_eq!(child_definition.workspace().revision().get(), 1);
        state.close().await;

        let reopened = open(root.path())
            .await
            .expect("the child retaining Agent revision one reopens");
        assert_eq!(
            reopened
                .session_current_definition(child)
                .unwrap()
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_captures_the_source_current_definition_semantics() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &valid_ordinary_conversation(SESSION_ONE));
        create_session_generation(
            ...
            GENERATION_TWO,
            session_head_definition_g2_fixture(),
            Some(&session_definition_g2_fixture()),
        );
        let child = SessionId::from_str(SESSION_THREE).unwrap();
        let state = open(root.path()).await.expect("the G2 source opens");
        state
            .fork_session_with_test_candidates(
                SealedSessionForkAttempt::recorded_genesis(
            ...
            .await
            .expect("the Fork captures the source current definition");
        let definition = state.session_current_definition(child).unwrap();
        assert_eq!(definition.revision().get(), 1);
        assert_eq!(definition.workspace().revision().get(), 1);
        assert_eq!(definition.model().max_output_tokens().unwrap().get(), 8192);
        assert_eq!(
            definition.created_at(),
            "2026-08-03T10:03:00.000Z".parse().unwrap()
        );
        ...
        let reopened = open(root.path())
            .await
            .expect("the G2-derived child reopens");
        assert_eq!(
            reopened
                .session_current_definition(child)
                .unwrap()
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_requires_the_current_agent_to_be_enabled() {
        for (status, expected) in [
            (
                AgentStatus::Disabled,
                super::DurableSessionForkError::AgentDisabled,
            ),
            (
                AgentStatus::Deleted,
                super::DurableSessionForkError::AgentDeleted,
            ),
        ] {
            let root = TempRoot::existing();
            create_marked_empty_store(root.path());
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_source_conversation_failures_are_typed_and_session_local() {
        for (name, remove_source, expected) in [
            (
                "noncanonical Header",
                false,
                super::DurableSessionForkError::SourceConversationCorrupt,
            ),
            (
                "missing conversation",
                true,
                super::DurableSessionForkError::StorageUnavailable,
            ),
        ] {
            let root = TempRoot::existing();
            create_marked_empty_store(root.path());
            ...
            let state = open(root.path())
                .await
                .unwrap_or_else(|error| panic!("the {name} source is initially valid: {error:?}"));
            let source_conversation =
                session_path(root.path(), SESSION_ONE).join("conversation.jsonl");
            if remove_source {
                fs::remove_file(&source_conversation).unwrap();
            } else {
                let canonical = fs::read(&source_conversation).unwrap();
                let mut noncanonical = Vec::with_capacity(canonical.len() + 1);
                noncanonical.push(b' ');
                noncanonical.extend_from_slice(&canonical);
                fs::write(&source_conversation, noncanonical).unwrap();
            }
            assert_eq!(
                state
                    .fork_session_with_test_candidates(
                        ...
                        [Ok(session_candidate(96))],
                    )
                    .await,
                Err(expected),
                "{name} maps through the source capability"
            );
            assert_eq!(
                state
                    .create_session_with_test_candidates(
                        ordinary_session_create_attempt(AgentId::from_str(AGENT_ONE).unwrap()),
                        [Ok(session_candidate(97))],
                    )
                    .await
                    .map(|head| head.session_id()),
                Ok(session_candidate(97)),
                "{name} degrades only that source Session rather than closing DurableState"
            );
            state.close().await;
        }
    }
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_treats_a_changed_required_source_definition_as_fatal() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        let state = open(root.path())
            .await
            .expect("the source is initially valid");
        fs::write(
            session_path(root.path(), SESSION_ONE)
                .join("generations")
                .join(GENERATION_ONE)
                .join("definition.json"),
            b"corrupt source definition",
        )
        .unwrap();
        assert_eq!(
            state
                .fork_session_with_test_candidates(
                    ...
                    [Ok(session_candidate(98))],
                )
                .await,
            Err(super::DurableSessionForkError::InternalDispatchUnavailable)
        );
        assert!(!session_path(root.path(), &session_candidate(98).to_string()).exists());
        state.close().await;
        assert!(matches!(
            open(root.path()).await,
            Err(DurableOpenError::DurableStateCorrupt)
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_exactly_revalidates_the_source_current_generation() {
        for remove_current_definition in [false, true] {
            let root = TempRoot::existing();
            create_marked_empty_store(root.path());
            create_valid_g1_agent(root.path());
            create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
            ...
            let state = open(root.path())
                .await
                .expect("the G2 source is initially valid");
            let current_definition_path = session_path(root.path(), SESSION_ONE)
                .join("generations")
                .join(GENERATION_TWO)
                .join("definition.json");
            if remove_current_definition {
                fs::remove_file(&current_definition_path).unwrap();
            } else {
                let changed = replace_fixture(
                    &fs::read(&current_definition_path).unwrap(),
                    "\"maxOutputTokens\":8192",
                    "\"maxOutputTokens\":8193",
                );
                fs::write(&current_definition_path, changed).unwrap();
            }
            let child = session_candidate(if remove_current_definition { 100 } else { 101 });
            assert_eq!(
                state
                    .fork_session_with_test_candidates(
                        SealedSessionForkAttempt::recorded_genesis(
            ...
            assert!(!session_path(root.path(), &child.to_string()).exists());
            state.close().await;
            if remove_current_definition {
                assert!(matches!(
                    open(root.path()).await,
                ...
                let reopened = open(root.path())
                    .await
                    .expect("restart may adopt a separately valid externally changed store");
                assert_eq!(
                    reopened
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_portably_rejects_a_child_equal_to_its_source() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        ...
        assert_eq!(
            state
                ...
                .await,
            Err(super::DurableSessionForkError::SourceIdentityConflict)
        );
        assert_eq!(
            fs::read_dir(root.path().join(SESSIONS_DIRECTORY))
                .unwrap()
                .count(),
            1
        );
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_rejects_source_identity_before_reservation_io() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        ...
        assert_eq!(
            state
                ...
                .await,
            Err(super::DurableSessionForkError::SourceIdentityConflict),
            "the untouched BeforeCreate fault proves no reservation syscall occurred"
        );
        assert_eq!(
            fs::read_dir(root.path().join(SESSIONS_DIRECTORY))
                .unwrap()
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_maps_pre_source_worker_owner_close_to_closing() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        ...
        let child = session_candidate(99);
        assert_eq!(
            state
                .fork_session_with_test_candidates(
                    SealedSessionForkAttempt::recorded_genesis(
                ...
                .await,
            Err(super::DurableSessionForkError::Closing)
        );
        assert!(
            root.path()
                .join(RESERVATIONS_DIRECTORY)
                ...
                .is_file(),
            "the child identity was burned before the source job was rejected"
        );
        assert!(!session_path(root.path(), &child.to_string()).exists());
        state.close().await;
    }
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn agent_metadata_faults_clean_precommit_or_reconcile_committed_generation() {
        let cases = [
            (
                "head-write-after",
                PublicationFault::After(PublicationOperation::WriteHead),
        ...
        for (name, fault, publishes, closes) in cases {
            ...
            let first = state.update_agent_metadata(make_attempt()).await;
            if publishes {
                let outcome = first.unwrap_or_else(|error| panic!("{name}: {error:?}"));
                assert!(outcome.changed(), "{name}");
                assert_eq!(outcome.head().metadata().revision().get(), 2);
                ...
                assert!(
                    !agent_path(root.path(), AGENT_ONE)
                        ...
                        .join("definition.json")
                        .exists(),
                    "{name} proves the metadata generation has no definition"
                );
                let publication_operations = filesystem.publication_operations();
                for operation in [
                    PublicationOperation::ReadbackStagedDefinition,
                    PublicationOperation::ReadbackCommittedDefinition,
                    PublicationOperation::ReadbackPublishedDefinition,
                    ...
                    assert!(
                        publication_operations.contains(&operation),
                        "{name} observes definition absence at every publication stage"
                    );
                }
                if closes {
                    assert_eq!(
                        state
                            .update_agent_metadata(make_attempt())
                            .await
                            .unwrap_err(),
                        super::DurableAgentMetadataError::Closing,
                        "{name} settles success before remembered close"
                    );
                }
            } else {
                assert_eq!(
                    first.unwrap_err(),
                    super::DurableAgentMetadataError::StorageUnavailable,
                    "{name} is an ordinary pre-COMMITTED failure"
                );
                assert!(
                    !agent_path(root.path(), AGENT_ONE)
            ...
            assert_eq!(
                reopened
                    ...
                    .get(),
                2,
                "{name} recovers the retained metadata"
            );
            reopened.close().await;
        }
    }
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn agent_metadata_definition_absence_fault_certainty_is_stage_specific() {
        let committed_cases = [
            (
                "committed-definition-before",
                PublicationFault::Before(PublicationOperation::ReadbackCommittedDefinition),
            ),
            (
                "committed-definition-after",
                PublicationFault::After(PublicationOperation::ReadbackCommittedDefinition),
            ),
        ];
        for (name, fault) in committed_cases {
            let root = TempRoot::existing();
            create_marked_empty_store(root.path());
            create_valid_g1_agent(root.path());
            let filesystem =
            ...
            let result = state.update_agent_metadata(make_attempt()).await;
            let outcome = result.unwrap_or_else(|error| panic!("{name}: {error:?}"));
            assert!(outcome.changed(), "{name}");
            assert_eq!(outcome.head().metadata().revision().get(), 2, "{name}");
            assert_eq!(
            ...
            assert_eq!(
                state
                    ...
                    .unwrap_err(),
                super::DurableAgentMetadataError::Closing,
                "{name} settles Completed before remembered Closing"
            );
            state.close().await;
            let reopened = open(root.path())
                .await
            ...
            assert_eq!(
                reopened
                    ...
                    .get(),
                2,
                "{name} reopens the committed new metadata"
            );
            reopened.close().await;
        }
        ...
        for (name, fault, corrupt) in poisoned_cases {
            ...
            assert!(
                matches!(
                    result,
                    Err(super::DurableAgentMetadataError::InternalDispatchUnavailable)
                ),
                "{name} poisons the current actor after COMMITTED/PUBLISHED"
            );
            if corrupt {
                assert_eq!(
                    fs::read(
                    ...
                    .unwrap(),
                    b"corrupt",
                    "{name} creates the unexpected definition payload"
                );
            } else {
                assert!(
                    !agent_path(root.path(), AGENT_ONE)
                        ...
                        .join("definition.json")
                        .exists(),
                    "{name} retains definition absence"
                );
            }
            state.close().await;
            if corrupt {
                assert!(
                    ...
                    "{name} reopens corrupt"
                );
            } else {
                let reopened = open(root.path())
                    .await
                    .unwrap_or_else(|error| panic!("{name} reopens: {error:?}"));
                let head = reopened.agent_head(agent_id).unwrap();
                assert_eq!(head.storage_generation().get(), 2, "{name}");
                assert_eq!(head.metadata().revision().get(), 2, "{name}");
                assert_eq!(head.metadata().name(), "Planner revised", "{name}");
                reopened.close().await;
            }
        }
    }
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn recorded_genesis_fork_faults_preserve_source_dependency_and_visibility_certainty() {
        let cases = [
            (
                "conversation-write-after",
                PublicationFault::After(PublicationOperation::WriteConversation),
        ...
        for (name, fault, publishes, closes) in cases {
            ...
            let state =
                open_with_reservation_filesystem(root.path(), Arc::clone(&filesystem)).await;
            let source = SessionId::from_str(SESSION_ONE).unwrap();
            let first = session_candidate(91);
            let second = session_candidate(92);
            let make_attempt = || {
                SealedSessionForkAttempt::recorded_genesis(
                    source,
                    "2026-08-03T10:03:00.000Z".parse().unwrap(),
            ...
            let first_result = state
                .fork_session_with_test_candidates(make_attempt(), [Ok(first)])
                .await;
            ...
            if publishes {
                ...
                assert!(
                    state.session_head(source).is_some(),
                    "{name} retains source"
                );
                assert!(state.session_head(first).is_some(), "{name} installs child");
                let second_result = state
                    .fork_session_with_test_candidates(make_attempt(), [Ok(second)])
                    .await;
                if closes {
                    assert_eq!(
                        second_result,
                        Err(super::DurableSessionForkError::Closing),
                        "{name} closes only after child success"
                    );
                } else {
                    assert_eq!(
                        second_result.map(|head| head.session_id()),
                ...
                assert_eq!(
                    first_result,
                    Err(super::DurableSessionForkError::StorageUnavailable),
                    "{name} is an ordinary prepublication failure"
                );
                assert!(
                    state.session_head(source).is_some(),
                    "{name} retains source"
                ...
                assert_eq!(
                    state
                        ...
                        .map(|head| head.session_id()),
                    Ok(second),
                    "{name} cleanup validated the source dependency"
                );
            }
            ...
            let reopened = open(root.path())
                .await
                .unwrap_or_else(|error| panic!("{name} Fork store reopens: {error:?}"));
            assert!(reopened.session_head(source).is_some(), "{name}");
            assert_eq!(reopened.session_head(first).is_some(), publishes, "{name}");
            reopened.close().await;
        }
    }
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn dropped_fork_facades_cannot_release_the_root_lease_before_actor_job_settlement() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
        let filesystem = Arc::new(DeterministicPersistentFaultFilesystem::new([]));
        let state = open_with_reservation_filesystem(root.path(), Arc::clone(&filesystem)).await;
        let owner = state.task_context.clone();
        let (entered, entered_receiver) = std::sync::mpsc::channel();
        let (release, release_receiver) = std::sync::mpsc::channel();
        filesystem.install_commit_barrier(ReservationBarrier::new(entered, release_receiver));
        let child = session_candidate(95);
        ...
        assert!(
            fork.await
                .expect_err("the dispatch future is aborted")
                .is_cancelled()
        );
        drop(state);

        let contender_context = RuntimeTaskContext::new(Handle::current())
            .await
            .expect("the contender has a time driver");
        assert!(matches!(
            DurableState::open(root.path().to_owned(), contender_context.clone()).await,
            Err(DurableOpenError::StoreInUse)
        ));
        contender_context.shutdown().await;

        release
            .send(())
            .expect("the actor-owned Fork publication resumes");
        owner.shutdown().await;
        let reopened = open(root.path())
            .await
            .expect("the root lease releases only after actor and child settlement");
        assert!(reopened.session_head(child).is_some());
        reopened.close().await;
    }
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "current_thread")]
    async fn agent_metadata_actor_abort_after_committed_keeps_agent_gate_and_lease() {
        let root = TempRoot::existing();
        create_marked_empty_store(root.path());
        create_valid_g1_agent(root.path());
        let filesystem = Arc::new(DeterministicPersistentFaultFilesystem::new([]));
        ...
        assert!(
            ...
            "the dead actor cannot claim metadata publication completion"
        );
        assert_eq!(
            actor_task.wait().await,
            Err(RuntimeTaskError::WorkerUnavailable)
        ...
        assert!(
            state
                ...
                .try_write_owned()
                .is_err(),
            "the owner-tracked post-COMMITTED worker clone retains the Agent gate"
        );
        ...
        release
            .send(())
            .expect("the committed metadata publication resumes");
        close.await;
        let reopened = open(root.path())
            .await
            .expect("post-COMMITTED metadata actor loss reopens");
        ...
        assert_eq!(head.storage_generation().get(), 2);
        assert_eq!(head.metadata().revision().get(), 2);
        assert_eq!(head.metadata().name(), "Planner revised");
        reopened.close().await;
    }
    ...
    async fn native_process_abort_session_fork_child() {
        if std::env::var("MINICORE_TEST_SESSION_FORK_CRASH_CHILD").as_deref() != Ok("v1") {
            return;
        }
        let root = PathBuf::from(
            std::env::var_os("MINICORE_TEST_CRASH_ROOT")
                .expect("the parent supplies the Session Fork crash root"),
        );
        let candidate: SessionId = std::env::var("MINICORE_TEST_CRASH_SESSION")
            .expect("the parent supplies the Fork child Session")
            .parse()
            .expect("the Fork child Session ID is canonical");
        let operation = match std::env::var("MINICORE_TEST_CRASH_POINT").as_deref() {
            Ok("before_published") => PublicationOperation::SyncCommittedGenerationDirectory,
            Ok("after_published") => PublicationOperation::SyncPublishedSessionsDirectory,
            _ => panic!("the parent supplies a known Session Fork crash coordinate"),
        };
        let proof_path = PathBuf::from(
            std::env::var_os("MINICORE_TEST_CRASH_PROOF")
                .expect("the parent supplies the Session Fork crash proof path"),
        );
        let proof = std::env::var("MINICORE_TEST_CRASH_NONCE")
            .expect("the parent supplies the Session Fork crash proof nonce");
        let filesystem = Arc::new(DeterministicPersistentFaultFilesystem::new([]));
        let state = open_with_reservation_filesystem(&root, Arc::clone(&filesystem)).await;
        filesystem.abort_after_publication_operation(operation, proof_path, proof);
        let source = SessionId::from_str(SESSION_ONE).unwrap();
    ...
    async fn native_process_abort_reopens_session_fork_as_invisible_or_published() {
        for (point, candidate, published) in [
            ("before_published", session_candidate(93), false),
            ("after_published", session_candidate(94), true),
        ] {
            let root = TempRoot::existing();
            create_marked_empty_store(root.path());
            create_valid_g1_agent(root.path());
            create_valid_g1_session(root.path(), &ordinary_header_only_conversation(SESSION_ONE));
            run_native_abort_session_fork_child(root.path(), candidate, point);
            ...
            assert!(
                reservation.is_file(),
                "{point} retains the burned Fork child ID"
            );
            assert!(generation.join("COMMITTED").is_file(), "{point}");
            assert_eq!(entity.join("PUBLISHED").is_file(), published, "{point}");
            ...
            let reopened = open(root.path())
                .await
                .unwrap_or_else(|error| panic!("{point} Session Fork store reopens: {error:?}"));
            assert!(
                reopened
                    .session_head(SessionId::from_str(SESSION_ONE).unwrap())
                    .is_some(),
                "{point} retains the published source"
            );
            assert_eq!(
                reopened.session_head(candidate).is_some(),
                published,
            ...
            if !published {
                assert!(
                    !entity.exists(),
                    "restart exactly cleans the invisible aborted Fork child"
                );
            }
            reopened.close().await;
        }
