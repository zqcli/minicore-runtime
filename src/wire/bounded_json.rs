...
impl JsonParseLimits {
    ...
    #[allow(
        dead_code,
        reason = "M5 Store V1 document codec is consumed by future DurableState recovery"
    )]
    pub(super) const fn durable_store(max_encoded_bytes: usize) -> Self {
        Self {
            max_encoded_bytes: WireLimit::new(max_encoded_bytes),
            max_depth: WireLimit::new(64),
            max_array_items: WireLimit::new(4_096),
            max_object_members: WireLimit::new(256),
            max_string_bytes: WireLimit::new(262_144),
            max_nodes: None,
            canonicalize_numbers: false,
        }
    }
...
pub(super) fn decode_json_string_token(
    input: &[u8],
    maximum_decoded_bytes: usize,
) -> Result<Box<str>, BoundedJsonError> {
    let input = std::str::from_utf8(input).map_err(|_| BoundedJsonError::InvalidUtf8)?;
    let limits = JsonParseLimits {
        max_encoded_bytes: WireLimit::new(input.len()),
        max_depth: WireLimit::new(1),
        max_array_items: WireLimit::new(0),
        max_object_members: WireLimit::new(0),
        max_string_bytes: WireLimit::new(maximum_decoded_bytes),
        max_nodes: None,
        canonicalize_numbers: false,
    };
