...
pub(super) struct CloseCompletion {
    sender: watch::Sender<Option<Result<(), LegacySessionError>>>,
}

#[derive(Clone)]
pub(super) struct CloseCompletionWaiter {
    receiver: watch::Receiver<Option<Result<(), LegacySessionError>>>,
}
...
impl CloseCompletion {
    ...
    pub(super) fn complete(&self, result: Result<(), LegacySessionError>) {
        self.sender.send_replace(Some(result));
    }
}

impl CloseCompletionWaiter {
    async fn wait(mut self) -> Result<(), LegacySessionError> {
        loop {
...
pub(crate) enum SessionCommand {
    Submit {
        input: String,
        reply: oneshot::Sender<Result<TurnId, LegacySessionError>>,
    },
    Answer {
        interaction_id: InteractionId,
        answer: LegacyUserAnswer,
        reply: oneshot::Sender<Result<(), LegacySessionError>>,
    },
}
...
impl SessionHandle {
    ...
    pub(crate) async fn submit(&self, input: String) -> Result<TurnId, LegacySessionError> {
        validate_user_text(&input).map_err(|_| LegacySessionError::InvalidInput)?;
        ...
        self.commands
            ...
            .map_err(|error| match error {
                mpsc::error::TrySendError::Full(_) => LegacySessionError::Busy,
                mpsc::error::TrySendError::Closed(_) => LegacySessionError::Closing,
            })?;
        receiver.await.map_err(|_| LegacySessionError::Closing)?
    }
    ...
    pub(crate) fn cancel(&self) -> Result<(), LegacySessionError> {
        if self.close_requested.is_cancelled() {
            return Err(LegacySessionError::Closing);
        }
    ...
    pub(crate) async fn close(&self) -> Result<(), LegacySessionError> {
