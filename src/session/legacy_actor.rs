...
use super::legacy_command::{CancelSlot, CloseCompletion, SessionCommand, SessionHandle};
...
use crate::error::{LegacySessionError, PublicErrorCode, PublicErrorSummary};
...
impl SessionActorDependencies {
    fn validate(&self) -> Result<(), LegacySessionError> {
        if self.close_timeout.is_zero() || self.close_timeout > MAX_CLOSE_TIMEOUT {
            return Err(LegacySessionError::InvalidInput);
        }
        if !(1..=super::legacy_command::MAX_COMMAND_CAPACITY).contains(&self.command_capacity)
...
impl SessionActor {
    pub(crate) async fn new(
    ...
    ) -> Result<(SessionHandle, Self), LegacySessionError> {
        ...
        if stored.health() == ConversationHealth::Degraded {
            return Err(LegacySessionError::Unavailable);
        }
        ...
        let resolved = dependencies
            .model_gateway
            .resolve(&selection)
            .map_err(|_| LegacySessionError::Unavailable)?;
    ...
    pub(crate) async fn run(mut self) -> Result<(), LegacySessionError> {
        loop {
    ...
    async fn handle_idle_command(
        ...
        match command {
            ...
            SessionCommand::Answer { reply, .. } => {
                let _ = reply.send(Err(LegacySessionError::InteractionMismatch));
            }
        }
        Ok(())
    }

    async fn active_step(&mut self) -> Result<(), LegacySessionError> {
        let signal = {
            let active = self.active.as_mut().ok_or(LegacySessionError::Internal)?;
            tokio::select! {
                biased;
    ...
    async fn handle_active_command(
        ...
        match command {
            SessionCommand::Submit { reply, .. } => {
                let _ = reply.send(Err(LegacySessionError::Busy));
            }
            SessionCommand::Answer {
                ...
                let result = if matches!(self.status, LegacySessionStatus::WaitingForInput { .. }) {
                    ...
                    Err(LegacySessionError::InteractionMismatch)
                };
                let _ = reply.send(result);
            }
        }
    ...
    async fn handle_submit(&mut self, input: String) -> Result<TurnId, LegacySessionError> {
        if self.unavailable {
            return Err(LegacySessionError::Unavailable);
        }
        ...
        let turn_id = TurnId::new().map_err(|_| LegacySessionError::Internal)?;
        ...
        let timestamp =
            (self.dependencies.timestamp_source)().map_err(|_| LegacySessionError::Internal)?;
    ...
    fn build_context(
    ...
    ) -> Result<TurnContext, LegacySessionError> {
        TurnContext::new(
        ...
        .map_err(|error| match error {
            crate::agent::LegacyTurnContextError::ModelUnavailable => {
                LegacySessionError::Unavailable
            }
            crate::agent::LegacyTurnContextError::InvalidModelConfiguration
            | crate::agent::LegacyTurnContextError::UnknownTool
            | crate::agent::LegacyTurnContextError::InvalidToolRounds => {
                LegacySessionError::InvalidInput
            }
        })
    }
    ...
    async fn handle_answer(
        ...
        let Some(pending) = self.pending.as_ref() else {
            return Err(LegacySessionError::InteractionMismatch);
        };
        ...
        let cancellation = self
            ...
            .ok_or(LegacySessionError::InteractionMismatch)?;
        ...
        let response = {
            ...
            if !owns_turn || self.close_requested.is_cancelled() || cancellation.is_cancelled() {
                ...
                return Err(if close_requested {
                    LegacySessionError::Closing
                } else {
                    LegacySessionError::InteractionMismatch
                });
            }
            let mut pending = self.pending.take().ok_or(LegacySessionError::Internal)?;
        ...
        let timestamp = match (self.dependencies.timestamp_source)() {
            ...
            Err(_) => {
                ...
                self.force_failure(TurnFailure::Internal);
                return Err(LegacySessionError::Internal);
            }
        };
    ...
    async fn handle_runner_event(&mut self, event: RunnerEvent) -> Result<(), LegacySessionError> {
        let turn_id = self
            .active
            .as_ref()
            .ok_or(LegacySessionError::Internal)?
            .turn_id;
    ...
    async fn drain_runner_events(&mut self) -> Result<(), LegacySessionError> {
        loop {
            let events = {
    ...
    fn mark_closing(&mut self) -> Result<(), LegacySessionError> {
        if self.status == LegacySessionStatus::Closing {
            return Ok(());
        }
        self.reject_pending();
    ...
    async fn close_session(
        &mut self,
        prior_error: Option<LegacySessionError>,
    ) -> Result<(), LegacySessionError> {
            ...
            match waited {
                Ok(result) => {
                    if self.drain_runner_events().await.is_err() && close_error.is_none() {
                        close_error = Some(LegacySessionError::Internal);
                        self.force_failure(TurnFailure::Internal);
                    }
    ...
    fn drain_commands(&mut self) {
        while let Ok(command) = self.commands.try_recv() {
            match command {
                SessionCommand::Submit { reply, .. } => {
                    let _ = reply.send(Err(LegacySessionError::Closing));
                }
                SessionCommand::Answer { reply, .. } => {
                    let _ = reply.send(Err(LegacySessionError::Closing));
                }
    ...
    fn snapshot(&self) -> Result<LegacySessionSnapshot, LegacySessionError> {
        ...
        .map_err(|_| LegacySessionError::Internal)
    }

    fn publish_snapshot(&self) -> Result<(), LegacySessionError> {
        let snapshot = self.snapshot()?;
        self.observation.publish_snapshot(snapshot)
    }

    fn publish_event(&self, event: LegacySessionEvent) -> Result<(), LegacySessionError> {
        let snapshot = self.snapshot()?;
...
fn map_conversation_error(error: ConversationError) -> LegacySessionError {
    match error {
        ...
        | ConversationError::IncompleteToolExchange => LegacySessionError::Unavailable,
        ConversationError::Closing => LegacySessionError::Closing,
        ConversationError::Busy => LegacySessionError::Busy,
        ConversationError::Corrupt
        | ConversationError::CorruptAt { .. }
        | ConversationError::TooLarge
        | ConversationError::NotFound
        | ConversationError::Stale => LegacySessionError::Internal,
    }
}
...
mod tests {
    ...
    async fn new_rejects_model_without_disabled_reasoning_support() {
        ...
        assert!(matches!(
            result,
            Err(crate::error::LegacySessionError::InvalidInput)
        ));
        log.close().await.unwrap();
        workspace.shutdown().await.unwrap();
        store.shutdown().await.unwrap();
    ...
    async fn ask_user_waits_persists_interaction_before_resume_and_rejects_stale_answers() {
        ...
        assert_eq!(
            ...
            Err(crate::error::LegacySessionError::InteractionMismatch)
        );
        assert!(matches!(
            handle.snapshot().status(),
            LegacySessionStatus::WaitingForInput {
    ...
    async fn close_first_rejects_answer_without_resuming_tool_work() {
        ...
        assert_eq!(
            handle
                ...
                .await,
            Err(crate::error::LegacySessionError::Closing)
        );
        close.await.unwrap();
        assert!(
            order
    ...
    async fn failed_interaction_append_rejects_claimed_response_without_hanging() {
        ...
        assert_eq!(
            handle
                ...
                .await,
            Err(crate::error::LegacySessionError::Internal)
        );
        assert!(matches!(
            stream.recv().await,
            Some(LegacySessionEvent::TurnFinished {
    ...
    async fn submit_validates_user_text_before_mailbox_full() {
        ...
        assert!(matches!(
            ...
            Poll::Ready(Err(crate::error::LegacySessionError::Busy))
        ));
        drop(accepted);
        drop(actor);
        log.close().await.unwrap();
    ...
    async fn second_submit_is_busy_without_an_implicit_queue() {
        ...
        assert_eq!(
            ...
            Err(crate::error::LegacySessionError::Busy)
        );
        handle.cancel().unwrap();
        let _ = stream.recv().await;
        handle.close().await.unwrap();
    ...
    async fn actor_step_failure_is_preserved_for_close_completion() {
        ...
        let actor_task = tokio::runtime::Handle::current().spawn(actor.run());
        assert_eq!(handle.close().await, Err(LegacySessionError::Internal));
        assert_eq!(actor_task.await.unwrap(), Err(LegacySessionError::Internal));
        store.shutdown().await.unwrap();
        workspace.shutdown().await.unwrap();
        fs::remove_dir_all(root).unwrap();
    }
