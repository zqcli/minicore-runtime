...
use super::legacy_types::{LegacyToolError, LegacyUserAnswer, LegacyUserQuestion};
use LegacyToolError as ToolError;
...
struct RequestState {
    ...
    active: AtomicBool,
    reply: Mutex<Option<oneshot::Sender<Result<LegacyUserAnswer, ToolError>>>>,
}

pub(crate) struct InteractionResponse {
    reply: Option<oneshot::Sender<Result<LegacyUserAnswer, ToolError>>>,
}

impl InteractionResponse {
    pub(crate) fn respond(mut self, answer: LegacyUserAnswer) -> Result<(), ToolError> {
...
impl RequestState {
    fn new(
        ...
        slot_id: InteractionSlotId,
        reply: oneshot::Sender<Result<LegacyUserAnswer, ToolError>>,
    ) -> Arc<Self> {
        Arc::new(Self {
    ...
    fn take_reply(&self) -> Option<oneshot::Sender<Result<LegacyUserAnswer, ToolError>>> {
        self.reply
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .take()
    ...
    fn finish(
        self: &Arc<Self>,
        result: Result<LegacyUserAnswer, ToolError>,
    ) -> Result<(), ToolError> {
...
pub(crate) struct InteractionClient {
    inner: Arc<InteractionChannelInner>,
}

pub(crate) struct InteractionReceiver {
    inner: Arc<InteractionChannelInner>,
}
...
impl InteractionClient {
    pub(crate) fn channel() -> (Self, InteractionReceiver) {
    ...
    ) -> Result<LegacyUserAnswer, ToolError> {
...
impl InteractionReceiver {
    pub(crate) async fn recv(&mut self) -> Option<InteractionRequest> {
        loop {
...
impl InteractionRequest {
    ...
    ) -> Result<LegacyUserQuestion, ToolError> {
        if !self.inner.state.is_active() {
            return Err(ToolError::InteractionClosed);
        }
        LegacyUserQuestion::new(
            interaction_id,
    ...
    pub(crate) fn respond(self, answer: LegacyUserAnswer) -> Result<(), ToolError> {
        self.claim_response()?.respond(answer)
    }

    pub(crate) fn reject(self, error: ToolError) -> Result<(), ToolError> {
...
pub(crate) struct LegacyToolContext<'a> {
...
impl<'a> LegacyToolContext<'a> {
    ...
    pub(crate) const fn workspace(&self) -> &'a Workspace {
        self.workspace
    }
    ...
    pub(crate) const fn interactions(&self) -> &'a InteractionClient {
        self.interactions
    }
...
const _: () = {
    // P6 deletion target: remove with the legacy runner interaction context.
    let _ = InteractionRequest::question;
    let _ = InteractionRequest::choices;
    let _ = InteractionRequest::respond;
    let _ = LegacyToolContext::session_id;
    let _ = LegacyToolContext::turn_id;
    let _ = LegacyToolContext::workspace;
    let _ = LegacyToolContext::cancellation;
    let _ = LegacyToolContext::interactions;
};

#[cfg(test)]
mod tests {
    use super::{
        InteractionChannelState, InteractionClient, InteractionId, LegacyUserAnswer, ToolError,
        TurnId,
    };
    use tokio_util::sync::CancellationToken;
    ...
    async fn claimed_response_owns_sender_and_responds_without_request_finish() {
        ...
        response
            .respond(LegacyUserAnswer::new("answer").unwrap())
            .unwrap();
    ...
    async fn claimed_response_wins_after_runner_cancellation_attempt() {
        ...
        cancellation.cancel();
        response
            .respond(LegacyUserAnswer::new("accepted").unwrap())
            .unwrap();
